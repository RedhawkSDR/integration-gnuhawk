...
#include "gr_head_octet.h"

PREPARE_LOGGING(gr_head_octet_i)

gr_head_octet_i::gr_head_octet_i(const char *uuid, const char *label) : 
    gr_head_octet_base(uuid, label)
{
}

gr_head_octet_i::~gr_head_octet_i()
{
}
...
/***********************************************************************************************
    ...
    Properties:
        ...
        setPropertyChangeListener(<property name>, this, &gr_head_octet::<callback method>)
        in the constructor.
        ...
        //Add to gr_head_octet.cpp
        gr_head_octet_i::gr_head_octet_i(const char *uuid, const char *label) :
            gr_head_octet_base(uuid, label)
        {
            setPropertyChangeListener("scaleValue", this, &gr_head_octet_i::scaleChanged);
        }

        void gr_head_octet_i::scaleChanged(const std::string& id){
            std::cout << "scaleChanged scaleValue " << scaleValue << std::endl;
        }
            
        //Add to gr_head_octet.h
        void scaleChanged(const std::string&);
        
        
************************************************************************************************/
int gr_head_octet_i::serviceFunction()
{
    LOG_DEBUG(gr_head_octet_i, "serviceFunction() example log message");
    if (done)
         return NOOP;

     BULKIO_dataOctet_In_i::dataTransfer *tmp = octet_in->getPacket(-1);
     if (not tmp) { // No data is available
         return NOOP;
     }
     ...
     if (num > 0){
         if (num <= (int)tmp->dataBuffer.size()){
             ...
             for (unsigned int i=0; i<(unsigned int)num; i++) {
                 outputData[i] = (unsigned char)tmp->dataBuffer[i];
             }
             // NOTE: You must make at least one valid pushSRI call
             if (tmp->sriChanged) {
                 octet_out->pushSRI(tmp->SRI);
             }
             octet_out->pushPacket(outputData, tmp->T, true, tmp->streamID);
             done = true;
         }else{
             ...
             if (numToPush > 0){
                 ...
                 if (numToPush < (int)tmp->dataBuffer.size()){
                     ...
                     octet_out->pushPacket(outputData, tmp->T, tmp->EOS, tmp->streamID);
                 }
             }
         }
     } else {
