...
/**************************************************************************
    ...
    the ports can also be done from this class
 	...
 	Generated on: Thu Feb 07 12:37:20 EST 2013
 	Redhawk IDE
 	Version:R.1.8.2
...
#include "gr_head_float.h"

PREPARE_LOGGING(gr_head_float_i)

gr_head_float_i::gr_head_float_i(const char *uuid, const char *label) : 
    gr_head_float_base(uuid, label)
{
    numOutCount=0;
}

gr_head_float_i::~gr_head_float_i()
{
}
...
/***********************************************************************************************
    ...
    Properties:
        ...
        setPropertyChangeListener(<property name>, this, &gr_head_float::<callback method>)
        in the constructor.
        ...
        //Add to gr_head_float.cpp
        gr_head_float_i::gr_head_float_i(const char *uuid, const char *label) :
            gr_head_float_base(uuid, label)
        {
            setPropertyChangeListener("scaleValue", this, &gr_head_float_i::scaleChanged);
        }

        void gr_head_float_i::scaleChanged(const std::string& id){
            std::cout << "scaleChanged scaleValue " << scaleValue << std::endl;
        }
            
        //Add to gr_head_float.h
        void scaleChanged(const std::string&);
        
        
************************************************************************************************/
int gr_head_float_i::serviceFunction()
{
    LOG_DEBUG(gr_head_float_i, "serviceFunction() example log message");


    BULKIO_dataFloat_In_i::dataTransfer *tmp = float_in->getPacket(-1);
    if (not tmp) { // No data is available
        return NOOP;
    }
    ...
    if (num > 0){
        if (num <= (int)tmp->dataBuffer.size()){
            ...
            float_out->pushPacket(outputData, tmp->T, true, tmp->streamID);
            done = true;
        }else{
