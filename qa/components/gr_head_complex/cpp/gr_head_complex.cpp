...
PREPARE_LOGGING(gr_head_complex_i)

gr_head_complex_i::gr_head_complex_i(const char *uuid, const char *label) : 
    gr_head_complex_base(uuid, label)
{
    numOutCount=0;
}

gr_head_complex_i::~gr_head_complex_i()
{
}
...
/***********************************************************************************************
    ...
    Properties:
        ...
        setPropertyChangeListener(<property name>, this, &gr_head_complex::<callback method>)
        in the constructor.
        ...
        //Add to gr_head_complex.cpp
        gr_head_complex_i::gr_head_complex_i(const char *uuid, const char *label) :
            gr_head_complex_base(uuid, label)
        {
            setPropertyChangeListener("scaleValue", this, &gr_head_complex_i::scaleChanged);
        }

        void gr_head_complex_i::scaleChanged(const std::string& id){
            std::cout << "scaleChanged scaleValue " << scaleValue << std::endl;
        }
            
        //Add to gr_head_complex.h
        void scaleChanged(const std::string&);
        
        
************************************************************************************************/
int gr_head_complex_i::serviceFunction()
{
    LOG_DEBUG(gr_head_complex_i, "serviceFunction() example log message");

    BULKIO_dataFloat_In_i::dataTransfer *tmp = complex_in->getPacket(-1);
    if (not tmp) { // No data is available
        return NOOP;
    }
    ...
    std::vector<float> outputData;
    if (num > 0){
        if (num*2 <= (int)tmp->dataBuffer.size()){
            outputData.resize(num*2);
            for (unsigned int i=0; i<(unsigned int)num*2; i++) {
                outputData[i] = (float)tmp->dataBuffer[i];
            }
            // NOTE: You must make at least one valid pushSRI call
            if (tmp->sriChanged) {
                complex_out->pushSRI(tmp->SRI);
            }
            complex_out->pushPacket(outputData, tmp->T, true, tmp->streamID);
            done = true;
        }else{
            int numToPush = tmp->dataBuffer.size();
            if ((numOutCount+ (int)tmp->dataBuffer.size()) > num*2)
                numToPush = num*2 - numOutCount;
            if ((numToPush > 0)||(tmp->EOS)){
                ...
                if (numToPush < (int)tmp->dataBuffer.size()){
                    ...
                    complex_out->pushPacket(outputData, tmp->T, tmp->EOS, tmp->streamID);
                    done = tmp->EOS;
                }
            }
        }
