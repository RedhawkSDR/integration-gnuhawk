...
#include "gr_head_int.h"

PREPARE_LOGGING(gr_head_int_i)

gr_head_int_i::gr_head_int_i(const char *uuid, const char *label) : 
    gr_head_int_base(uuid, label)
{
   numOutCount = 0;
}

gr_head_int_i::~gr_head_int_i()
{
}
...
/***********************************************************************************************
    ...
    Properties:
        ...
        setPropertyChangeListener(<property name>, this, &gr_head_int::<callback method>)
        in the constructor.
        ...
        //Add to gr_head_int.cpp
        gr_head_int_i::gr_head_int_i(const char *uuid, const char *label) :
            gr_head_int_base(uuid, label)
        {
            setPropertyChangeListener("scaleValue", this, &gr_head_int_i::scaleChanged);
        }

        void gr_head_int_i::scaleChanged(const std::string& id){
            std::cout << "scaleChanged scaleValue " << scaleValue << std::endl;
        }
            
        //Add to gr_head_int.h
        void scaleChanged(const std::string&);
        
        
************************************************************************************************/
int gr_head_int_i::serviceFunction()
{
    LOG_DEBUG(gr_head_int_i, "serviceFunction() example log message");

    BULKIO_dataLong_In_i::dataTransfer *tmp = long_in->getPacket(-1);
    if (not tmp) { // No data is available
        return NOOP;
    }
    ...
    std::vector<CORBA::Long> outputData;
    if (num > 0){
        if (num <= (int)tmp->dataBuffer.size()){
            outputData.resize(num);
            for (unsigned int i=0; i<(unsigned int)num; i++) {
                outputData[i] = (CORBA::Long)tmp->dataBuffer[i];
            }
            // NOTE: You must make at least one valid pushSRI call
            if (tmp->sriChanged) {
                long_out->pushSRI(tmp->SRI);
            }
            long_out->pushPacket(outputData, tmp->T, true, tmp->streamID);
            done = true;
        }else{
            ...
            if ((numToPush > 0)||(tmp->EOS)){
                ...
                if (numToPush < (int)tmp->dataBuffer.size()){
                    ...
                    long_out->pushPacket(outputData, tmp->T, tmp->EOS, tmp->streamID);
                }
            }
        }
    } else {
