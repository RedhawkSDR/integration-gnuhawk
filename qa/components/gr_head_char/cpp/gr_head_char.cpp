...
#include "gr_head_char.h"

PREPARE_LOGGING(gr_head_char_i)

gr_head_char_i::gr_head_char_i(const char *uuid, const char *label) : 
    gr_head_char_base(uuid, label)
{
    numOutCount = 0;
}

gr_head_char_i::~gr_head_char_i()
{
}
...
/***********************************************************************************************
    ...
    Properties:
        ...
        setPropertyChangeListener(<property name>, this, &gr_head_char::<callback method>)
        in the constructor.
        ...
        //Add to gr_head_char.cpp
        gr_head_char_i::gr_head_char_i(const char *uuid, const char *label) :
            gr_head_char_base(uuid, label)
        {
            setPropertyChangeListener("scaleValue", this, &gr_head_char_i::scaleChanged);
        }

        void gr_head_char_i::scaleChanged(const std::string& id){
            std::cout << "scaleChanged scaleValue " << scaleValue << std::endl;
        }
            
        //Add to gr_head_char.h
        void scaleChanged(const std::string&);
        
        
************************************************************************************************/
int gr_head_char_i::serviceFunction()
{
    LOG_DEBUG(gr_head_char_i, "serviceFunction() example log message");

     BULKIO_dataChar_In_i::dataTransfer *tmp = char_in->getPacket(-1);
     if (not tmp) { // No data is available
         return NOOP;
     }
     ...
     if (num > 0){
         if (num <= (int)tmp->dataBuffer.size()){
             ...
             for (unsigned int i=0; i<(unsigned int)num; i++) {
                 outputData[i] = (char)tmp->dataBuffer[i];
             }
             // NOTE: You must make at least one valid pushSRI call
             if (tmp->sriChanged) {
                 char_out->pushSRI(tmp->SRI);
             }
             char_out->pushPacket(outputData, tmp->T, true, tmp->streamID);
             done = true;
         }else{
             ...
             if ((numToPush > 0)||(tmp->EOS)){
                 ...
                 if (numToPush < (int)tmp->dataBuffer.size()){
                     ...
                     char_out->pushPacket(outputData, tmp->T, tmp->EOS, tmp->streamID);
                 }
             }
         }
     } else {
