...
def fft_filter_ccc(decimation=1, taps=[],nthreads=1):
    c = sb.Component(components_dir+'fft_filter_ccc/fft_filter_ccc.spd.xml', execparams=execparams)
    c.decimation = decimation
    ctaps=[{'real':x.real,'imag':x.imag } for x in taps ]
    c.taps = ctaps
    c.nthreads = 1
    return c

def fir_filter_ccc(decimation=1, taps=[]):
    c = sb.Component(components_dir+'fir_filter_ccc/fir_filter_ccc.spd.xml', execparams=execparams)
    c.decimation = decimation
    ctaps=[{'real':x.real,'imag':x.imag } for x in taps ]
    c.taps = ctaps
    return c
...
def fir_filter_scc(decimation=1, taps=[]):
    c = sb.Component(components_dir+'fir_filter_scc/fir_filter_scc.spd.xml', execparams=execparams)
    c.decimation = decimation
    ctaps=[{'real':x.real,'imag':x.imag } for x in taps ]
    c.ctaps = taps 
    return c
...
def sig_source_c(sampling_freq=10000.0, waveform=101, frequency=10.0, amplitude=1000.0, offset=0.0):
    c = sb.Component(components_dir+'components_with_complex_props/sig_source_c/sig_source_c.spd.xml', execparams=execparams)
    c.waveform = waveform
