...
#ifndef SUB_CC_1I_IMPL_BASE_H
#define SUB_CC_1I_IMPL_BASE_H
...
#include "sub_cc_1i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class sub_cc_1i_base;
...
class sub_cc_1i_base : public GnuHawkBlock
{
    public:
        sub_cc_1i_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<sub_cc_1i_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( sub_cc_1i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( sub_cc_1i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( sub_cc_1i_base, " serviceFunction: retval:" << retval);
