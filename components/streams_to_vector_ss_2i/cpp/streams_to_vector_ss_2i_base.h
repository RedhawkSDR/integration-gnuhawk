...
#ifndef STREAMS_TO_VECTOR_SS_2I_IMPL_BASE_H
#define STREAMS_TO_VECTOR_SS_2I_IMPL_BASE_H
...
#include "streams_to_vector_ss_2i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class streams_to_vector_ss_2i_base;
...
class streams_to_vector_ss_2i_base : public GnuHawkBlock
{
    public:
        streams_to_vector_ss_2i_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<streams_to_vector_ss_2i_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( streams_to_vector_ss_2i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( streams_to_vector_ss_2i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( streams_to_vector_ss_2i_base, " serviceFunction: retval:" << retval);
