...
add_const_vss_i::add_const_vss_i(const char *uuid, const char *label) :
add_const_vss_base(uuid, label)
{
}

add_const_vss_i::~add_const_vss_i()
{
}
...
//
void add_const_vss_i::createBlock()
{
  //
  ...
  setupIOMappings();
  this->registerGetterSetter("k", &gr_add_const_vss::k, &gr_add_const_vss::set_k);
} 
...
//      
BULKIO::StreamSRI add_const_vss_i::createOutputSRI( int32_t idx)
{
  //
