...
#ifndef ADD_CONST_VSS_IMPL_BASE_H
#define ADD_CONST_VSS_IMPL_BASE_H
...
#include "add_const_vss_GnuHawkBlock.h"


class add_const_vss_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataShort_In_i::SRIListener {
    ...
    friend class  add_const_vss_base;
  ...
  SRIListener( add_const_vss_base &parent ) : _parent(parent) {};

    add_const_vss_base &_parent;
  };
    

    public:

        add_const_vss_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataShort_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataShort_Out_i >::List   OStreamList;  
   ...
   ProcessThread<add_const_vss_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( add_const_vss_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( add_const_vss_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( add_const_vss_base, " serviceFunction: retval:" << retval);
