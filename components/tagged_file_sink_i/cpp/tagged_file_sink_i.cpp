...
tagged_file_sink_i_i::tagged_file_sink_i_i(const char *uuid, const char *label) :
tagged_file_sink_i_base(uuid, label)
{
}

tagged_file_sink_i_i::~tagged_file_sink_i_i()
{
}
...
void tagged_file_sink_i_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	try {
		gr_sptr = gr_make_tagged_file_sink (this->itemsize, this->samp_rate);
	}
	catch(...){
		return;
	}
