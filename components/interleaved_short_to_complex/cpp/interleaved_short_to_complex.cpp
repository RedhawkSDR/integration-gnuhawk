...
interleaved_short_to_complex_i::interleaved_short_to_complex_i(const char *uuid, const char *label) :
interleaved_short_to_complex_base(uuid, label)
{
}

interleaved_short_to_complex_i::~interleaved_short_to_complex_i()
{
}
...
void interleaved_short_to_complex_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_interleaved_short_to_complex();
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI interleaved_short_to_complex_i::createOutputSRI( int32_t idx)
{
    //
