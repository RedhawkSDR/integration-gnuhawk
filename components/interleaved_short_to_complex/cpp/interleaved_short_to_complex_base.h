...
#ifndef INTERLEAVED_SHORT_TO_COMPLEX_IMPL_BASE_H
#define INTERLEAVED_SHORT_TO_COMPLEX_IMPL_BASE_H
...
#include "interleaved_short_to_complex_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class interleaved_short_to_complex_base;
...
class interleaved_short_to_complex_base : public GnuHawkBlock
{
    public:
        interleaved_short_to_complex_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<interleaved_short_to_complex_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Ports
        bulkio::InShortPort *short_in;
    ...
    protected:
        ...
        void  short_in_newStreamCallback( BULKIO::StreamSRI &sri );
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( interleaved_short_to_complex_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( interleaved_short_to_complex_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( interleaved_short_to_complex_base, " serviceFunction: retval:" << retval);
