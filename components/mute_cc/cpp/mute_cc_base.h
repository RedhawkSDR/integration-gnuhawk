...
#ifndef MUTE_CC_IMPL_BASE_H
#define MUTE_CC_IMPL_BASE_H
...
#include "mute_cc_GnuHawkBlock.h"
...
class mute_cc_base : public GnuHawkBlock
{
    public:
        mute_cc_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<mute_cc_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( mute_cc_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( mute_cc_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( mute_cc_base, " serviceFunction: retval:" << retval);
