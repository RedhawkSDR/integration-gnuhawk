...
void int_to_float_base::construct()
{
    ...
    long_in->setNewStreamListener(this, &int_to_float_base::long_in_newStreamCallback);
...
void int_to_float_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        long_in->unblock();
        serviceThread = new ProcessThread<int_to_float_base>(this, 0.1);
        serviceThread->start();
    }
...
// Destructor
int_to_float_base::~int_to_float_base()
{
    // Free input streams
...
void int_to_float_base::long_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( int_to_float_base, "START NotifySRI  port:stream " << long_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( long_in, sri ) );

  LOG_TRACE( int_to_float_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << long_in->getName() << "/" << sri.streamID); 
}
...
BULKIO::StreamSRI int_to_float_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI int_to_float_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
...
int int_to_float_base::_transformerServiceFunction( std::vector< gr_istream_base * > &istreams ,
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > (*istream)->nitems() ) {
                        LOG_WARN( int_to_float_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE "
...
int int_to_float_base::_forecastAndProcess( bool &eos, std::vector< gr_istream_base * > &istreams ,
                                 std::vector< gr_ostream_base * > &ostreams  )
{
    typedef std::vector< gr_istream_base * >   _IStreamList;
    typedef std::vector< gr_ostream_base * >  _OStreamList;
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( int_to_float_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << 
        ...
        if ( !gr_sptr->fixed_rate() )  {
            ...
            LOG_TRACE( int_to_float_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( int_to_float_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   (*istream)->nelems() << 
        ...
        if ( !dataReady ) {
            LOG_TRACE( int_to_float_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0]->nitems() << 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( int_to_float_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems 
...
// use incoming packet's time stamp to forward
      ...
      LOG_DEBUG( int_to_float_base, "                  mode:" <<  pktTs.tcmode );
      LOG_DEBUG( int_to_float_base, "                status:" <<  pktTs.tcstatus );
      LOG_DEBUG( int_to_float_base, "                offset:" <<  pktTs.toff );
      LOG_DEBUG( int_to_float_base, "                 whole:" <<  std::setprecision(10) << pktTs.twsec );
      LOG_DEBUG( int_to_float_base, "PKT - TS          frac:" <<  std::setprecision(12) << pktTs.tfsec );
#endif
