...
#ifndef INT_TO_FLOAT_IMPL_BASE_H
#define INT_TO_FLOAT_IMPL_BASE_H
...
#include "int_to_float_GnuHawkBlock.h"
#include "gr_malloc.h"

class int_to_float_base : public GnuHawkBlock
{
    friend class BULKIO_dataLong_In_i;
    friend class BULKIO_dataFloat_Out_i;

  class SRIListener : BULKIO_dataLong_In_i::SRIListener {

    friend class  int_to_float_base;

  public:
    void operator() ( BULKIO_dataLong_In_i *port, BULKIO::StreamSRI &sri  ) {
      _parent.notifySRI( port, sri );
    };
  
  private:
    
  SRIListener( int_to_float_base &parent ) : _parent(parent) {};

    int_to_float_base &_parent;
  };
    

    public:

        int_to_float_base(const char *uuid, const char *label);
    ...
    void  notifySRI( BULKIO_dataLong_In_i *port, BULKIO::StreamSRI &sri );
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataLong_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  
  typedef _IStream< BULKIO_dataLong_In_i >::List    IStreamList;

  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
    
    
   ProcessThread<int_to_float_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( int_to_float_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( int_to_float_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( int_to_float_base, " serviceFunction: retval:" << retval);
