...
#ifndef SHORT_TO_FLOAT_IMPL_BASE_H
#define SHORT_TO_FLOAT_IMPL_BASE_H
...
#include "short_to_float_GnuHawkBlock.h"
#include "gr_malloc.h"

class short_to_float_base : public GnuHawkBlock
{
    friend class BULKIO_dataShort_In_i;
    friend class BULKIO_dataFloat_Out_i;

  class SRIListener : BULKIO_dataShort_In_i::SRIListener {

    friend class  short_to_float_base;

  public:
    void operator() ( BULKIO_dataShort_In_i *port, BULKIO::StreamSRI &sri  ) {
      _parent.notifySRI( port, sri );
    };
  
  private:
    
  SRIListener( short_to_float_base &parent ) : _parent(parent) {};

    short_to_float_base &_parent;
  };
    

    public:

        short_to_float_base(const char *uuid, const char *label);
    ...
    void  notifySRI( BULKIO_dataShort_In_i *port, BULKIO::StreamSRI &sri );
  ...
  template < typename IN_PORT_TYPE > struct gr_istream {
    ...
    std::vector< typename IN_PORT_TYPE::RH_NativeType, GR_MemAlign< typename IN_PORT_TYPE::RH_NativeType >  >      _data;     // buffered data from port
 ...
 template < typename OUT_PORT_TYPE > struct gr_ostream {
   ...
   std::vector< typename OUT_PORT_TYPE::RH_NativeType, GR_MemAlign< typename OUT_PORT_TYPE::RH_NativeType >   >  _data;    // output buffer used by GR_Block
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataShort_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  
  typedef _IStream< BULKIO_dataShort_In_i >::List    IStreamList;

  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
    
    
   ProcessThread<short_to_float_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( short_to_float_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( short_to_float_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( short_to_float_base, " serviceFunction: retval:" << retval);
