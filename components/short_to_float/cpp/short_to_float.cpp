...
short_to_float_i::short_to_float_i(const char *uuid, const char *label) :
short_to_float_base(uuid, label)
{
}

short_to_float_i::~short_to_float_i()
{
}


float short_to_float_i::getScale()
{
	return this->gr_sptr->scale();
}

void short_to_float_i::changedScale(const std::string &id)
{
	this->gr_sptr->set_scale(this->scale);
}
...
//
void short_to_float_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
	gr_sptr = gr_make_short_to_float( this->vlen, this->scale );
	this->setPropertyChangeListener("scale", this, &short_to_float_i::changedScale);
	this->registerGetValue("scale", this, &short_to_float_i::getScale);
...
//      
BULKIO::StreamSRI short_to_float_i::createOutputSRI( int32_t idx)
{
  //
