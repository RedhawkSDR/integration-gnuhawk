...
//RESOLVE    ENABLE_LOGGING;
    public:
        ...
        BULKIO_dataFloat_Out_i(std::string port_name, short_to_float_base *_parent);
       ...
       template <typename ALLOCATOR>
        void pushPacket(std::vector<CORBA::Float, ALLOCATOR>& data, BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID) {
				 ...
				 for (port = outConnections.begin(); port != outConnections.end(); port++) {
					 ...
					 //RESOLVE LOG_ERROR(BULKIO_dataFloat_Out_i, "Call to pushPacket by BULKIO_dataFloat_Out_i failed");
				     }
				 }
			     }
			 };
    ...
    public:
     ...
     class SRIListener {
       ...
       public:
          ...
          virtual void operator() ( BULKIO_dataShort_In_i *, BULKIO::StreamSRI &  ) = 0;
     };
      
     //BULKIO_dataShort_In_i(std::string port_name, short_to_float_base *_parent);
     BULKIO_dataShort_In_i(std::string port_name, SRIListener *listener=NULL, SRICompare cmp=CompareSRI);
