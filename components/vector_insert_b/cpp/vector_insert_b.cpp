...
vector_insert_b_i::vector_insert_b_i(const char *uuid, const char *label) :
vector_insert_b_base(uuid, label)
{
}

vector_insert_b_i::~vector_insert_b_i()
{
}

void vector_insert_b_i::rewind_data(const std::string &id)
{
    if (this->rewind == true){
        gr_sptr->rewind();
    }
...
void vector_insert_b_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_vector_insert_b( this->data, this->periodicity, this->offset);
    } catch (...) {
        return;
    }

    this->registerSetter("data", &gr_vector_insert_b::set_data);
    this->setPropertyChangeListener("rewind", this, &vector_insert_b_i::rewind_data);
...
//      
BULKIO::StreamSRI vector_insert_b_i::createOutputSRI( int32_t idx)
{
    //
