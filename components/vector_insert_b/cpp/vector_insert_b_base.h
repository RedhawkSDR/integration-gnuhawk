...
#ifndef VECTOR_INSERT_B_IMPL_BASE_H
#define VECTOR_INSERT_B_IMPL_BASE_H
...
#include "vector_insert_b_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class vector_insert_b_base;
...
class vector_insert_b_base : public GnuHawkBlock
{
    public:
        vector_insert_b_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<vector_insert_b_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::Long periodicity;
        CORBA::Long offset;
        bool rewind;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( vector_insert_b_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( vector_insert_b_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( vector_insert_b_base, " serviceFunction: retval:" << retval);
