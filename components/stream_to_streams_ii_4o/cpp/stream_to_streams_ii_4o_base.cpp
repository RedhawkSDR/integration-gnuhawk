...
BULKIO::StreamSRI stream_to_streams_ii_4o_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int stream_to_streams_ii_4o_base::_transformerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int stream_to_streams_ii_4o_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
                                 typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
    typedef typename std::vector< gr_istream< IN_PORT_TYPE > >   _IStreamList;
    typedef typename std::vector< gr_ostream< OUT_PORT_TYPE > >  _OStreamList;
...
// use incoming packet's time stamp to forward
      ...
      LOG_DEBUG( stream_to_streams_ii_4o_base, "                  mode:" <<  pkt->T.tcmode );
      LOG_DEBUG( stream_to_streams_ii_4o_base, "                status:" <<  pkt->T.tcstatus );
      LOG_DEBUG( stream_to_streams_ii_4o_base, "                offset:" <<  pkt->T.toff );
      LOG_DEBUG( stream_to_streams_ii_4o_base, "                 whole:" <<  std::setprecision(10) << pkt->T.twsec );
      LOG_DEBUG( stream_to_streams_ii_4o_base, "PKT - TS          frac:" <<  std::setprecision(12) << pkt->T.tfsec );
#endif
