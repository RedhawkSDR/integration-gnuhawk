...
#ifndef BINARY_SLICER_FB_IMPL_BASE_H
#define BINARY_SLICER_FB_IMPL_BASE_H
...
#include "binary_slicer_fb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class binary_slicer_fb_base;
...
class binary_slicer_fb_base : public GnuHawkBlock
{
    public:
        binary_slicer_fb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<binary_slicer_fb_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( binary_slicer_fb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( binary_slicer_fb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( binary_slicer_fb_base, " serviceFunction: retval:" << retval);
