...
head_ss_i::head_ss_i(const char *uuid, const char *label) :
head_ss_base(uuid, label)
{
}
...
void head_ss_i::reset_data(const std::string &id)
{
    if (this->reset == true) {
        gr_sptr->reset();
    }
...
void head_ss_i::createBlock()
{
    ...
    try {
        gr_sptr = gr_make_head( this->sizeof_stream_item, this->nitems );
    } catch (...) {
        return;
    }

    this->registerSetter("nitems", &gr_head::set_length);
    this->setPropertyChangeListener("reset", this, &head_ss_i::reset_data);
...
//      
BULKIO::StreamSRI head_ss_i::createOutputSRI( int32_t idx)
{
    //
