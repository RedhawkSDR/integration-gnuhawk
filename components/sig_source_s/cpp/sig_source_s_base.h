...
#ifndef SIG_SOURCE_S_IMPL_BASE_H
#define SIG_SOURCE_S_IMPL_BASE_H
...
#include "sig_source_s_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class sig_source_s_base;
...
class sig_source_s_base : public GnuHawkBlock
{
    public:
        sig_source_s_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<sig_source_s_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( sig_source_s_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( sig_source_s_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( sig_source_s_base, " serviceFunction: retval:" << retval);
