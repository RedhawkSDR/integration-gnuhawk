...
sig_source_s_i::sig_source_s_i(const char *uuid, const char *label) :
sig_source_s_base(uuid, label)
{
}

sig_source_s_i::~sig_source_s_i()
{
}

void sig_source_s_i::changedSamplingFreq(const std::string &id)
{
...
void sig_source_s_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
    try{
	    this->gr_sptr = gr_make_sig_source_s(this->sampling_freq, (gr_waveform_t)this->waveform, this->frequency, this->amplitude, this->offset);
    }
    catch(...){
    	return;
    }
	this->setPropertyChangeListener("sampling_freq", this, &sig_source_s_i::changedSamplingFreq);
	this->registerGetter("sampling_freq", &gr_sig_source_s::sampling_freq);
	this->registerGetterSetter("waveform", &gr_sig_source_s::waveform, &gr_sig_source_s::set_waveform);
	this->registerGetterSetter("frequency", &gr_sig_source_s::frequency, &gr_sig_source_s::set_frequency);
	this->registerGetterSetter("amplitude", &gr_sig_source_s::amplitude, &gr_sig_source_s::set_amplitude);
	this->registerGetterSetter("offset", &gr_sig_source_s::offset, &gr_sig_source_s::set_offset);
    // 
...
//      
BULKIO::StreamSRI sig_source_s_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
