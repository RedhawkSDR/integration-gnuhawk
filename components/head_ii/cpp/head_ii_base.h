...
#ifndef HEAD_II_IMPL_BASE_H
#define HEAD_II_IMPL_BASE_H
...
#include "head_ii_GnuHawkBlock.h"
...
class head_ii_base : public GnuHawkBlock
{
    public:
        head_ii_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<head_ii_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::ULongLong nitems;
        CORBA::ULong sizeof_stream_item;
        bool reset;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( head_ii_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( head_ii_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( head_ii_base, " serviceFunction: retval:" << retval);
