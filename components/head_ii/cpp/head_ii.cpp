...
head_ii_i::head_ii_i(const char *uuid, const char *label) :
head_ii_base(uuid, label)
{
}
...
void head_ii_i::reset_data(const std::string &id)
{
    if (this->reset == true) {
        gr_sptr->reset();
    }
...
void head_ii_i::createBlock()
{
    ...
    this->setPropertyChangeListener("reset", this, &head_ii_i::reset_data);
...
//      
BULKIO::StreamSRI head_ii_i::createOutputSRI( int32_t idx)
{
    //
