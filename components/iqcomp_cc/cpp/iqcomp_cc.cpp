...
iqcomp_cc_i::iqcomp_cc_i(const char *uuid, const char *label) :
iqcomp_cc_base(uuid, label)
{
}

iqcomp_cc_i::~iqcomp_cc_i()
{
}
...
void iqcomp_cc_i::createBlock()
{
    ...
    try {
        gr_sptr = gr_make_iqcomp_cc( this->mu );
    } catch (...) {
        return;
    }

    this->registerGetterSetter("mu", &gr_iqcomp_cc::mu, &gr_iqcomp_cc::set_mu);
...
//      
BULKIO::StreamSRI iqcomp_cc_i::createOutputSRI( int32_t idx)
{
    //
