...
flex_sync_bb_4o_i::flex_sync_bb_4o_i(const char *uuid, const char *label) :
flex_sync_bb_4o_base(uuid, label)
{
    setPropertyChangeListener ("stream_id_map" , this, &flex_sync_bb_4o_i::streamIdChanged );
}

flex_sync_bb_4o_i::~flex_sync_bb_4o_i()
{
}

void flex_sync_bb_4o_i::streamIdChanged(const std::string& id)
{ 
...
void flex_sync_bb_4o_i::createBlock()
{
    //
    //
	try {
		gr_sptr = pager_make_flex_sync ();
	}
	catch(...){
		return;
	}
...
//      
BULKIO::StreamSRI flex_sync_bb_4o_i::createOutputSRI( int32_t oidx, int32_t &in_idx, std::string &ext)
{
    //
