...
vector_source_i_i::vector_source_i_i(const char *uuid, const char *label) :
vector_source_i_base(uuid, label)
{
}

vector_source_i_i::~vector_source_i_i()
{
}

void vector_source_i_i::setData( const std::string &id ) {
    if ( validGRBlock() ) {
      std::vector<int> t_data( data.begin(), data.end() );
      gr_sptr->set_data( t_data );
    }
}


void vector_source_i_i::changedSamplingFreq(const std::string &id)
{
...
//
void vector_source_i_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  std::vector<int> t_data( data.begin(), data.end() );
  gr_sptr = gr_make_vector_source_i(t_data, repeat, vlen);

  this->setPropertyChangeListener("data", this, &vector_source_i_i::setData );
  this->setPropertyChangeListener("rewind", this, &vector_source_i_i::onRewind);
  this->setPropertyChangeListener("sampling_freq", this, &vector_source_i_i::changedSamplingFreq);
...
//      
BULKIO::StreamSRI vector_source_i_i::createOutputSRI( int32_t idx)
{
  //
...
void vector_source_i_i::onRewind (const std::string&)
{
  if (rewind && validGRBlock()) {
    gr_sptr->rewind();
    rewind = false;
  }
}
