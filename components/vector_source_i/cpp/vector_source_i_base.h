...
#ifndef VECTOR_SOURCE_I_IMPL_BASE_H
#define VECTOR_SOURCE_I_IMPL_BASE_H
...
#include "vector_source_i_GnuHawkBlock.h"


class vector_source_i_base : public GnuHawkBlock
{
    ...
    public:
        ...
        vector_source_i_base(const char *uuid, const char *label);
 ...
 template < typename OUT_PORT_TYPE > struct gr_ostream {
  ...
  typedef _OStream< BULKIO_dataLong_Out_i >::List   OStreamList;  
   ...
   ProcessThread<vector_source_i_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( vector_source_i_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( vector_source_i_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( vector_source_i_base, " serviceFunction: retval:" << retval);
