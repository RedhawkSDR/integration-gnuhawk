...
copy_bb_i::copy_bb_i(const char *uuid, const char *label) :
copy_bb_base(uuid, label)
{
}
...
void copy_bb_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
    gr_sptr = gr_make_copy (this->itemsize);
    this->registerGetterSetter("enable", &gr_copy::enabled, &gr_copy::set_enabled);
    gr_sptr->set_enabled(this->enable);
...
//      
BULKIO::StreamSRI copy_bb_i::createOutputSRI( int32_t idx)
{
    //
