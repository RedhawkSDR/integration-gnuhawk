...
#ifndef RMS_CF_IMPL_BASE_H
#define RMS_CF_IMPL_BASE_H
...
class rms_cf_base : public GnuHawkBlock
{
    public:
        rms_cf_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<rms_cf_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( rms_cf_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( rms_cf_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( rms_cf_base, " serviceFunction: retval:" << retval);
