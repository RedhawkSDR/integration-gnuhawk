...
rms_cf_i::rms_cf_i(const char *uuid, const char *label) :
rms_cf_base(uuid, label)
{
}
...
void rms_cf_i::createBlock()
{
    ...
    try {
        gr_sptr = gr_make_rms_cf( this->alpha );
    } catch (...) {
        return;
    }

    this->registerSetter("alpha", &gr_rms_cf::set_alpha);
    this->registerGetter("unmuted", &gr_rms_cf::unmuted);
...
//      
BULKIO::StreamSRI rms_cf_i::createOutputSRI( int32_t idx)
{
    //
