...
#ifndef PLL_FREQDET_CF_IMPL_BASE_H
#define PLL_FREQDET_CF_IMPL_BASE_H
...
#include "pll_freqdet_cf_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class pll_freqdet_cf_base;
...
class pll_freqdet_cf_base : public GnuHawkBlock
{
    public:
        pll_freqdet_cf_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<pll_freqdet_cf_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( pll_freqdet_cf_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( pll_freqdet_cf_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( pll_freqdet_cf_base, " serviceFunction: retval:" << retval);
