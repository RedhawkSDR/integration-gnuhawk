...
#ifndef SINGLE_POLE_IIR_FILTER_FF_IMPL_BASE_H
#define SINGLE_POLE_IIR_FILTER_FF_IMPL_BASE_H
...
#include "single_pole_iir_filter_ff_GnuHawkBlock.h"


class single_pole_iir_filter_ff_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  single_pole_iir_filter_ff_base;
  ...
  SRIListener( single_pole_iir_filter_ff_base &parent ) : _parent(parent) {};

    single_pole_iir_filter_ff_base &_parent;
  };
    

    public:

        single_pole_iir_filter_ff_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<single_pole_iir_filter_ff_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( single_pole_iir_filter_ff_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( single_pole_iir_filter_ff_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( single_pole_iir_filter_ff_base, " serviceFunction: retval:" << retval);
