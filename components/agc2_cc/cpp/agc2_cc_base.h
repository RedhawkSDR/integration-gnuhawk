...
#ifndef AGC2_CC_IMPL_BASE_H
#define AGC2_CC_IMPL_BASE_H
...
#include "agc2_cc_GnuHawkBlock.h"


class agc2_cc_base : public GnuHawkBlock
{
  ...
  SRIListener( agc2_cc_base &parent ) : _parent(parent) {};

    agc2_cc_base &_parent;
  };
    

    public:

        agc2_cc_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<agc2_cc_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( agc2_cc_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( agc2_cc_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( agc2_cc_base, " serviceFunction: retval:" << retval);
