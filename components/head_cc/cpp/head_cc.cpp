...
head_cc_i::head_cc_i(const char *uuid, const char *label) :
head_cc_base(uuid, label)
{
}
...
void head_cc_i::reset_data(const std::string &id)
{
    if (this->reset == true) {
        gr_sptr->reset();
    }
...
void head_cc_i::createBlock()
{
    ...
    this->setPropertyChangeListener("reset", this, &head_cc_i::reset_data);
...
//      
BULKIO::StreamSRI head_cc_i::createOutputSRI( int32_t idx)
{
    //
