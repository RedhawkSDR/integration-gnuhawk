...
fir_filter_ccc_i::fir_filter_ccc_i(const char *uuid, const char *label) :
fir_filter_ccc_base(uuid, label)
{
}

fir_filter_ccc_i::~fir_filter_ccc_i()
{
}

void fir_filter_ccc_i::setTaps(const std::string &id)
{
  if ( validGRBlock() ) gr_sptr->set_taps(taps);
}

std::vector<std::complex<float> > fir_filter_ccc_i::getTaps()
{
  if ( validGRBlock() ) return gr_sptr->taps();
...
void fir_filter_ccc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
  if ( taps.size() < 1 ) return;

  try {
    gr_sptr = gr::filter::fir_filter_ccc::make( decimation, taps );
  }
  catch(...) {
    return;
  }

  //this->registerGetValue("taps", this, &fir_filter_ccc_i::getTaps);
  //this->setPropertyChangeListener("taps", this, &fir_filter_ccc_i::setTaps);

  registerGetter("taps", &gr::filter::fir_filter_ccc::taps );
  registerSetter("taps",  &gr::filter::fir_filter_ccc::set_taps);
  //registerGetter("decimation", &gr::filter::fir_filter_ccc::decimation );
  //registerSetter("decimation", &gr::filter::fir_filter_ccc::set_decimation);
    // 
...
//      
BULKIO::StreamSRI fir_filter_ccc_i::createOutputSRI( int32_t idx)
{
    //
