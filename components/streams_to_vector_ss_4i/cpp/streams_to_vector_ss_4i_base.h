...
#ifndef STREAMS_TO_VECTOR_SS_4I_IMPL_BASE_H
#define STREAMS_TO_VECTOR_SS_4I_IMPL_BASE_H
...
#include "streams_to_vector_ss_4i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class streams_to_vector_ss_4i_base;
...
class streams_to_vector_ss_4i_base : public GnuHawkBlock
{
    public:
        streams_to_vector_ss_4i_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<streams_to_vector_ss_4i_base> *serviceThread; 
        ...
        // Ports
        bulkio::InShortPort *short_in_0;
        bulkio::InShortPort *short_in_1;
        bulkio::InShortPort *short_in_2;
        bulkio::InShortPort *short_in_3;
    ...
    protected:
        ...
        void  short_in_0_newStreamCallback( BULKIO::StreamSRI &sri );

        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  short_in_1_newStreamCallback( BULKIO::StreamSRI &sri );

        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  short_in_2_newStreamCallback( BULKIO::StreamSRI &sri );

        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  short_in_3_newStreamCallback( BULKIO::StreamSRI &sri );
    ...
    typedef std::deque< std::pair< bulkio::InShortPort *, BULKIO::StreamSRI > >  SRIQueue;
    typedef _IStream< bulkio::InShortPort >::List    IStreamList;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( streams_to_vector_ss_4i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( streams_to_vector_ss_4i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( streams_to_vector_ss_4i_base, " serviceFunction: retval:" << retval);
