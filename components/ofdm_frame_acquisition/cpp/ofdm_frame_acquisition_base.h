...
#ifndef OFDM_FRAME_ACQUISITION_IMPL_BASE_H
#define OFDM_FRAME_ACQUISITION_IMPL_BASE_H
...
#include "ofdm_frame_acquisition_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class ofdm_frame_acquisition_base;
...
class ofdm_frame_acquisition_base : public GnuHawkBlock
{
    public:
        ofdm_frame_acquisition_base(const char *uuid, const char *label);
        ...
        ~ofdm_frame_acquisition_base();

        void loadProperties();

    protected:
        ProcessThread<ofdm_frame_acquisition_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::ULong occupied_carriers;
        CORBA::ULong fft_length;
        CORBA::ULong cplen;
        CORBA::ULong max_fft_shift_len;
        float snr;
        std::vector<std::complex<float> > known_symbol;

        // Ports
        bulkio::InFloatPort *complex_in;
        bulkio::InOctetPort *signal_in;
        bulkio::OutFloatPort *complex_out;
        bulkio::OutOctetPort *signal_out;
    ...
    protected:
        ...
        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  signal_in_newStreamCallback( BULKIO::StreamSRI &sri );
...
std::cout << "   getting data: " << std::endl;
    ...
    template < typename OUT_PORT_TYPE > struct gr_ostream : gr_ostream_base {
        ...
        //
        void pushSRI() {
            std::cout << "PUSHSRI111111111111111" << std::endl;
        ...
        void pushSRI( BULKIO::StreamSRI &inSri ) {
            std::cout << "PUSHSRI@@2222222222" << std::endl;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( ofdm_frame_acquisition_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( ofdm_frame_acquisition_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( ofdm_frame_acquisition_base, " serviceFunction: retval:" << retval);
