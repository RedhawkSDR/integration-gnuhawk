...
moving_average_ff_i::moving_average_ff_i(const char *uuid, const char *label) :
moving_average_ff_base(uuid, label)
{
}

moving_average_ff_i::~moving_average_ff_i()
{
}

void moving_average_ff_i::set_length_and_scale(const std::string &id)
{
...
void moving_average_ff_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_moving_average_ff( this->length, this->scale, this->max_iter );
    } catch (...) {
        return;
    }

    this->registerGetter("length", &gr_moving_average_ff::length);
    this->registerGetter("scale", &gr_moving_average_ff::scale);

    this->setPropertyChangeListener("length", this, &moving_average_ff_i::set_length_and_scale);
    this->setPropertyChangeListener("scale", this, &moving_average_ff_i::set_length_and_scale);
    // 
...
//      
BULKIO::StreamSRI moving_average_ff_i::createOutputSRI( int32_t idx)
{
    //
