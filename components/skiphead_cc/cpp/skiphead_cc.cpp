...
skiphead_cc_i::skiphead_cc_i(const char *uuid, const char *label) :
skiphead_cc_base(uuid, label)
{
}

skiphead_cc_i::~skiphead_cc_i()
{
}
...
void skiphead_cc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
    gr_sptr = gr_make_skiphead (this->itemsize, this->nitems_to_skip);
...
//      
BULKIO::StreamSRI skiphead_cc_i::createOutputSRI( int32_t idx)
{
    //
