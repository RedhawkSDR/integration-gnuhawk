...
#ifndef SAMPLE_AND_HOLD_BB_IMPL_BASE_H
#define SAMPLE_AND_HOLD_BB_IMPL_BASE_H
...
#include "sample_and_hold_bb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class sample_and_hold_bb_base;
...
class sample_and_hold_bb_base : public GnuHawkBlock
{
    public:
        sample_and_hold_bb_base(const char *uuid, const char *label);
        ...
        ~sample_and_hold_bb_base();

        void loadProperties();

    protected:
        ProcessThread<sample_and_hold_bb_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( sample_and_hold_bb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( sample_and_hold_bb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( sample_and_hold_bb_base, " serviceFunction: retval:" << retval);
