...
#include "fake_channel_encoder_pp_bb.h"
...
fake_channel_encoder_pp_bb_i::fake_channel_encoder_pp_bb_i(const char *uuid, const char *label) :
fake_channel_encoder_pp_bb_base(uuid, label)
{
}

fake_channel_encoder_pp_bb_i::~fake_channel_encoder_pp_bb_i()
{
}
...
void fake_channel_encoder_pp_bb_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	try{
        this->gr_sptr = gr_make_fake_channel_encoder_pp(this->input_vlen, this->output_vlen);
	}
	catch (...) {
		return;
	}
...
//      
BULKIO::StreamSRI fake_channel_encoder_pp_bb_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
