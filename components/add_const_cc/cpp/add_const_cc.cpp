...
add_const_cc_i::add_const_cc_i(const char *uuid, const char *label) :
add_const_cc_base(uuid, label)
{
}

add_const_cc_i::~add_const_cc_i()
{
}
...
void add_const_cc_i::createBlock()
{
    //
  ...
  try {
    gr_sptr = gr_make_add_const_cc( k );
  }
  catch(...) {
    return;
  }

  registerGetterSetter("k",
		       &gr_add_const_cc::k,
		       &gr_add_const_cc::set_k);
...
//      
BULKIO::StreamSRI add_const_cc_i::createOutputSRI( int32_t idx)
{
    //
