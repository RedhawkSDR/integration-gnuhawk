...
#ifndef INTERLEAVE_FF_4I_IMPL_BASE_H
#define INTERLEAVE_FF_4I_IMPL_BASE_H
...
#include "interleave_ff_4i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class interleave_ff_4i_base;
...
class interleave_ff_4i_base : public GnuHawkBlock
{
    public:
        interleave_ff_4i_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<interleave_ff_4i_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( interleave_ff_4i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( interleave_ff_4i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( interleave_ff_4i_base, " serviceFunction: retval:" << retval);
