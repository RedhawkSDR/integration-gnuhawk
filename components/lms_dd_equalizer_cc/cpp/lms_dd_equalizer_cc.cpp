...
lms_dd_equalizer_cc_i::lms_dd_equalizer_cc_i(const char *uuid, const char *label) :
lms_dd_equalizer_cc_base(uuid, label)
{
}

lms_dd_equalizer_cc_i::~lms_dd_equalizer_cc_i()
{
}

void lms_dd_equalizer_cc_i::set_gain(const std::string &id)
{
    gr_sptr->set_gain(this->mu);
}

float lms_dd_equalizer_cc_i::get_gain()
{
    return gr_sptr->get_gain();
}
...
void lms_dd_equalizer_cc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
    digital_constellation_sptr cons;
    switch (constellation) {
    case 1: cons = digital_make_constellation_bpsk(); break;
    case 2: cons = digital_make_constellation_qpsk(); break;
    case 3: cons = digital_make_constellation_dqpsk(); break;
    case 4: cons = digital_make_constellation_8psk(); break;
    default:
      std::cerr << "Invalid constellation" << std::endl;
      return;
    }

    gr_sptr = digital_make_lms_dd_equalizer_cc( this->num_taps, this->mu, this->sps, cons );

    this->setPropertyChangeListener("mu", this, &lms_dd_equalizer_cc_i::set_gain);
    this->registerGetValue("mu", this, &lms_dd_equalizer_cc_i::get_gain);
    // 
...
//      
BULKIO::StreamSRI lms_dd_equalizer_cc_i::createOutputSRI( int32_t idx)
{
    //
