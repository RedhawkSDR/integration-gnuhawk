...
cma_equalizer_cc_i::cma_equalizer_cc_i(const char *uuid, const char *label) :
cma_equalizer_cc_base(uuid, label)
{
}

cma_equalizer_cc_i::~cma_equalizer_cc_i()
{
}

void cma_equalizer_cc_i::set_gain(const std::string& id)
{
    gr_sptr->set_gain(this->mu);
}

void cma_equalizer_cc_i::set_modulus(const std::string& id)
{
    gr_sptr->set_modulus(this->modulus);
}

float cma_equalizer_cc_i::get_gain()
{
    return this->gr_sptr->get_gain();
}

float cma_equalizer_cc_i::get_modulus()
{
    return this->gr_sptr->get_modulus();
}
...
void cma_equalizer_cc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = digital_make_cma_equalizer_cc( this->num_taps, this->modulus, this->mu, this->sps );

    this->setPropertyChangeListener("mu", this, &cma_equalizer_cc_i::set_gain);
    this->setPropertyChangeListener("modulus", this, &cma_equalizer_cc_i::set_modulus);
    this->registerGetValue("mu", this, &cma_equalizer_cc_i::get_gain);
    this->registerGetValue("modulus", this, &cma_equalizer_cc_i::get_modulus);
...
//      
BULKIO::StreamSRI cma_equalizer_cc_i::createOutputSRI( int32_t idx)
{
    //
