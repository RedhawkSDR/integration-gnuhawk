...
#include "interp_fir_filter_fsf.h"
...
interp_fir_filter_fsf_i::interp_fir_filter_fsf_i(const char *uuid, const char *label) :
interp_fir_filter_fsf_base(uuid, label)
{
}

interp_fir_filter_fsf_i::~interp_fir_filter_fsf_i()
{
}
...
void interp_fir_filter_fsf_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr::filter::interp_fir_filter_fsf::make( this->interpolation, this->taps );

    this->registerGetter("taps", &gr::filter::interp_fir_filter_fsf::taps);
    this->registerSetter("taps", &gr::filter::interp_fir_filter_fsf::set_taps);
...
//      
BULKIO::StreamSRI interp_fir_filter_fsf_i::createOutputSRI( int32_t idx)
{
    //
