...
#ifndef INTERP_FIR_FILTER_FSF_IMPL_BASE_H
#define INTERP_FIR_FILTER_FSF_IMPL_BASE_H
...
#include "interp_fir_filter_fsf_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class interp_fir_filter_fsf_base;
...
class interp_fir_filter_fsf_base : public GnuHawkBlock
{
    public:
        interp_fir_filter_fsf_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<interp_fir_filter_fsf_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( interp_fir_filter_fsf_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( interp_fir_filter_fsf_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( interp_fir_filter_fsf_base, " serviceFunction: retval:" << retval);
