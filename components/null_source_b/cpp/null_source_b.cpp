...
null_source_b_i::null_source_b_i(const char *uuid, const char *label) :
null_source_b_base(uuid, label)
{
}

null_source_b_i::~null_source_b_i()
{
}

void null_source_b_i::changedSamplingFreq(const std::string &id)
{
...
void null_source_b_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_null_source( this->sizeof_stream_item );
    } catch (...) {
        return;
    }

    this->setPropertyChangeListener("sampling_freq", this, &null_source_b_i::changedSamplingFreq);
...
//      
BULKIO::StreamSRI null_source_b_i::createOutputSRI( int32_t idx)
{
    //
