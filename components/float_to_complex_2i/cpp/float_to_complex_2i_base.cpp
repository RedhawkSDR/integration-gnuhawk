...
// Destructor
float_to_complex_2i_base::~float_to_complex_2i_base()
{
    // Free input streams
...
void float_to_complex_2i_base::setupIOMappings( )
{
   ...
   //
    ...
    p_in = inPorts.find("real_float_in");
    if ( p_in != inPorts.end() ) {
    ...
    // grab ports based on their order in the scd.xml file
    p_in = inPorts.find("imag_float_in");
    if ( p_in != inPorts.end() ) {
        bulkio::InFloatPort *port = dynamic_cast< bulkio::InFloatPort * >(p_in->second);
        int mode = inMode;
        sid = "";
...
int float_to_complex_2i_base::_transformerServiceFunction( std::vector< gr_istream_base * > &istreams ,
...
int float_to_complex_2i_base::_forecastAndProcess( bool &eos, std::vector< gr_istream_base * > &istreams ,
                                 std::vector< gr_ostream_base * > &ostreams  )
{
    typedef std::vector< gr_istream_base * >   _IStreamList;
    typedef std::vector< gr_ostream_base * >  _OStreamList;
...
// use incoming packet's time stamp to forward
      ...
      LOG_DEBUG( float_to_complex_2i_base, "                  mode:" <<  pktTs.tcmode );
      LOG_DEBUG( float_to_complex_2i_base, "                status:" <<  pktTs.tcstatus );
      LOG_DEBUG( float_to_complex_2i_base, "                offset:" <<  pktTs.toff );
      LOG_DEBUG( float_to_complex_2i_base, "                 whole:" <<  std::setprecision(10) << pktTs.twsec );
      LOG_DEBUG( float_to_complex_2i_base, "PKT - TS          frac:" <<  std::setprecision(12) << pktTs.tfsec );
#endif
