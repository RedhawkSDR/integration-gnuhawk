...
#ifndef FLOAT_TO_COMPLEX_2I_IMPL_BASE_H
#define FLOAT_TO_COMPLEX_2I_IMPL_BASE_H
...
#include "float_to_complex_2i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class float_to_complex_2i_base;
...
class float_to_complex_2i_base : public GnuHawkBlock
{
    public:
        float_to_complex_2i_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<float_to_complex_2i_base> *serviceThread; 
        ...
        // Ports
        bulkio::InFloatPort *real_float_in;
        bulkio::InFloatPort *imag_float_in;
    ...
    protected:
        ...
        void  real_float_in_newStreamCallback( BULKIO::StreamSRI &sri );

        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  imag_float_in_newStreamCallback( BULKIO::StreamSRI &sri );
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( float_to_complex_2i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( float_to_complex_2i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( float_to_complex_2i_base, " serviceFunction: retval:" << retval);
