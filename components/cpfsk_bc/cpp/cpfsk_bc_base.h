...
#ifndef CPFSK_BC_IMPL_BASE_H
#define CPFSK_BC_IMPL_BASE_H
...
#include "cpfsk_bc_GnuHawkBlock.h"
...
class cpfsk_bc_base : public GnuHawkBlock
{
    public:
        cpfsk_bc_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<cpfsk_bc_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        float k;
        float amplitude;
        CORBA::Long samples_per_sym;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( cpfsk_bc_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( cpfsk_bc_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( cpfsk_bc_base, " serviceFunction: retval:" << retval);
