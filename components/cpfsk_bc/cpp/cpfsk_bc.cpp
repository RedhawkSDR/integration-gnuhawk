...
cpfsk_bc_i::cpfsk_bc_i(const char *uuid, const char *label) :
cpfsk_bc_base(uuid, label)
{
}

cpfsk_bc_i::~cpfsk_bc_i()
{
}
...
void cpfsk_bc_i::createBlock()
{
    ...
    try {
        gr_sptr = gr_make_cpfsk_bc( this->k, this->amplitude, this->samples_per_sym );
    } catch (...) {
        return;
    }

    this->registerSetter("amplitude", &gr_cpfsk_bc::set_amplitude);
...
//      
BULKIO::StreamSRI cpfsk_bc_i::createOutputSRI( int32_t idx)
{
    //
