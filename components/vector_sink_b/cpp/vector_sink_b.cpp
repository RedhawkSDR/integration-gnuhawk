...
vector_sink_b_i::vector_sink_b_i(const char *uuid, const char *label) :
vector_sink_b_base(uuid, label)
{
}

vector_sink_b_i::~vector_sink_b_i()
{
}
...
//
void vector_sink_b_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_vector_sink_b(vlen);
  ...
  setupIOMappings();
  this->registerGetter("data", &gr_vector_sink_b::data);
  this->setPropertyChangeListener("reset", this, &vector_sink_b_i::on_reset);
} 

void vector_sink_b_i::on_reset (const std::string&)
{
