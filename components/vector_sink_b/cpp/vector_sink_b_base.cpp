...
#include "vector_sink_b_base.h"

/*******************************************************************************************

    AUTO-GENERATED CODE. DO NOT MODIFY
    
 	Source: vector_sink_b.spd.xml
 	Generated on: Tue Mar 05 08:08:19 EST 2013
 	Redhawk IDE
 	Version:M.1.8.3
...
//  Allow for logging 
// 
PREPARE_LOGGING(vector_sink_b_base);
...
vector_sink_b_base::vector_sink_b_base(const char *uuid, const char *label) :
...
void vector_sink_b_base::construct()
{
...
*******************************************************************************************/
void vector_sink_b_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void vector_sink_b_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
...
void vector_sink_b_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( vector_sink_b_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( vector_sink_b_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr vector_sink_b_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void vector_sink_b_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
    ...
    LOG_TRACE( vector_sink_b_base, "COMPLETED RELEASE OBJECT" );
}

void vector_sink_b_base::loadProperties()
{
    addProperty(vlen,
                1, 
               "vlen",
...
void vector_sink_b_base::setupIOMappings( ) 
{
  ...
  LOG_DEBUG( vector_sink_b_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
  ...
  // 
   ...
   if ( _istreams.size() > 0 ) {
    ...
    LOG_DEBUG(  vector_sink_b_base, "RESET INPUT SIGNATURE SIZE:" << _istreams.size() );
    IStreamList::iterator istream;
    for ( int idx=0 ; istream != _istreams.end(); idx++, istream++ ) {
        // re-add existing stream definitons
      LOG_DEBUG(  vector_sink_b_base, "ADD READ INDEX TO GNU RADIO BLOCK");
   ...
   //
   LOG_TRACE( vector_sink_b_base, "setupIOMappings INPUT PORTS: " << inPorts.size() );
   ...
   for( int i=0; pname != inputPortOrder.end(); pname++ ) {
	   ...
	   LOG_DEBUG(  vector_sink_b_base, "ADDING INPUT MAP IDX:" << i << " SID:" << sid );	
...
void vector_sink_b_base::notifySRI( BULKIO_dataOctet_In_i *port, BULKIO::StreamSRI &sri ) {

  LOG_TRACE( vector_sink_b_base, "START NotifySRI  port:stream " << port->getName() << "/" << sri.streamID);
  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( port, sri ) );
  LOG_TRACE( vector_sink_b_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << port->getName() << "/" << sri.streamID); 
  
}
 
void vector_sink_b_base::processStreamIdChanges() {

  boost::mutex::scoped_lock lock(_sriMutex);

  LOG_TRACE( vector_sink_b_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
  ...
  if ( validGRBlock() ) {
    ...
    for ( ; item != _sriQueue.end(); item++ ) {
	   ...
	   if ( istream->port == item->first ) {
	       LOG_DEBUG(  vector_sink_b_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
	       istream->sri(true);
	       istream->spe(mode);
   ...
   _sriQueue.clear();
 	...
 	LOG_WARN(  vector_sink_b_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
  }
...
vector_sink_b_base::TimeDuration vector_sink_b_base::getTargetDuration() {
  ...
  LOG_TRACE( vector_sink_b_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
	     " target_duration " << t_drate );
  return t_drate;
}

vector_sink_b_base::TimeDuration vector_sink_b_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time ) {
  ...
  if ( start_time.is_not_a_date_time() == false ) {
    ...
    delta /= 4;
    LOG_TRACE( vector_sink_b_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
	      " delta " << delta );
  }
  return delta;
}
...
template < typename IN_PORT_TYPE > int vector_sink_b_base::_analyzerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ) {
  ...
  if ( validGRBlock() == false ) {
    ...
    LOG_DEBUG(  vector_sink_b_base, " FINISHED BUILDING  GNU RADIO BLOCK");
  }
  ...
  if ( !validGRBlock() || istreams.size() == 0 ) {
    LOG_WARN(vector_sink_b_base, "NO STREAMS ATTACHED TO BLOCK..." );
    return NOOP;
  }
  ...
  for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
    ...
    if ( istream->overrun() ) {
        LOG_WARN( vector_sink_b_base, " NOT KEEPING UP WITH STREAM ID:" << istream->streamID );
    }
    
    // RESOLVE issue when SRI changes that could affect the GNU Radio BLOCK
    if ( istream->sriChanged() ) {
      LOG_DEBUG( vector_sink_b_base, "SRI CHANGED, STREAMD IDX/ID: " 
               << idx << "/" << istream->pkt->streamID );
    }
  }

  LOG_TRACE( vector_sink_b_base, "READ NITEMS: "  << nitems );
  ...
  while ( nout > -1 && !exitServiceFunction && serviceThread->threadRunning() ) {
	...
	if ( _input_ready[idx] ) {
	  ...
	  if ( nitems > istream->nitems() ) {
	       LOG_WARN( vector_sink_b_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE " << nitems << "/" << istream->nitems() );
               nitems = istream->nitems();
	  }
	  istream->consume( nitems );
          LOG_TRACE( vector_sink_b_base, " CONSUME READ DATA  ITEMS/REMAIN " << nitems << "/" << istream->nitems());
	}
    ...
    if ( nout == -1 ) {
      ...
      if ( eos ) {
        LOG_TRACE( vector_sink_b_base, " DATA NOT READY, EOS:" << eos );
  ...
  if ( eos ) {
    ...
    for ( ; istream != istreams.end() ; istream++) {
      int idx=std::distance( istreams.begin(), istream );
      LOG_TRACE( vector_sink_b_base, " CLOSING INPUT STREAM IDX:" << idx );
      istream->close();
    }
  }
  ...
  LOG_TRACE( vector_sink_b_base, " END OF ANALYZER SERVICE FUNCTION....." << noutput_items );
...
template <  typename IN_PORT_TYPE > int vector_sink_b_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams )
{
  typedef typename std::vector< gr_istream< IN_PORT_TYPE > >   _IStreamList;
  ...
  if ( !eos ) {
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
      LOG_TRACE(  vector_sink_b_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << istream->nitems() << "/" << istream->_data.size() );
    ...
    if ( noutput_items <= 0  ) {
       LOG_TRACE( vector_sink_b_base, "DATA CHECK - MAX ITEMS  NOUTPUT/MAX_ITEMS:" <<   noutput_items << "/" << max_items_avail);
       return -1;
    }
	...
	if ( gr_sptr->fixed_rate() ) {
      ...
      LOG_TRACE( vector_sink_b_base, " FIXED FORECAST NOUTPUT/output_multiple == " << noutput_items  << "/" << gr_sptr->output_multiple());
    }
    ...
    while ( !dataReady && noutput_items >= outMultiple  ) {
      ...
      LOG_TRACE( vector_sink_b_base, "--> FORECAST IN/OUT " << _ninput_items_required[0]  << "/" << noutput_items  );
	...
	LOG_TRACE( vector_sink_b_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << "/" << istream->nitems() << "/" << _ninput_items_required[idx] << "/" << _input_ready[idx]);
      }
      ...
      LOG_TRACE( vector_sink_b_base, " TRIM FORECAST NOUTPUT/READY " << noutput_items << "/" << dataReady );
    }

    // check if data is ready...
    if ( !dataReady ) {
      LOG_TRACE( vector_sink_b_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << "/" << _ninput_items_required[0] );
      return -1;	 
    }
	...
	LOG_ERROR( vector_sink_b_base, "MISSING INPUT STREAM FOR GR BLOCK, STREAM ID:" <<   istream->streamID );
        return -2;
      } 
    
      nitems = istream->nitems() - ritems;
      LOG_TRACE( vector_sink_b_base,  " ISTREAM: IDX:" << idx  << " ITEMS AVAIL/READ/REQ " << nitems << "/" 
    ...
    if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
      LOG_TRACE( vector_sink_b_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems << " ISTREAMS:" << _input_items.size() << " OSTREAMS:" << _output_items.size());
      ...
      LOG_TRACE( vector_sink_b_base, "RETURN  WORK ..... N_OUT:" << nout);
    }

    // check for stop condition from work method
    if ( nout < gr_block::WORK_DONE ) {
      LOG_WARN( vector_sink_b_base, "WORK RETURNED STOP CONDITION..." << nout );
      nout=0;
      eos = true;
    }
  }
