...
burst_tagger_ff_i::burst_tagger_ff_i(const char *uuid, const char *label) :
burst_tagger_ff_base(uuid, label)
{
}

burst_tagger_ff_i::~burst_tagger_ff_i()
{
}

void burst_tagger_ff_i::true_tag_changed(const std::string &id)
{
    gr_sptr->set_true_tag(true_tag.true_tag_key, true_tag.true_tag_value);
}

void burst_tagger_ff_i::false_tag_changed(const std::string &id)
{
    gr_sptr->set_false_tag(false_tag.false_tag_key, false_tag.false_tag_value);
}
...
void burst_tagger_ff_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_burst_tagger( this->itemsize );
    } catch (...) {
        return;
    }

    this->setPropertyChangeListener("true_tag", this, &burst_tagger_ff_i::true_tag_changed);
    this->setPropertyChangeListener("false_tag", this, &burst_tagger_ff_i::false_tag_changed);
...
//      
BULKIO::StreamSRI burst_tagger_ff_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
