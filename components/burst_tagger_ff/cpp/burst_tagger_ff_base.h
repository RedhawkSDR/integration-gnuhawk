...
#ifndef BURST_TAGGER_FF_IMPL_BASE_H
#define BURST_TAGGER_FF_IMPL_BASE_H
...
#include "burst_tagger_ff_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class burst_tagger_ff_base;
...
class burst_tagger_ff_base : public GnuHawkBlock
{
    public:
        burst_tagger_ff_base(const char *uuid, const char *label);
        ...
        ~burst_tagger_ff_base();

        void loadProperties();

    protected:
        ProcessThread<burst_tagger_ff_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( burst_tagger_ff_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( burst_tagger_ff_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( burst_tagger_ff_base, " serviceFunction: retval:" << retval);
