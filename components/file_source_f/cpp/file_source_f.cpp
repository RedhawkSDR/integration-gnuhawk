...
file_source_f_i::file_source_f_i(const char *uuid, const char *label) :
file_source_f_base(uuid, label)
{
}

file_source_f_i::~file_source_f_i()
{
}

void file_source_f_i::changedSeek(const std::string &id){
	this->seek = gr_sptr->seek(this->seek_point, this->whence);
}

bool file_source_f_i::getSeek(){
	return this->seek;
}
...
void file_source_f_i::createBlock()
{
    //
	...
	try{
	        gr_sptr = gr_make_file_source(this->itemsize, this->filename.c_str(), this->repeat);
	}
	catch(...){
		return;
	}
	this->setPropertyChangeListener("seek_point", this, &file_source_f_i::changedSeek);
	this->setPropertyChangeListener("whence", this, &file_source_f_i::changedSeek);
    // 
...
//      
BULKIO::StreamSRI file_source_f_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
