...
#ifndef FILE_SOURCE_F_IMPL_BASE_H
#define FILE_SOURCE_F_IMPL_BASE_H
...
#include "file_source_f_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class file_source_f_base;
...
class file_source_f_base : public GnuHawkBlock
{
    public:
        file_source_f_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<file_source_f_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( file_source_f_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( file_source_f_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( file_source_f_base, " serviceFunction: retval:" << retval);
