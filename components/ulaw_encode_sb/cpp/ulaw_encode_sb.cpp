...
ulaw_encode_sb_i::ulaw_encode_sb_i(const char *uuid, const char *label) :
ulaw_encode_sb_base(uuid, label)
{
}

ulaw_encode_sb_i::~ulaw_encode_sb_i()
{
}
...
void ulaw_encode_sb_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = vocoder_make_ulaw_encode_sb();
    } catch (...){
        return;
    }
...
//      
BULKIO::StreamSRI ulaw_encode_sb_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
