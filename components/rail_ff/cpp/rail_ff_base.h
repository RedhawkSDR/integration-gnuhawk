...
#ifndef RAIL_FF_IMPL_BASE_H
#define RAIL_FF_IMPL_BASE_H
...
#include "rail_ff_GnuHawkBlock.h"
...
class rail_ff_base : public GnuHawkBlock
{
    public:
        rail_ff_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<rail_ff_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( rail_ff_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( rail_ff_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( rail_ff_base, " serviceFunction: retval:" << retval);
