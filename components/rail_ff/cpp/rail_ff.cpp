...
rail_ff_i::rail_ff_i(const char *uuid, const char *label) :
rail_ff_base(uuid, label)
{
}
...
void rail_ff_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_rail_ff( this->lo, this->hi );

    this->registerGetterSetter("lo",
                               &gr_rail_ff::lo,
                               &gr_rail_ff::set_lo);
    this->registerGetterSetter("hi",
                               &gr_rail_ff::hi,
                               &gr_rail_ff::set_hi);
...
//      
BULKIO::StreamSRI rail_ff_i::createOutputSRI( int32_t idx)
{
    //
