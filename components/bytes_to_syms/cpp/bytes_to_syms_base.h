...
#ifndef BYTES_TO_SYMS_IMPL_BASE_H
#define BYTES_TO_SYMS_IMPL_BASE_H
...
#include "bytes_to_syms_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class bytes_to_syms_base;
...
class bytes_to_syms_base : public GnuHawkBlock
{
    public:
        bytes_to_syms_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<bytes_to_syms_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( bytes_to_syms_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( bytes_to_syms_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( bytes_to_syms_base, " serviceFunction: retval:" << retval);
