...
bytes_to_syms_i::bytes_to_syms_i(const char *uuid, const char *label) :
bytes_to_syms_base(uuid, label)
{
}

bytes_to_syms_i::~bytes_to_syms_i()
{
}
...
void bytes_to_syms_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = digital_make_bytes_to_syms( );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI bytes_to_syms_i::createOutputSRI( int32_t idx)
{
    //
