...
#ifndef INTERP_FIR_FILTER_SCC_IMPL_BASE_H
#define INTERP_FIR_FILTER_SCC_IMPL_BASE_H
...
#include "interp_fir_filter_scc_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class interp_fir_filter_scc_base;
...
class interp_fir_filter_scc_base : public GnuHawkBlock
{
    public:
        interp_fir_filter_scc_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<interp_fir_filter_scc_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( interp_fir_filter_scc_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( interp_fir_filter_scc_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( interp_fir_filter_scc_base, " serviceFunction: retval:" << retval);
