...
#include "interp_fir_filter_scc.h"
...
interp_fir_filter_scc_i::interp_fir_filter_scc_i(const char *uuid, const char *label) :
interp_fir_filter_scc_base(uuid, label)
{
}

interp_fir_filter_scc_i::~interp_fir_filter_scc_i()
{
}
...
void interp_fir_filter_scc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr::filter::interp_fir_filter_scc::make( this->interpolation, this->taps );

    this->registerGetter("taps", &gr::filter::interp_fir_filter_scc::taps);
    this->registerSetter("taps", &gr::filter::interp_fir_filter_scc::set_taps);
...
//      
BULKIO::StreamSRI interp_fir_filter_scc_i::createOutputSRI( int32_t idx)
{
    //
