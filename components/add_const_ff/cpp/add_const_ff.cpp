...
add_const_ff_i::add_const_ff_i(const char *uuid, const char *label) :
add_const_ff_base(uuid, label)
{
}

add_const_ff_i::~add_const_ff_i()
{
}
...
//
void add_const_ff_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_add_const_ff(k);
  ...
  setupIOMappings();
  registerGetterSetter("k", &gr_add_const_ff::k, &gr_add_const_ff::set_k);
} 
...
//      
BULKIO::StreamSRI add_const_ff_i::createOutputSRI( int32_t idx)
{
  //
