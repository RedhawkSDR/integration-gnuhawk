...
vector_sink_s_i::vector_sink_s_i(const char *uuid, const char *label) :
vector_sink_s_base(uuid, label)
{
}

vector_sink_s_i::~vector_sink_s_i()
{
}
...
//
void vector_sink_s_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_vector_sink_s(vlen);
  ...
  setupIOMappings();
  this->registerGetter("data", &gr_vector_sink_s::data);
  this->setPropertyChangeListener("reset", this, &vector_sink_s_i::on_reset);
} 

void vector_sink_s_i::on_reset(const std::string&)
{
