...
noise_source_s_i::noise_source_s_i(const char *uuid, const char *label) :
noise_source_s_base(uuid, label)
{
}

noise_source_s_i::~noise_source_s_i()
{
}
...
//
void noise_source_s_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_noise_source_s((gr_noise_type_t)type, amplitude, seed);
  this->registerGetterSetter("type", &gr_noise_source_s::type, &gr_noise_source_s::set_type);
  this->registerGetterSetter("amplitude", &gr_noise_source_s::amplitude, &gr_noise_source_s::set_amplitude);
  this->setPropertyChangeListener("sampling_freq", this, &noise_source_s_i::changedSamplingFreq);
...
//      
BULKIO::StreamSRI noise_source_s_i::createOutputSRI( int32_t idx)
{
  //
...
void noise_source_s_i::changedSamplingFreq(const std::string &id)
{
