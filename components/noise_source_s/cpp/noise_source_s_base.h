...
#ifndef NOISE_SOURCE_S_IMPL_BASE_H
#define NOISE_SOURCE_S_IMPL_BASE_H
...
#include "noise_source_s_GnuHawkBlock.h"


class noise_source_s_base : public GnuHawkBlock
{
    ...
    public:
        ...
        noise_source_s_base(const char *uuid, const char *label);
 ...
 template < typename OUT_PORT_TYPE > struct gr_ostream {
  ...
  typedef _OStream< BULKIO_dataShort_Out_i >::List   OStreamList;  
   ...
   ProcessThread<noise_source_s_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( noise_source_s_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( noise_source_s_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( noise_source_s_base, " serviceFunction: retval:" << retval);
