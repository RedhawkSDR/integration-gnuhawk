...
/*******************************************************************************************
    ...
    AUTO-GENERATED CODE. DO NOT MODIFY
 	...
 	Source: multiply_ss_2i.spd.xml
 	Generated on: Fri Mar 01 09:50:38 EST 2013
 	Redhawk IDE
 	Version:M.1.8.3
...
void multiply_ss_2i_base::construct()
{
    ...
    data_in_0 = new BULKIO_dataShort_In_i("data_in_0",&_sriListener );
    oid = ossie::corba::RootPOA()->activate_object(data_in_0);
    data_in_1 = new BULKIO_dataShort_In_i("data_in_1",&_sriListener );
    oid = ossie::corba::RootPOA()->activate_object(data_in_1);
    data_out = new BULKIO_dataShort_Out_i("data_out", this);
...
void multiply_ss_2i_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
    ...
    LOG_TRACE( multiply_ss_2i_base, "COMPLETED RELEASE OBJECT" );
}
...
void multiply_ss_2i_base::setupIOMappings( ) 
{
  ...
  // 
   ...
   for( int i=0; pname != inputPortOrder.end(); pname++ ) {
       ...
       if ( p_in != inPorts.end() ) {
           BULKIO_dataShort_In_i *port = dynamic_cast< BULKIO_dataShort_In_i * >(p_in->second);
   	   int mode = inMode;
	   sid = "";
	   ...
	   _istreams.push_back( gr_istream< BULKIO_dataShort_In_i > ( port, gr_sptr, i, mode, sid ));
   ...
   for( int i=0; pname != outputPortOrder.end(); pname++ ) {
       ...
       if ( p_out != outPorts.end() ) {
           BULKIO_dataShort_Out_i *port = dynamic_cast< BULKIO_dataShort_Out_i * >(p_out->second);
	   BULKIO::StreamSRI sri = createOutputSRI( i );
	   int mode = sri.mode;
	   sid = sri.streamID;
	   _ostreams.push_back( gr_ostream< BULKIO_dataShort_Out_i > ( port, gr_sptr, i, mode, sid ));
...
void multiply_ss_2i_base::notifySRI( BULKIO_dataShort_In_i *port, BULKIO::StreamSRI &sri ) {

  LOG_TRACE( multiply_ss_2i_base, "START NotifySRI  port:stream " << port->getName() << "/" << sri.streamID);
  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( port, sri ) );
  LOG_TRACE( multiply_ss_2i_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << port->getName() << "/" << sri.streamID); 
...
BULKIO::StreamSRI multiply_ss_2i_base::createOutputSRI( int32_t idx)
{
  ...
  while ( nout > -1 && !exitServiceFunction && serviceThread->threadRunning() ) {
	...
	if ( _input_ready[idx] ) {
	  ...
	  if ( nitems > istream->nitems() ) {
	       LOG_WARN( multiply_ss_2i_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE " << nitems << "/" << istream->nitems() );
											...
											typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
  ...
  if ( !eos ) {
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
      LOG_TRACE(  multiply_ss_2i_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << istream->nitems() << "/" << istream->_data.size() );
    ...
    if ( !gr_sptr->fixed_rate() )  {
      ...
      LOG_TRACE( multiply_ss_2i_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << noutput_items  << "/" << gr_sptr->output_multiple());
    }
	...
	LOG_TRACE( multiply_ss_2i_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << "/" << istream->nitems() << "/" << _ninput_items_required[idx] << "/" << _input_ready[idx]);
      }
    ...
    if ( !dataReady ) {
      LOG_TRACE( multiply_ss_2i_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << "/" << _ninput_items_required[0] );
      return -1;	 
    }
    ...
    if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
      LOG_TRACE( multiply_ss_2i_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems << " ISTREAMS:" << _input_items.size() << " OSTREAMS:" << _output_items.size());
