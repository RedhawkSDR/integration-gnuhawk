...
#ifndef PFB_CLOCK_SYNC_CCF_4O_IMPL_BASE_H
#define PFB_CLOCK_SYNC_CCF_4O_IMPL_BASE_H
...
#include "pfb_clock_sync_ccf_4o_GnuHawkBlock.h"


class pfb_clock_sync_ccf_4o_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  pfb_clock_sync_ccf_4o_base;
  ...
  SRIListener( pfb_clock_sync_ccf_4o_base &parent ) : _parent(parent) {};

    pfb_clock_sync_ccf_4o_base &_parent;
  };
    

    public:

        pfb_clock_sync_ccf_4o_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<pfb_clock_sync_ccf_4o_base> *serviceThread; 
  ...
  BULKIO_dataFloat_Out_i *data_error_out;
  BULKIO_dataFloat_Out_i *data_rate_out;
  BULKIO_dataFloat_Out_i *data_k_out;
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( pfb_clock_sync_ccf_4o_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( pfb_clock_sync_ccf_4o_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( pfb_clock_sync_ccf_4o_base, " serviceFunction: retval:" << retval);
