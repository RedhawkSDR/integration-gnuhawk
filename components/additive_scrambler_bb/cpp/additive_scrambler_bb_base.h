...
#ifndef ADDITIVE_SCRAMBLER_BB_IMPL_BASE_H
#define ADDITIVE_SCRAMBLER_BB_IMPL_BASE_H
...
#include "additive_scrambler_bb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class additive_scrambler_bb_base;
...
class additive_scrambler_bb_base : public GnuHawkBlock
{
    public:
        additive_scrambler_bb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<additive_scrambler_bb_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( additive_scrambler_bb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( additive_scrambler_bb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( additive_scrambler_bb_base, " serviceFunction: retval:" << retval);
