...
additive_scrambler_bb_i::additive_scrambler_bb_i(const char *uuid, const char *label) :
additive_scrambler_bb_base(uuid, label)
{
}

additive_scrambler_bb_i::~additive_scrambler_bb_i()
{
}
...
void additive_scrambler_bb_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = digital_make_additive_scrambler_bb( this->mask, this->seed, this->len, this->count );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI additive_scrambler_bb_i::createOutputSRI( int32_t idx)
{
    //
