...
#ifndef INTERLEAVER_IMPL_BASE_H
#define INTERLEAVER_IMPL_BASE_H
...
#include "interleaver_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class interleaver_base;
...
class interleaver_base : public GnuHawkBlock
{
    public:
        interleaver_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<interleaver_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( interleaver_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( interleaver_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( interleaver_base, " serviceFunction: retval:" << retval);
