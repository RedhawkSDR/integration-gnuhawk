...
sig_source_c_i::sig_source_c_i(const char *uuid, const char *label) :
sig_source_c_base(uuid, label)
{
}

sig_source_c_i::~sig_source_c_i()
{
}

void sig_source_c_i::changedSamplingFreq(const std::string &id)
{
...
void sig_source_c_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
    this->gr_sptr = gr_make_sig_source_c( this->sampling_freq, (gr_waveform_t)this->waveform, this->frequency, this->amplitude, this->offset );
    this->setPropertyChangeListener("sampling_freq", this, &sig_source_c_i::changedSamplingFreq);
    this->registerGetter("sampling_freq", &gr_sig_source_c::sampling_freq);
    this->registerGetterSetter("waveform", &gr_sig_source_c::waveform, &gr_sig_source_c::set_waveform);
    this->registerGetterSetter("frequency", &gr_sig_source_c::frequency, &gr_sig_source_c::set_frequency);
    this->registerGetterSetter("amplitude", &gr_sig_source_c::amplitude, &gr_sig_source_c::set_amplitude);
    this->registerGetterSetter("offset", &gr_sig_source_c::offset, &gr_sig_source_c::set_offset);
    // 
...
//      
BULKIO::StreamSRI sig_source_c_i::createOutputSRI( int32_t idx)
{
    //
