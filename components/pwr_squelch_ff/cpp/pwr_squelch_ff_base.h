...
#ifndef PWR_SQUELCH_FF_IMPL_BASE_H
#define PWR_SQUELCH_FF_IMPL_BASE_H
...
#include "pwr_squelch_ff_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class pwr_squelch_ff_base;
...
class pwr_squelch_ff_base : public GnuHawkBlock
{
    public:
        pwr_squelch_ff_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<pwr_squelch_ff_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( pwr_squelch_ff_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( pwr_squelch_ff_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( pwr_squelch_ff_base, " serviceFunction: retval:" << retval);
