...
pwr_squelch_ff_i::pwr_squelch_ff_i(const char *uuid, const char *label) :
pwr_squelch_ff_base(uuid, label)
{
}

pwr_squelch_ff_i::~pwr_squelch_ff_i()
{
}

bool pwr_squelch_ff_i::get_unmuted()
{
    return gr_sptr->unmuted();
}

int pwr_squelch_ff_i::get_ramp()
{
    return gr_sptr->ramp();
}

bool pwr_squelch_ff_i::get_gate()
{
    return gr_sptr->gate();
}

void pwr_squelch_ff_i::set_ramp(const std::string &id)
{
    gr_sptr->set_ramp(this->ramp);
}

void pwr_squelch_ff_i::set_gate(const std::string &id)
{
...
void pwr_squelch_ff_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_pwr_squelch_ff( this->threshold, this->alpha, this->ramp, this->gate);
    } catch (...) {
        return;
    }

    this->registerGetterSetter("threshold", &gr_pwr_squelch_ff::threshold, &gr_pwr_squelch_ff::set_threshold);
    this->registerSetter("alpha", &gr_pwr_squelch_ff::set_alpha);
    this->registerGetter("squelch_range", &gr_pwr_squelch_ff::squelch_range);
    this->registerGetValue("unmuted", this, &pwr_squelch_ff_i::get_unmuted);
    this->registerGetValue("ramp", this, &pwr_squelch_ff_i::get_ramp);
    this->registerGetValue("gate", this, &pwr_squelch_ff_i::get_gate);
    this->setPropertyChangeListener("ramp", this, &pwr_squelch_ff_i::set_ramp);
    this->setPropertyChangeListener("gate", this, &pwr_squelch_ff_i::set_gate);
...
//      
BULKIO::StreamSRI pwr_squelch_ff_i::createOutputSRI( int32_t idx)
{
    //
