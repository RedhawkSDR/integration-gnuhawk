...
peak_detector_sb_i::peak_detector_sb_i(const char *uuid, const char *label) :
peak_detector_sb_base(uuid, label)
{
}

peak_detector_sb_i::~peak_detector_sb_i()
{
}

// Getters
float peak_detector_sb_i::get_threshold_factor_rise()
{
    return gr_sptr->threshold_factor_rise();
}

float peak_detector_sb_i::get_threshold_factor_fall()
{
    return gr_sptr->threshold_factor_fall();
}

int peak_detector_sb_i::get_look_ahead()
{
    return gr_sptr->look_ahead();
}

float peak_detector_sb_i::get_alpha()
{
    return gr_sptr->alpha();
}
...
void peak_detector_sb_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_peak_detector_sb( this->threshold_factor_rise, this->threshold_factor_fall, this->look_ahead, this->alpha );
    } catch (...) {
        return;
    }

    this->registerSetter("threshold_factor_rise", &gr_peak_detector_sb::set_threshold_factor_rise);
    this->registerSetter("threshold_factor_fall", &gr_peak_detector_sb::set_threshold_factor_fall);
    this->registerSetter("look_ahead", &gr_peak_detector_sb::set_look_ahead);
    this->registerSetter("alpha", &gr_peak_detector_sb::set_alpha);

    // Note, need to use redhawk getValue because GNURadio versions are not const
    this->registerGetValue("threshold_factor_rise", this, &peak_detector_sb_i::get_threshold_factor_rise);
    this->registerGetValue("threshold_factor_fall", this, &peak_detector_sb_i::get_threshold_factor_fall);
    this->registerGetValue("look_ahead", this, &peak_detector_sb_i::get_look_ahead);
    this->registerGetValue("alpha", this, &peak_detector_sb_i::get_alpha);
...
//      
BULKIO::StreamSRI peak_detector_sb_i::createOutputSRI( int32_t idx)
{
    //
