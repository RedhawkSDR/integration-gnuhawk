...
#ifndef PEAK_DETECTOR_SB_IMPL_BASE_H
#define PEAK_DETECTOR_SB_IMPL_BASE_H
...
#include "peak_detector_sb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class peak_detector_sb_base;
...
class peak_detector_sb_base : public GnuHawkBlock
{
    public:
        peak_detector_sb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<peak_detector_sb_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        float threshold_factor_rise;
        float threshold_factor_fall;
        CORBA::Long look_ahead;
        float alpha;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( peak_detector_sb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( peak_detector_sb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( peak_detector_sb_base, " serviceFunction: retval:" << retval);
