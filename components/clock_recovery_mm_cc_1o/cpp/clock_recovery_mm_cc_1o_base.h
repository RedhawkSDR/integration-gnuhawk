...
#ifndef CLOCK_RECOVERY_MM_CC_1O_IMPL_BASE_H
#define CLOCK_RECOVERY_MM_CC_1O_IMPL_BASE_H
...
#include "clock_recovery_mm_cc_1o_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class clock_recovery_mm_cc_1o_base;
...
class clock_recovery_mm_cc_1o_base : public GnuHawkBlock
{
    public:
        clock_recovery_mm_cc_1o_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<clock_recovery_mm_cc_1o_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( clock_recovery_mm_cc_1o_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( clock_recovery_mm_cc_1o_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( clock_recovery_mm_cc_1o_base, " serviceFunction: retval:" << retval);
