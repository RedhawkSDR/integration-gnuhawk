...
agc2_ff_i::agc2_ff_i(const char *uuid, const char *label) :
agc2_ff_base(uuid, label)
{
}
...
//
void agc2_ff_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
	gr_sptr = gr_make_agc2_ff(this->attack_rate, this->decay_rate, this->reference, this->gain, this->max_gain);
...
//      
BULKIO::StreamSRI agc2_ff_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
  //
