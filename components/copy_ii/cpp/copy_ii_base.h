...
#ifndef COPY_II_IMPL_BASE_H
#define COPY_II_IMPL_BASE_H
...
#include "copy_ii_GnuHawkBlock.h"
...
class copy_ii_base : public GnuHawkBlock
{
    public:
        copy_ii_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<copy_ii_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( copy_ii_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( copy_ii_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( copy_ii_base, " serviceFunction: retval:" << retval);
