...
scrambler_bb_i::scrambler_bb_i(const char *uuid, const char *label) :
scrambler_bb_base(uuid, label)
{
}

scrambler_bb_i::~scrambler_bb_i()
{
}
...
void scrambler_bb_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = digital_make_scrambler_bb( this->mask, this->seed, this->len );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI scrambler_bb_i::createOutputSRI( int32_t idx)
{
    //
