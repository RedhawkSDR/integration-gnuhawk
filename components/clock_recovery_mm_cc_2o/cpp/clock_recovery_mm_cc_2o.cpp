...
clock_recovery_mm_cc_2o_i::clock_recovery_mm_cc_2o_i(const char *uuid, const char *label) :
clock_recovery_mm_cc_2o_base(uuid, label)
{
    setPropertyChangeListener ("stream_id_map" , this, &clock_recovery_mm_cc_2o_i::streamIdChanged );
}

clock_recovery_mm_cc_2o_i::~clock_recovery_mm_cc_2o_i()
{
}

void clock_recovery_mm_cc_2o_i::streamIdChanged(const std::string& id)
{ 
   RH_ProvidesPortMap::iterator in_port;
   in_port = inPorts.find("complex_in");
...
void clock_recovery_mm_cc_2o_i::createBlock()
{
    //
    //

    try {
        gr_sptr = digital_make_clock_recovery_mm_cc( this->omega, this->gain_omega, this->mu, this->gain_mu, this->omega_relative_limit );
    } catch(...) {
        return;
    }

    this->registerGetterSetter("omega",
                               &digital_clock_recovery_mm_cc::omega,
                               &digital_clock_recovery_mm_cc::set_omega);

    this->registerGetterSetter("gain_omega",
                               &digital_clock_recovery_mm_cc::gain_omega,
                               &digital_clock_recovery_mm_cc::set_gain_omega);

    this->registerGetterSetter("mu",
                               &digital_clock_recovery_mm_cc::mu,
                               &digital_clock_recovery_mm_cc::set_mu);

    this->registerGetterSetter("gain_mu",
                               &digital_clock_recovery_mm_cc::gain_mu,
                               &digital_clock_recovery_mm_cc::set_gain_mu);

    this->registerSetter("verbose",
                         &digital_clock_recovery_mm_cc::set_verbose);
...
//      
BULKIO::StreamSRI clock_recovery_mm_cc_2o_i::createOutputSRI( int32_t oidx, int32_t &in_idx, std::string &ext)
{
    //
