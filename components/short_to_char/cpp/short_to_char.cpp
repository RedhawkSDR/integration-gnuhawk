...
short_to_char_i::short_to_char_i(const char *uuid, const char *label) :
short_to_char_base(uuid, label)
{
}

short_to_char_i::~short_to_char_i()
{
}
...
void short_to_char_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_short_to_char( this->vlen);
...
//      
BULKIO::StreamSRI short_to_char_i::createOutputSRI( int32_t idx)
{
    //
