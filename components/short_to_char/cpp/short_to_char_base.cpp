...
// Destructor
short_to_char_base::~short_to_char_base()
{
    // Free input streams
...
void short_to_char_base::setupIOMappings( )
{
   ...
   //
    ...
    if ( p_out != outPorts.end() ) {
        ...
        int mode = sri.mode;
        sid = sri.streamID;
        _ostreams.push_back( new gr_ostream< bulkio::OutCharPort > ( port, gr_sptr, i, mode, sid ));
...
int short_to_char_base::_transformerServiceFunction( std::vector< gr_istream_base * > &istreams ,
...
int short_to_char_base::_forecastAndProcess( bool &eos, std::vector< gr_istream_base * > &istreams ,
                                 std::vector< gr_ostream_base * > &ostreams  )
{
    typedef std::vector< gr_istream_base * >   _IStreamList;
    typedef std::vector< gr_ostream_base * >  _OStreamList;
...
// use incoming packet's time stamp to forward
      ...
      LOG_DEBUG( short_to_char_base, "                  mode:" <<  pktTs.tcmode );
      LOG_DEBUG( short_to_char_base, "                status:" <<  pktTs.tcstatus );
      LOG_DEBUG( short_to_char_base, "                offset:" <<  pktTs.toff );
      LOG_DEBUG( short_to_char_base, "                 whole:" <<  std::setprecision(10) << pktTs.twsec );
      LOG_DEBUG( short_to_char_base, "PKT - TS          frac:" <<  std::setprecision(12) << pktTs.tfsec );
#endif
