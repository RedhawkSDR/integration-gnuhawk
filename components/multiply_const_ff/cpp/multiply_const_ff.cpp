...
multiply_const_ff_i::multiply_const_ff_i(const char *uuid, const char *label) :
multiply_const_ff_base(uuid, label)
{
}

multiply_const_ff_i::~multiply_const_ff_i()
{
}
...
//
void multiply_const_ff_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_multiply_const_ff(k);
  ...
  // maintainTimestamp( bool onoff );
   ...
   setupIOMappings();
  this->registerGetterSetter("k", &gr_multiply_const_ff::k, &gr_multiply_const_ff::set_k);
...
//      
BULKIO::StreamSRI multiply_const_ff_i::createOutputSRI( int32_t idx)
{
  //
