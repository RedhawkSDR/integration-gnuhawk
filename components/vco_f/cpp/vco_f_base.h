...
#ifndef VCO_F_IMPL_BASE_H
#define VCO_F_IMPL_BASE_H
...
class vco_f_base : public GnuHawkBlock
{
    public:
        vco_f_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<vco_f_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( vco_f_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( vco_f_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( vco_f_base, " serviceFunction: retval:" << retval);
