...
vco_f_i::vco_f_i(const char *uuid, const char *label) :
vco_f_base(uuid, label)
{
}
...
void vco_f_i::createBlock()
{
    ...
    try {
        gr_sptr = gr_make_vco_f( this->sampling_rate, this->sensitivity, this->amplitude );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI vco_f_i::createOutputSRI( int32_t idx)
{
    //
