...
#ifndef FFT_VFC_IMPL_BASE_H
#define FFT_VFC_IMPL_BASE_H
...
#include "fft_vfc_GnuHawkBlock.h"


class fft_vfc_base : public GnuHawkBlock
{
  ...
  SRIListener( fft_vfc_base &parent ) : _parent(parent) {};

    fft_vfc_base &_parent;
  };
    

    public:

        fft_vfc_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<fft_vfc_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( fft_vfc_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( fft_vfc_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( fft_vfc_base, " serviceFunction: retval:" << retval);
