...
#ifndef MAP_BB_IMPL_BASE_H
#define MAP_BB_IMPL_BASE_H
...
class map_bb_base : public GnuHawkBlock
{
    public:
        map_bb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<map_bb_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        std::vector<CORBA::Long> map;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( map_bb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( map_bb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( map_bb_base, " serviceFunction: retval:" << retval);
