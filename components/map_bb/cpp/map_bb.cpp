...
map_bb_i::map_bb_i(const char *uuid, const char *label) :
map_bb_base(uuid, label)
{
}
...
void map_bb_i::createBlock()
{
  ...
  try {
    std::vector<int> tmap( map.begin(), map.end() );
    gr_sptr = digital_make_map_bb(tmap);
  }
  catch(...) {
    return;
  }
...
//      
BULKIO::StreamSRI map_bb_i::createOutputSRI( int32_t idx)
{
    //
