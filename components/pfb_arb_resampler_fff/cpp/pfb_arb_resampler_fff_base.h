...
#ifndef PFB_ARB_RESAMPLER_FFF_IMPL_BASE_H
#define PFB_ARB_RESAMPLER_FFF_IMPL_BASE_H
...
#include "pfb_arb_resampler_fff_GnuHawkBlock.h"


class pfb_arb_resampler_fff_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  pfb_arb_resampler_fff_base;
  ...
  SRIListener( pfb_arb_resampler_fff_base &parent ) : _parent(parent) {};

    pfb_arb_resampler_fff_base &_parent;
  };
    

    public:

        pfb_arb_resampler_fff_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<pfb_arb_resampler_fff_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( pfb_arb_resampler_fff_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( pfb_arb_resampler_fff_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( pfb_arb_resampler_fff_base, " serviceFunction: retval:" << retval);
