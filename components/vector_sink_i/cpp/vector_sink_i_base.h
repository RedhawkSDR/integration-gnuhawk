...
#ifndef VECTOR_SINK_I_IMPL_BASE_H
#define VECTOR_SINK_I_IMPL_BASE_H
...
#include "vector_sink_i_GnuHawkBlock.h"


class vector_sink_i_base : public GnuHawkBlock
{
    friend class BULKIO_dataLong_In_i;

  class SRIListener : BULKIO_dataLong_In_i::SRIListener {

    friend class  vector_sink_i_base;
  ...
  SRIListener( vector_sink_i_base &parent ) : _parent(parent) {};

    vector_sink_i_base &_parent;
  };
    

    public:

        vector_sink_i_base(const char *uuid, const char *label);
 ...
 typedef std::deque< std::pair< BULKIO_dataLong_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _IStream< BULKIO_dataLong_In_i >::List    IStreamList;
   ...
   ProcessThread<vector_sink_i_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( vector_sink_i_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( vector_sink_i_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( vector_sink_i_base, " serviceFunction: retval:" << retval);
