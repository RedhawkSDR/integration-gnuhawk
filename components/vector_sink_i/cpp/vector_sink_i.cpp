...
vector_sink_i_i::vector_sink_i_i(const char *uuid, const char *label) :
vector_sink_i_base(uuid, label)
{
}

vector_sink_i_i::~vector_sink_i_i()
{
}
...
std::vector< CORBA::Long > vector_sink_i_i::getData() {

  std::vector< CORBA::Long > out_data;
  if ( validGRBlock() ) {

    std::vector<int> t_data;
    t_data = gr_sptr->data();
    out_data.resize(t_data.size());
    std::transform( t_data.begin(), t_data.end(), out_data.begin(), int_to_long() );
  }
...
//
void vector_sink_i_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_vector_sink_i(vlen);
  this->registerGetValue("data", this, &vector_sink_i_i::getData);
  this->setPropertyChangeListener("reset", this, &vector_sink_i_i::on_reset);
...
void vector_sink_i_i::on_reset (const std::string&)
{
