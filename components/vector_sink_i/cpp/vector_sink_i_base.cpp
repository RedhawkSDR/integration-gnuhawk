...
void vector_sink_i_base::construct()
{
    ...
    long_in->setNewStreamListener(this, &vector_sink_i_base::long_in_newStreamCallback);
...
void vector_sink_i_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        long_in->unblock();
        serviceThread = new ProcessThread<vector_sink_i_base>(this, 0.1);
        serviceThread->start();
    }
...
void vector_sink_i_base::long_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( vector_sink_i_base, "START NotifySRI  port:stream " << long_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( long_in, sri ) );

  LOG_TRACE( vector_sink_i_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << long_in->getName() << "/" << sri.streamID); 
}
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > istream->nitems() ) {
                        LOG_WARN( vector_sink_i_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE " 
...
template <  typename IN_PORT_TYPE > int vector_sink_i_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams )
{
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( vector_sink_i_base, "GET MAX ITEMS: STREAM:" << idx << " NITEMS/SCALARS:"
        ...
        if ( gr_sptr->fixed_rate() ) {
            ...
            LOG_TRACE( vector_sink_i_base, " FIXED FORECAST NOUTPUT/output_multiple == " 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( vector_sink_i_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" << 
        ...
        if ( !dataReady ) {
            LOG_TRACE( vector_sink_i_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( vector_sink_i_base, " CALLING WORK.....N_OUT:" << noutput_items << 
