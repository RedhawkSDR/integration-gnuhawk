...
void vector_source_s_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        serviceThread = new ProcessThread<vector_source_s_base>(this, 0.1);
        serviceThread->start();
    }
...
BULKIO::StreamSRI vector_source_s_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI vector_source_s_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
...
template < typename OUT_PORT_TYPE > int vector_source_s_base::_generatorServiceFunction( typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams ) 
{
    ...
    if (numOut != 0 or (eos and !sentEOS)){
        ...
        ostream = ostreams.begin();
        for ( ; ostream != ostreams.end(); ostream++ ) {
            LOG_TRACE( vector_source_s_base, "PUSHING DATA   NOUT/NITEMS/OITEMS/STREAM_ID " << numOut << 
