...
vector_source_s_i::vector_source_s_i(const char *uuid, const char *label) :
vector_source_s_base(uuid, label)
{
}

vector_source_s_i::~vector_source_s_i()
{
}

void vector_source_s_i::changedSamplingFreq(const std::string &id)
{
...
//
void vector_source_s_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_vector_source_s(data, repeat, vlen);
  ...
  setupIOMappings();
  this->registerSetter("data", &gr_vector_source_s::set_data);
  this->setPropertyChangeListener("rewind", this, &vector_source_s_i::onRewind);
  this->setPropertyChangeListener("sampling_freq", this, &vector_source_s_i::changedSamplingFreq);
} 
...
//      
BULKIO::StreamSRI vector_source_s_i::createOutputSRI( int32_t idx)
{
  //
...
void vector_source_s_i::onRewind(const std::string&)
{
