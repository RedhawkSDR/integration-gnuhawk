...
#ifndef REPEAT_II_IMPL_BASE_H
#define REPEAT_II_IMPL_BASE_H
...
#include "repeat_ii_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class repeat_ii_base;
...
class repeat_ii_base : public GnuHawkBlock
{
    public:
        repeat_ii_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<repeat_ii_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( repeat_ii_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( repeat_ii_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( repeat_ii_base, " serviceFunction: retval:" << retval);
