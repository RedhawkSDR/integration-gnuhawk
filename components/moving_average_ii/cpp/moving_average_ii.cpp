...
moving_average_ii_i::moving_average_ii_i(const char *uuid, const char *label) :
moving_average_ii_base(uuid, label)
{
}

moving_average_ii_i::~moving_average_ii_i()
{
}

void moving_average_ii_i::set_length_and_scale(const std::string &id)
{
...
void moving_average_ii_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_moving_average_ii( this->length, this->scale, this->max_iter );
    } catch (...) {
        return;
    }

    this->registerGetter("length", &gr_moving_average_ii::length);
    this->registerGetter("scale", &gr_moving_average_ii::scale);

    this->setPropertyChangeListener("length", this, &moving_average_ii_i::set_length_and_scale);
    this->setPropertyChangeListener("scale", this, &moving_average_ii_i::set_length_and_scale);
...
//      
BULKIO::StreamSRI moving_average_ii_i::createOutputSRI( int32_t idx)
{
    //
