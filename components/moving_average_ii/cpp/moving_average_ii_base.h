...
#ifndef MOVING_AVERAGE_II_IMPL_BASE_H
#define MOVING_AVERAGE_II_IMPL_BASE_H
...
#include "moving_average_ii_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class moving_average_ii_base;
...
class moving_average_ii_base : public GnuHawkBlock
{
    public:
        moving_average_ii_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<moving_average_ii_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( moving_average_ii_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( moving_average_ii_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( moving_average_ii_base, " serviceFunction: retval:" << retval);
