...
#ifndef FFT_VCC_IMPL_BASE_H
#define FFT_VCC_IMPL_BASE_H
...
#include "fft_vcc_GnuHawkBlock.h"


class fft_vcc_base : public GnuHawkBlock
{
  ...
  SRIListener( fft_vcc_base &parent ) : _parent(parent) {};

    fft_vcc_base &_parent;
  };
    

    public:

        fft_vcc_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<fft_vcc_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( fft_vcc_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( fft_vcc_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( fft_vcc_base, " serviceFunction: retval:" << retval);
