...
#ifndef INTERP_FIR_FILTER_CCF_IMPL_BASE_H
#define INTERP_FIR_FILTER_CCF_IMPL_BASE_H
...
#include "interp_fir_filter_ccf_GnuHawkBlock.h"


class interp_fir_filter_ccf_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  interp_fir_filter_ccf_base;
  ...
  SRIListener( interp_fir_filter_ccf_base &parent ) : _parent(parent) {};

    interp_fir_filter_ccf_base &_parent;
  };
    

    public:

        interp_fir_filter_ccf_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<interp_fir_filter_ccf_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( interp_fir_filter_ccf_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( interp_fir_filter_ccf_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( interp_fir_filter_ccf_base, " serviceFunction: retval:" << retval);
