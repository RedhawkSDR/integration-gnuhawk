...
#ifndef TAGGED_FILE_SINK_S_IMPL_BASE_H
#define TAGGED_FILE_SINK_S_IMPL_BASE_H
...
#include "tagged_file_sink_s_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class tagged_file_sink_s_base;
...
class tagged_file_sink_s_base : public GnuHawkBlock
{
    public:
        tagged_file_sink_s_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<tagged_file_sink_s_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( tagged_file_sink_s_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( tagged_file_sink_s_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( tagged_file_sink_s_base, " serviceFunction: retval:" << retval);
