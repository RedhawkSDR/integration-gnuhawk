...
vector_source_f_i::vector_source_f_i(const char *uuid, const char *label) :
vector_source_f_base(uuid, label)
{
}

vector_source_f_i::~vector_source_f_i()
{
}

void vector_source_f_i::changedSamplingFreq(const std::string &id)
{
...
//
void vector_source_f_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_vector_source_f(data, repeat, vlen);
  ...
  setupIOMappings();
  this->registerSetter("data", &gr_vector_source_f::set_data);
  this->setPropertyChangeListener("rewind", this, &vector_source_f_i::onRewind);
  this->setPropertyChangeListener("sampling_freq", this, &vector_source_f_i::changedSamplingFreq);
} 
...
//      
BULKIO::StreamSRI vector_source_f_i::createOutputSRI( int32_t idx)
{
  //
...
void vector_source_f_i::onRewind (const std::string&)
{
  if (rewind && this->validGRBlock()) {
    gr_sptr->rewind();
    rewind = false;
  }
}
