...
#include "deinterleave_ff_2o_base.h"
...
deinterleave_ff_2o_base::deinterleave_ff_2o_base(const char *uuid, const char *label) :
...
void deinterleave_ff_2o_base::construct()
{
    ...
    float_in->setNewStreamListener(this, &deinterleave_ff_2o_base::float_in_newStreamCallback);
...
*******************************************************************************************/
void deinterleave_ff_2o_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void deinterleave_ff_2o_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    boost::mutex::scoped_lock lock(serviceThreadLock);
    if (serviceThread == 0) {
        float_in->unblock();
        serviceThread = new ProcessThread<deinterleave_ff_2o_base>(this, 0.1);
        serviceThread->start();
    }
...
void deinterleave_ff_2o_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( deinterleave_ff_2o_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( deinterleave_ff_2o_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr deinterleave_ff_2o_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void deinterleave_ff_2o_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void deinterleave_ff_2o_base::loadProperties()
{
...
// Destructor
deinterleave_ff_2o_base::~deinterleave_ff_2o_base()
{
    // Free input streams
...
//  Allow for logging 
// 
PREPARE_LOGGING(deinterleave_ff_2o_base);
...
uint32_t deinterleave_ff_2o_base::getNOutputStreams() {
    return 0;
}

void deinterleave_ff_2o_base::setupIOMappings( )
{
    ...
    LOG_DEBUG( deinterleave_ff_2o_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
    ...
    if ( _istreams.size() > 0 || _ostreams.size() > 0 ) {
        ...
        LOG_DEBUG( deinterleave_ff_2o_base, "RESET INPUT SIGNATURE SIZE:" << _istreams.size() );
        IStreamList::iterator istream;
        for ( int idx=0 ; istream != _istreams.end(); idx++, istream++ ) {
            // re-add existing stream definitons
            LOG_DEBUG(  deinterleave_ff_2o_base, "ADD READ INDEX TO GNU RADIO BLOCK");
        ...
        LOG_DEBUG( deinterleave_ff_2o_base, "RESET OUTPUT SIGNATURE SIZE:" << _ostreams.size() );
   ...
   //
    LOG_TRACE( deinterleave_ff_2o_base, "setupIOMappings INPUT PORTS: " << inPorts.size() );
    ...
    if ( p_in != inPorts.end() ) {
        ...
        LOG_DEBUG( deinterleave_ff_2o_base, "ADDING INPUT MAP IDX:" << i << " SID:" << sid );
    ...
    // streams over a single connection.  We are mapping a single RH Port to a single GNU Radio stream.
    //
    LOG_TRACE( deinterleave_ff_2o_base, "setupIOMappings OutputPorts: " << outPorts.size() );
    ...
    if ( p_out != outPorts.end() ) {
        ...
        LOG_DEBUG( deinterleave_ff_2o_base, "ADDING OUTPUT MAP IDX:" << i << " SID:" << sid );
...
void deinterleave_ff_2o_base::float_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( deinterleave_ff_2o_base, "START NotifySRI  port:stream " << float_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( float_in, sri ) );

  LOG_TRACE( deinterleave_ff_2o_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << float_in->getName() << "/" << sri.streamID); 
}


void deinterleave_ff_2o_base::processStreamIdChanges()
{
    boost::mutex::scoped_lock lock(_sriMutex);

    LOG_TRACE( deinterleave_ff_2o_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
    ...
    if ( validGRBlock() ) {
        ...
        for ( ; item != _sriQueue.end(); item++ ) {
            ...
            for ( ; istream != _istreams.end(); idx++, istream++ ) {
                ...
                if ( (*istream)->getPort() == item->first ) {
                    LOG_DEBUG( deinterleave_ff_2o_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
                    (*istream)->sri(true);
                    (*istream)->spe(mode);

                    LOG_DEBUG( deinterleave_ff_2o_base,  "  SETTING  OUT_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
        ...
        LOG_WARN( deinterleave_ff_2o_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
    }

}

BULKIO::StreamSRI deinterleave_ff_2o_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI deinterleave_ff_2o_base::createOutputSRI( int32_t oidx, int32_t &in_idx, std::string &ext)
{
    return createOutputSRI( oidx );
}

void deinterleave_ff_2o_base::adjustOutputRate(BULKIO::StreamSRI &sri )
{
    if ( validGRBlock() ) {
...
**/
        LOG_TRACE( deinterleave_ff_2o_base, "ADJUSTING SRI.XDELTA FROM/TO: " << sri.xdelta << "/" << ret );
        sri.xdelta = ret;
    }
}

deinterleave_ff_2o_base::TimeDuration deinterleave_ff_2o_base::getTargetDuration()
{
    ...
    LOG_TRACE( deinterleave_ff_2o_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
              " target_duration " << t_drate );
    return t_drate;
}

deinterleave_ff_2o_base::TimeDuration deinterleave_ff_2o_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time )
{
    TimeDuration delta;
    ...
    if ( start_time.is_not_a_date_time() == false ) {
        ...
        delta /= 4;
        LOG_TRACE( deinterleave_ff_2o_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
                  " delta " << delta );
    }
...
int deinterleave_ff_2o_base::_transformerServiceFunction( std::vector< gr_istream_base * > &istreams ,
    std::vector< gr_ostream_base * > &ostreams  )
{
    ...
    if ( validGRBlock() == false ) {
        ...
        LOG_DEBUG( deinterleave_ff_2o_base, " FINISHED BUILDING  GNU RADIO BLOCK");
    }
    ...
    if ( !validGRBlock() || istreams.size() == 0 || ostreams.size() == 0  ) {
        LOG_WARN( deinterleave_ff_2o_base, "NO STREAMS ATTACHED TO BLOCK..." );
        return NOOP;
    }
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
        ...
        if ( (*istream)->overrun() ) {
            LOG_WARN( deinterleave_ff_2o_base, " NOT KEEPING UP WITH STREAM ID:" << (*istream)->streamID );
        }

        if ( (*istream)->sriChanged() ) {
            // RESOLVE - need to look at how SRI changes can affect Gnu Radio BLOCK state
            LOG_DEBUG( deinterleave_ff_2o_base, "SRI CHANGED, STREAMD IDX/ID: " 
                      << idx << "/" << (*istream)->getPktStreamId() );
            setOutputStreamSRI( idx, (*istream)->getPktSri() );
        }
    }

    LOG_TRACE( deinterleave_ff_2o_base, "READ NITEMS: "  << nitems );
    ...
    while ( nout > -1 && serviceThread->threadRunning() ) {
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > (*istream)->nitems() ) {
                        LOG_WARN( deinterleave_ff_2o_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE "
                                 << nitems << "/" << (*istream)->nitems() );
                        nitems = (*istream)->nitems();
                    }
                    (*istream)->consume( nitems );
                    LOG_TRACE( deinterleave_ff_2o_base, " CONSUME READ DATA  ITEMS/REMAIN " << nitems << "/" << (*istream)->nitems());
                }
            }
        ...
        if ( nout == -1 ) {
            ...
            if ( eos ) {
                LOG_TRACE(  deinterleave_ff_2o_base, "EOS SEEN, SENDING DOWNSTREAM " );
    ...
    if ( eos ) {
        ...
        for ( ; istream != istreams.end() ; istream++ ) {
            int idx=std::distance( istreams.begin(), istream );
            LOG_DEBUG( deinterleave_ff_2o_base, " CLOSING INPUT STREAM IDX:" << idx );
            (*istream)->close();
        }
        ...
        for ( ; eos && ostream != ostreams.end(); ostream++ ) {
            ...
            LOG_DEBUG( deinterleave_ff_2o_base, " CLOSING OUTPUT STREAM IDX:" << idx );
            (*ostream)->close();
        }
    }
    ...
    LOG_TRACE( deinterleave_ff_2o_base, " END OF TRANSFORM SERVICE FUNCTION....." << noutput_items );
...
int deinterleave_ff_2o_base::_forecastAndProcess( bool &eos, std::vector< gr_istream_base * > &istreams ,
                                 std::vector< gr_ostream_base * > &ostreams  )
{
    typedef std::vector< gr_istream_base * >   _IStreamList;
    typedef std::vector< gr_ostream_base * >  _OStreamList;
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( deinterleave_ff_2o_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << 
                       (*istream)->nitems() << "/" << (*istream)->nelems() );
            max_items_avail = std::max( (*istream)->nitems(), max_items_avail );
        }

        if ( max_items_avail == 0  ) {
            LOG_TRACE( deinterleave_ff_2o_base, "DATA CHECK - MAX ITEMS  NOUTPUT/MAX_ITEMS:" <<   noutput_items << "/" << max_items_avail);
            return -1;
        }
        ...
        if ( !gr_sptr->fixed_rate() )  {
            ...
            LOG_TRACE( deinterleave_ff_2o_base, " VARIABLE FORECAST NOUTPUT == " << noutput_items );
        } else {
            istream = istreams.begin();
            ...
            LOG_TRACE( deinterleave_ff_2o_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            LOG_TRACE( deinterleave_ff_2o_base, "--> FORECAST IN/OUT " << _ninput_items_required[0]  << "/" << noutput_items  );
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( deinterleave_ff_2o_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   (*istream)->nelems() << 
            ...
            LOG_TRACE( deinterleave_ff_2o_base, " TRIM FORECAST NOUTPUT/READY " << noutput_items << "/" << dataReady );
        }

        // check if data is ready...
        if ( !dataReady ) {
            LOG_TRACE( deinterleave_ff_2o_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0]->nitems() << 
        ...
        for ( int idx=0 ; istream != istreams.end(); idx++, istream++ ) {
            ...
            try {
                ...
                LOG_ERROR( deinterleave_ff_2o_base, "MISSING INPUT STREAM FOR GR BLOCK, STREAM ID:" <<   (*istream)->streamID );
                return -2;
            } 
    
            nitems = (*istream)->nitems() - ritems;
            LOG_TRACE( deinterleave_ff_2o_base,  " ISTREAM: IDX:" << idx  << " ITEMS AVAIL/READ/REQ " << nitems << "/" 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( deinterleave_ff_2o_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems 
            ...
            LOG_TRACE( deinterleave_ff_2o_base, "RETURN  WORK ..... N_OUT:" << nout);
        }

        // check for stop condition from work method
        if ( nout < gr_block::WORK_DONE ) {
            LOG_WARN( deinterleave_ff_2o_base, "WORK RETURNED STOP CONDITION..." << nout );
            nout=0;
            eos = true;
        }
    }
    ...
    if (nout != 0 or eos ) {
        noutput_items = nout;
        LOG_TRACE( deinterleave_ff_2o_base, " WORK RETURNED: NOUT : " << nout << " EOS:" << eos);
        ostream = ostreams.begin();
        ...
        for ( int idx=0 ; ostream != ostreams.end(); idx++, ostream++ ) {
            ...
            LOG_TRACE( deinterleave_ff_2o_base,  "PUSHING DATA   ITEMS/STREAM_ID " << (*ostream)->nitems() << "/" << (*ostream)->streamID );    
            if ( _maintainTimeStamp ) {

                // set time stamp for output samples based on input time stamp
                if ( (*ostream)->nelems() == 0 )  {
#ifdef TEST_TIME_STAMP
      LOG_DEBUG( deinterleave_ff_2o_base, "SEED - TS SRI:  xdelta:" << std::setprecision(12) << ostream->sri.xdelta );
      LOG_DEBUG( deinterleave_ff_2o_base, "OSTREAM WRITE:   maint:" << _maintainTimeStamp );
      LOG_DEBUG( deinterleave_ff_2o_base, "                  mode:" <<  ostream->tstamp.tcmode );
      LOG_DEBUG( deinterleave_ff_2o_base, "                status:" <<  ostream->tstamp.tcstatus );
      LOG_DEBUG( deinterleave_ff_2o_base, "                offset:" <<  ostream->tstamp.toff );
      LOG_DEBUG( deinterleave_ff_2o_base, "                 whole:" <<  std::setprecision(10) << ostream->tstamp.twsec );
      LOG_DEBUG( deinterleave_ff_2o_base, "SEED - TS         frac:" <<  std::setprecision(12) << ostream->tstamp.tfsec );
#endif
...
// use incoming packet's time stamp to forward
                if ( gotPkt ) {
#ifdef TEST_TIME_STAMP
      LOG_DEBUG( deinterleave_ff_2o_base, "OSTREAM  SRI:  items/xdelta:" << noutput_items << "/" << std::setprecision(12) << ostream->sri.xdelta );
      LOG_DEBUG( deinterleave_ff_2o_base, "PKT - TS         maint:" << _maintainTimeStamp );
      LOG_DEBUG( deinterleave_ff_2o_base, "                  mode:" <<  pktTs.tcmode );
      LOG_DEBUG( deinterleave_ff_2o_base, "                status:" <<  pktTs.tcstatus );
      LOG_DEBUG( deinterleave_ff_2o_base, "                offset:" <<  pktTs.toff );
      LOG_DEBUG( deinterleave_ff_2o_base, "                 whole:" <<  std::setprecision(10) << pktTs.twsec );
      LOG_DEBUG( deinterleave_ff_2o_base, "PKT - TS          frac:" <<  std::setprecision(12) << pktTs.tfsec );
#endif
                    (*ostream)->write( noutput_items, eos, pktTs  );
                } else {
#ifdef TEST_TIME_STAMP
      LOG_DEBUG( deinterleave_ff_2o_base, "OSTREAM  SRI:  items/xdelta:" << noutput_items << "/" << std::setprecision(12) << ostream->sri.xdelta );
      LOG_DEBUG( deinterleave_ff_2o_base, "OSTREAM TOD      maint:" << _maintainTimeStamp );
      LOG_DEBUG( deinterleave_ff_2o_base, "                  mode:" <<  ostream->tstamp.tcmode );
      LOG_DEBUG( deinterleave_ff_2o_base, "                status:" <<  ostream->tstamp.tcstatus );
      LOG_DEBUG( deinterleave_ff_2o_base, "                offset:" <<  ostream->tstamp.toff );
      LOG_DEBUG( deinterleave_ff_2o_base, "                 whole:" <<  std::setprecision(10) << ostream->tstamp.twsec );
      LOG_DEBUG( deinterleave_ff_2o_base, "OSTREAM TOD       frac:" <<  std::setprecision(12) << ostream->tstamp.tfsec );
#endif
