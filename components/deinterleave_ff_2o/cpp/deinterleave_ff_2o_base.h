...
#ifndef DEINTERLEAVE_FF_2O_IMPL_BASE_H
#define DEINTERLEAVE_FF_2O_IMPL_BASE_H
...
#include "deinterleave_ff_2o_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class deinterleave_ff_2o_base;
...
class deinterleave_ff_2o_base : public GnuHawkBlock
{
    public:
        deinterleave_ff_2o_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<deinterleave_ff_2o_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( deinterleave_ff_2o_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( deinterleave_ff_2o_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( deinterleave_ff_2o_base, " serviceFunction: retval:" << retval);
