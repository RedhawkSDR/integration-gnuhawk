...
file_sink_i_i::file_sink_i_i(const char *uuid, const char *label) :
file_sink_i_base(uuid, label)
{
}

file_sink_i_i::~file_sink_i_i()
{
}
...
void file_sink_i_i::createBlock()
{
    //
    ...
    try{
    	this->gr_sptr = gr_make_file_sink(this->itemsize, this->filename.c_str());
    }
    catch(...){
    	return;
    }
