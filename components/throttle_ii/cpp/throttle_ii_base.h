...
#ifndef THROTTLE_II_IMPL_BASE_H
#define THROTTLE_II_IMPL_BASE_H
...
#include "throttle_ii_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class throttle_ii_base;
...
class throttle_ii_base : public GnuHawkBlock
{
    public:
        throttle_ii_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<throttle_ii_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::ULong itemsize;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( throttle_ii_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( throttle_ii_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( throttle_ii_base, " serviceFunction: retval:" << retval);
