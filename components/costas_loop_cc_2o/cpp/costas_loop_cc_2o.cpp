...
costas_loop_cc_2o_i::costas_loop_cc_2o_i(const char *uuid, const char *label) :
costas_loop_cc_2o_base(uuid, label)
{
    setPropertyChangeListener ("stream_id_map" , this, &costas_loop_cc_2o_i::streamIdChanged );
}

costas_loop_cc_2o_i::~costas_loop_cc_2o_i()
{
}

void costas_loop_cc_2o_i::streamIdChanged(const std::string& id)
{ 
...
void costas_loop_cc_2o_i::createBlock()
{
    //
    //

    try {
        gr_sptr = digital_make_costas_loop_cc( this->loop_bw, this->order );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI costas_loop_cc_2o_i::createOutputSRI( int32_t oidx, int32_t &in_idx, std::string &ext)
{
    //
