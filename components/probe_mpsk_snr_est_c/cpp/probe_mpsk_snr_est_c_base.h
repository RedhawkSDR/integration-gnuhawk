...
#ifndef PROBE_MPSK_SNR_EST_C_IMPL_BASE_H
#define PROBE_MPSK_SNR_EST_C_IMPL_BASE_H
...
#include "probe_mpsk_snr_est_c_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class probe_mpsk_snr_est_c_base;
...
class probe_mpsk_snr_est_c_base : public GnuHawkBlock
{
    public:
        probe_mpsk_snr_est_c_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<probe_mpsk_snr_est_c_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::Long type;
        CORBA::Long msg_nsamples;
        double alpha;
        double snr;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( probe_mpsk_snr_est_c_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( probe_mpsk_snr_est_c_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( probe_mpsk_snr_est_c_base, " serviceFunction: retval:" << retval);
