...
#ifndef ARGMAX_FS_4I_IMPL_BASE_H
#define ARGMAX_FS_4I_IMPL_BASE_H
...
#include "argmax_fs_4i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class argmax_fs_4i_base;
...
class argmax_fs_4i_base : public GnuHawkBlock
{
    public:
        argmax_fs_4i_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<argmax_fs_4i_base> *serviceThread; 
    ...
    protected:
        ...
        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  float_in_4_newStreamCallback( BULKIO::StreamSRI &sri );
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( argmax_fs_4i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( argmax_fs_4i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( argmax_fs_4i_base, " serviceFunction: retval:" << retval);
