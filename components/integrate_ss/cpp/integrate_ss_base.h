...
#ifndef INTEGRATE_SS_IMPL_BASE_H
#define INTEGRATE_SS_IMPL_BASE_H
...
#include "integrate_ss_GnuHawkBlock.h"


class integrate_ss_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataShort_In_i::SRIListener {
    ...
    friend class  integrate_ss_base;
  ...
  SRIListener( integrate_ss_base &parent ) : _parent(parent) {};

    integrate_ss_base &_parent;
  };
    

    public:

        integrate_ss_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataShort_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataShort_Out_i >::List   OStreamList;  
   ...
   ProcessThread<integrate_ss_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( integrate_ss_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( integrate_ss_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( integrate_ss_base, " serviceFunction: retval:" << retval);
