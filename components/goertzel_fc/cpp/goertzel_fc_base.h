...
#ifndef GOERTZEL_FC_IMPL_BASE_H
#define GOERTZEL_FC_IMPL_BASE_H
...
#include "goertzel_fc_GnuHawkBlock.h"


class goertzel_fc_base : public GnuHawkBlock
{
  ...
  SRIListener( goertzel_fc_base &parent ) : _parent(parent) {};

    goertzel_fc_base &_parent;
  };
    

    public:

        goertzel_fc_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<goertzel_fc_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( goertzel_fc_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( goertzel_fc_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( goertzel_fc_base, " serviceFunction: retval:" << retval);
