...
rms_ff_i::rms_ff_i(const char *uuid, const char *label) :
rms_ff_base(uuid, label)
{
}
...
void rms_ff_i::createBlock()
{
    ...
    try {
        gr_sptr = gr_make_rms_ff( this->alpha );
    } catch (...) {
        return;
    }

    this->registerSetter("alpha", &gr_rms_ff::set_alpha);
    this->registerGetter("unmuted", &gr_rms_ff::unmuted);
...
//      
BULKIO::StreamSRI rms_ff_i::createOutputSRI( int32_t idx)
{
    //
