...
#ifndef RMS_FF_IMPL_BASE_H
#define RMS_FF_IMPL_BASE_H
...
class rms_ff_base : public GnuHawkBlock
{
    public:
        rms_ff_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<rms_ff_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( rms_ff_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( rms_ff_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( rms_ff_base, " serviceFunction: retval:" << retval);
