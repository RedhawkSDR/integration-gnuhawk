...
//RESOLVE    ENABLE_LOGGING;
    public:
        ...
        BULKIO_dataOctet_Out_i(std::string port_name, psk_demod_cb_base *_parent);
       ...
       template <typename ALLOCATOR>
        void pushPacket(std::vector<unsigned char, ALLOCATOR>& data, BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID) {
				 ...
				 for (port = outConnections.begin(); port != outConnections.end(); port++) {
					 ...
					 //RESOLVE LOG_ERROR(BULKIO_dataOctet_Out_i, "Call to pushPacket by BULKIO_dataOctet_Out_i failed");
				     }
				 }
			     }
			 };
...
class BULKIO_dataFloat_In_i : public POA_BULKIO::dataFloat, public Port_Provides_base_impl
{
    public:
     ...
     //BULKIO_dataFloat_In_i(std::string port_name, psk_demod_cb_base *_parent);
