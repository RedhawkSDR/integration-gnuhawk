...
#ifndef PSK_DEMOD_CB_IMPL_BASE_H
#define PSK_DEMOD_CB_IMPL_BASE_H
...
#include "psk_demod_cb_GnuHawkBlock.h"


class psk_demod_cb_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  psk_demod_cb_base;
  ...
  SRIListener( psk_demod_cb_base &parent ) : _parent(parent) {};

    psk_demod_cb_base &_parent;
  };
    

    public:

        psk_demod_cb_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataOctet_Out_i >::List   OStreamList;  
   ...
   ProcessThread<psk_demod_cb_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( psk_demod_cb_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( psk_demod_cb_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( psk_demod_cb_base, " serviceFunction: retval:" << retval);
