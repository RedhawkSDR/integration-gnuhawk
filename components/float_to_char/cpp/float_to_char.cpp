...
float_to_char_i::float_to_char_i(const char *uuid, const char *label) :
float_to_char_base(uuid, label)
{
}

float_to_char_i::~float_to_char_i()
{
}
...
void float_to_char_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_float_to_char( this->vlen, this->scale );

    this->registerGetterSetter("scale",
                               &gr_float_to_char::scale,
                               &gr_float_to_char::set_scale);
    // 
...
//      
BULKIO::StreamSRI float_to_char_i::createOutputSRI( int32_t idx)
{
    //
