...
noise_source_f_i::noise_source_f_i(const char *uuid, const char *label) :
noise_source_f_base(uuid, label)
{
}

noise_source_f_i::~noise_source_f_i()
{
}
...
//
void noise_source_f_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_noise_source_f((gr_noise_type_t)type, amplitude, seed);
  this->registerGetterSetter("type", &gr_noise_source_f::type, &gr_noise_source_f::set_type);
  this->registerGetterSetter("amplitude", &gr_noise_source_f::amplitude, &gr_noise_source_f::set_amplitude);
  this->setPropertyChangeListener("sampling_freq", this, &noise_source_f_i::changedSamplingFreq);
  // 
...
//      
BULKIO::StreamSRI noise_source_f_i::createOutputSRI( int32_t idx)
{
  //
...
void noise_source_f_i::changedSamplingFreq(const std::string &id)
{
