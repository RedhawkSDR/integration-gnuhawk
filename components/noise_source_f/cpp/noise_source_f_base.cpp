...
#include "noise_source_f_base.h"

/*******************************************************************************************

    AUTO-GENERATED CODE. DO NOT MODIFY
    
 	Source: noise_source_f.spd.xml
 	Generated on: Fri Mar 08 13:30:14 EST 2013
 	Redhawk IDE
 	Version:M.1.8.3
...
//  Allow for logging 
// 
PREPARE_LOGGING(noise_source_f_base);
...
noise_source_f_base::noise_source_f_base(const char *uuid, const char *label) :
...
void noise_source_f_base::construct()
{
...
*******************************************************************************************/
void noise_source_f_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void noise_source_f_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
...
void noise_source_f_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( noise_source_f_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( noise_source_f_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr noise_source_f_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void noise_source_f_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
    ...
    LOG_TRACE( noise_source_f_base, "COMPLETED RELEASE OBJECT" );
}

void noise_source_f_base::loadProperties()
{
...
uint32_t noise_source_f_base::getNOutputStreams() {
	return 0;
}
  
void noise_source_f_base::setupIOMappings( ) 
{
  ...
  LOG_DEBUG( noise_source_f_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
  ...
  // 
   ...
   if ( _ostreams.size() > 0 ) {
    ...
    LOG_DEBUG(  noise_source_f_base, "RESET OUTPUT SIGNATURE SIZE:" << _ostreams.size() );
   ...
   //
   LOG_TRACE( noise_source_f_base, "setupIOMappings OutputPorts: " << outPorts.size() );
   ...
   for( int i=0; pname != outputPortOrder.end(); pname++ ) {
	   ...
	   LOG_DEBUG(  noise_source_f_base, "ADDING OUTPUT MAP IDX:" << i << " SID:" << sid );	
...
BULKIO::StreamSRI noise_source_f_base::createOutputSRI( int32_t idx)
{
...
void noise_source_f_base::adjustOutputRate(BULKIO::StreamSRI &sri ) {

   if ( validGRBlock() ) {
      double ret=sri.xdelta*gr_sptr->relative_rate();
/**      
**/
      LOG_TRACE(noise_source_f_base, "ADJUSTING SRI.XDELTA FROM/TO: " << sri.xdelta << "/" << ret );
      sri.xdelta = ret;
...
noise_source_f_base::TimeDuration noise_source_f_base::getTargetDuration() {
  ...
  LOG_TRACE( noise_source_f_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
	     " target_duration " << t_drate );
  return t_drate;
}

noise_source_f_base::TimeDuration noise_source_f_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time ) {
  ...
  if ( start_time.is_not_a_date_time() == false ) {
    ...
    delta /= 4;
    LOG_TRACE( noise_source_f_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
	      " delta " << delta );
  }
  return delta;
}
...
template < typename OUT_PORT_TYPE > int noise_source_f_base::_generatorServiceFunction( typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams ) 
{
  ...
  if ( validGRBlock() == false ) {
    ...
    LOG_DEBUG( noise_source_f_base, "FINISHED BUILDING  GNU RADIO BLOCK");
  }


  if ( !validGRBlock() || ostreams.size() == 0  ) {
    LOG_WARN( noise_source_f_base, "NO OUTPUT STREAMS DEFINED FOR GNU RADIO BLOCK..." );
    return NOOP;
  }
  ...
  LOG_TRACE(  noise_source_f_base, " FORECAST == " << noutput_items );
  ...
  if (numOut != 0 or (eos and !sentEOS)){
    ...
    ostream = ostreams.begin();
    for( ; ostream != ostreams.end(); ostream++ ) {
      LOG_TRACE( noise_source_f_base, "PUSHING DATA   NOUT/NITEMS/OITEMS/STREAM_ID " << numOut << "/" << ostream->nitems()  << "/" << ostream->oitems() << "/" << ostream->streamID );
#ifdef TEST_TIME_STAMP
      LOG_DEBUG(  noise_source_f_base, "OSTREAM SRI:    xdelta:" << std::setprecision(12) << ostream->sri.xdelta );
      LOG_DEBUG(  noise_source_f_base, "OSTREAM WRITE:   maint:" << _maintainTimeStamp );
      LOG_DEBUG(  noise_source_f_base, "                  mode:" <<  ostream->tstamp.tcmode );
      LOG_DEBUG(  noise_source_f_base, "                status:" <<  ostream->tstamp.tcstatus );
      LOG_DEBUG(  noise_source_f_base, "                offset:" <<  ostream->tstamp.toff );
      LOG_DEBUG(  noise_source_f_base, "                 whole:" <<  std::setprecision(10) << ostream->tstamp.twsec );
      LOG_DEBUG(  noise_source_f_base, "                  frac:" <<  std::setprecision(12) << ostream->tstamp.tfsec );
#endif
