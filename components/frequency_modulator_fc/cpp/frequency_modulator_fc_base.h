...
#ifndef FREQUENCY_MODULATOR_FC_IMPL_BASE_H
#define FREQUENCY_MODULATOR_FC_IMPL_BASE_H
...
#include "frequency_modulator_fc_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class frequency_modulator_fc_base;
...
class frequency_modulator_fc_base : public GnuHawkBlock
{
    public:
        frequency_modulator_fc_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<frequency_modulator_fc_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( frequency_modulator_fc_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( frequency_modulator_fc_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( frequency_modulator_fc_base, " serviceFunction: retval:" << retval);
