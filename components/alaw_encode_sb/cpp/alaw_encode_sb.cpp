...
alaw_encode_sb_i::alaw_encode_sb_i(const char *uuid, const char *label) :
alaw_encode_sb_base(uuid, label)
{
}

alaw_encode_sb_i::~alaw_encode_sb_i()
{
}
...
void alaw_encode_sb_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = vocoder_make_alaw_encode_sb();
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI alaw_encode_sb_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
