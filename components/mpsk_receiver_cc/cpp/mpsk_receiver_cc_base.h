...
#ifndef MPSK_RECEIVER_CC_IMPL_BASE_H
#define MPSK_RECEIVER_CC_IMPL_BASE_H
...
#include "mpsk_receiver_cc_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class mpsk_receiver_cc_base;
...
class mpsk_receiver_cc_base : public GnuHawkBlock
{
    public:
        mpsk_receiver_cc_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<mpsk_receiver_cc_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( mpsk_receiver_cc_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( mpsk_receiver_cc_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( mpsk_receiver_cc_base, " serviceFunction: retval:" << retval);
