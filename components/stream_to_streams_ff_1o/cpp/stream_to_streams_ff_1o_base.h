...
#ifndef STREAM_TO_STREAMS_FF_1O_IMPL_BASE_H
#define STREAM_TO_STREAMS_FF_1O_IMPL_BASE_H
...
#include "stream_to_streams_ff_1o_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class stream_to_streams_ff_1o_base;
...
class stream_to_streams_ff_1o_base : public GnuHawkBlock
{
    public:
        stream_to_streams_ff_1o_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<stream_to_streams_ff_1o_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( stream_to_streams_ff_1o_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( stream_to_streams_ff_1o_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( stream_to_streams_ff_1o_base, " serviceFunction: retval:" << retval);
