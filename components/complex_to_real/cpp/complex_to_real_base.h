...
#ifndef COMPLEX_TO_REAL_IMPL_BASE_H
#define COMPLEX_TO_REAL_IMPL_BASE_H
...
#include "complex_to_real_GnuHawkBlock.h"
#include "gr_malloc.h"

class complex_to_real_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  complex_to_real_base;
  ...
  SRIListener( complex_to_real_base &parent ) : _parent(parent) {};

    complex_to_real_base &_parent;
  };
    

    public:

        complex_to_real_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<complex_to_real_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( complex_to_real_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( complex_to_real_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( complex_to_real_base, " serviceFunction: retval:" << retval);
