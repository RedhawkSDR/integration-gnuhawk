...
and_const_ss_i::and_const_ss_i(const char *uuid, const char *label) :
and_const_ss_base(uuid, label)
{
}

and_const_ss_i::~and_const_ss_i()
{
}
...
void and_const_ss_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_and_const_ss( this->k );
    } catch (...) {
        return;
    }

    this->registerGetterSetter("k", &gr_and_const_ss::k, &gr_and_const_ss::set_k);
...
//      
BULKIO::StreamSRI and_const_ss_i::createOutputSRI( int32_t idx)
{
    //
