...
#ifndef AND_CONST_SS_IMPL_BASE_H
#define AND_CONST_SS_IMPL_BASE_H
...
#include "and_const_ss_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class and_const_ss_base;
...
class and_const_ss_base : public GnuHawkBlock
{
    public:
        and_const_ss_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<and_const_ss_base> *serviceThread; 
        ...
        // Ports
        bulkio::InLongPort *short_in;
        bulkio::OutLongPort *short_out;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( and_const_ss_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( and_const_ss_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( and_const_ss_base, " serviceFunction: retval:" << retval);
