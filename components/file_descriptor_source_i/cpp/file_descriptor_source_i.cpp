...
#include "file_descriptor_source_i.h"
...
file_descriptor_source_i_i::file_descriptor_source_i_i(const char *uuid, const char *label) :
file_descriptor_source_i_base(uuid, label)
{
}

file_descriptor_source_i_i::~file_descriptor_source_i_i()
{
}
...
void file_descriptor_source_i_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	try {
		gr_sptr = gr_make_file_descriptor_source (this->itemsize, this->fd, this->repeat);
	}
	catch(...) {
		return;
	}
...
//      
BULKIO::StreamSRI file_descriptor_source_i_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
