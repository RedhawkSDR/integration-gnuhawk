...
#ifndef MUTE_FF_IMPL_BASE_H
#define MUTE_FF_IMPL_BASE_H
...
#include "mute_ff_GnuHawkBlock.h"
...
class mute_ff_base : public GnuHawkBlock
{
    public:
        mute_ff_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<mute_ff_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( mute_ff_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( mute_ff_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( mute_ff_base, " serviceFunction: retval:" << retval);
