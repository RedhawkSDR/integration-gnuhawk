...
glfsr_source_b_i::glfsr_source_b_i(const char *uuid, const char *label) :
glfsr_source_b_base(uuid, label)
{
}

glfsr_source_b_i::~glfsr_source_b_i()
{
}
...
void glfsr_source_b_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = digital_make_glfsr_source_b(this->degree, this->repeat, this->mask, this->seed);
    } catch (...) {
        return;
    }

    this->registerGetter("mask", &digital_glfsr_source_b::mask);
    this->registerGetter("period", &digital_glfsr_source_b::period);
...
//      
BULKIO::StreamSRI glfsr_source_b_i::createOutputSRI( int32_t idx)
{
    //
