...
#ifndef VITERBI_DECODER_IMPL_BASE_H
#define VITERBI_DECODER_IMPL_BASE_H
...
#include "viterbi_decoder_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class viterbi_decoder_base;
...
class viterbi_decoder_base : public GnuHawkBlock
{
    public:
        viterbi_decoder_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<viterbi_decoder_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Ports
        bulkio::InOctetPort *atsc_soft_data_segment_in;
        bulkio::OutOctetPort *atsc_mpeg_packet_rs_encoded_out;
    ...
    protected:
        ...
        void  atsc_soft_data_segment_in_newStreamCallback( BULKIO::StreamSRI &sri );
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( viterbi_decoder_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( viterbi_decoder_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( viterbi_decoder_base, " serviceFunction: retval:" << retval);
