...
#ifndef NOT_BB_IMPL_BASE_H
#define NOT_BB_IMPL_BASE_H
...
class not_bb_base : public GnuHawkBlock
{
    public:
        not_bb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<not_bb_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( not_bb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( not_bb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( not_bb_base, " serviceFunction: retval:" << retval);
