...
pfb_interpolator_ccf_i::pfb_interpolator_ccf_i(const char *uuid, const char *label) :
pfb_interpolator_ccf_base(uuid, label)
{
}

pfb_interpolator_ccf_i::~pfb_interpolator_ccf_i()
{
}
...
void pfb_interpolator_ccf_i::createBlock()
{
  if ( taps.size() < 1 ) {
    return;
  }
  ...
  //
  try {
    gr_sptr = gr::filter::pfb_interpolator_ccf::make( this->interp, this->taps );
  }
  catch(...) {
    return;
  }

  this->registerGetter("taps",
		       &gr::filter::pfb_interpolator_ccf::taps);

  this->registerSetter("taps",
		       &gr::filter::pfb_interpolator_ccf::set_taps);
...
//      
BULKIO::StreamSRI pfb_interpolator_ccf_i::createOutputSRI( int32_t idx)
{
    //
