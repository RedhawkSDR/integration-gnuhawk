...
#ifndef PAD_IMPL_BASE_H
#define PAD_IMPL_BASE_H
...
class pad_base : public GnuHawkBlock
{
    public:
        pad_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<pad_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( pad_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( pad_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( pad_base, " serviceFunction: retval:" << retval);
