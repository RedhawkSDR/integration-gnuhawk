...
#ifndef QUADRATURE_DEMOD_CF_IMPL_BASE_H
#define QUADRATURE_DEMOD_CF_IMPL_BASE_H
...
#include "quadrature_demod_cf_GnuHawkBlock.h"


class quadrature_demod_cf_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  quadrature_demod_cf_base;
  ...
  SRIListener( quadrature_demod_cf_base &parent ) : _parent(parent) {};

    quadrature_demod_cf_base &_parent;
  };
    

    public:

        quadrature_demod_cf_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<quadrature_demod_cf_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( quadrature_demod_cf_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( quadrature_demod_cf_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( quadrature_demod_cf_base, " serviceFunction: retval:" << retval);
