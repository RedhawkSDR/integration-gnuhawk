...
multiply_const_cc_i::multiply_const_cc_i(const char *uuid, const char *label) :
multiply_const_cc_base(uuid, label)
{
}

multiply_const_cc_i::~multiply_const_cc_i()
{
}
...
void multiply_const_cc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_multiply_const_cc( this->k );

    this->registerGetterSetter("k",
                               &gr_multiply_const_cc::k,
                               &gr_multiply_const_cc::set_k);
...
//      
BULKIO::StreamSRI multiply_const_cc_i::createOutputSRI( int32_t idx)
{
    //
