...
#ifndef AND_BB_2I_IMPL_BASE_H
#define AND_BB_2I_IMPL_BASE_H
...
#include "and_bb_2i_GnuHawkBlock.h"


class and_bb_2i_base : public GnuHawkBlock
{
  ...
  SRIListener( and_bb_2i_base &parent ) : _parent(parent) {};

    and_bb_2i_base &_parent;
  };
    

    public:

        and_bb_2i_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataOctet_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataOctet_Out_i >::List   OStreamList;  
   ...
   ProcessThread<and_bb_2i_base> *serviceThread; 
  ...
  // Ports
  BULKIO_dataOctet_In_i *data_in_0;
  BULKIO_dataOctet_In_i *data_in_1;
  BULKIO_dataOctet_Out_i *data_out;
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( and_bb_2i_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( and_bb_2i_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( and_bb_2i_base, " serviceFunction: retval:" << retval);
