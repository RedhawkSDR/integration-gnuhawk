...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int and_bb_2i_base::_transformerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int and_bb_2i_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
                                 typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
    typedef typename std::vector< gr_istream< IN_PORT_TYPE > >   _IStreamList;
    typedef typename std::vector< gr_ostream< OUT_PORT_TYPE > >  _OStreamList;
...
// use incoming packet's time stamp to forward
      ...
      LOG_DEBUG( and_bb_2i_base, "                  mode:" <<  pkt->T.tcmode );
      LOG_DEBUG( and_bb_2i_base, "                status:" <<  pkt->T.tcstatus );
      LOG_DEBUG( and_bb_2i_base, "                offset:" <<  pkt->T.toff );
      LOG_DEBUG( and_bb_2i_base, "                 whole:" <<  std::setprecision(10) << pkt->T.twsec );
      LOG_DEBUG( and_bb_2i_base, "PKT - TS          frac:" <<  std::setprecision(12) << pkt->T.tfsec );
#endif
