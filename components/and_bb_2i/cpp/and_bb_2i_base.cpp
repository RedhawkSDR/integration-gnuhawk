...
/*******************************************************************************************
    ...
    AUTO-GENERATED CODE. DO NOT MODIFY
 	...
 	Source: and_bb_2i.spd.xml
 	Generated on: Thu Feb 28 17:38:38 EST 2013
 	Redhawk IDE
 	Version:M.1.8.3
...
void and_bb_2i_base::construct()
{
    ...
    data_in_0 = new BULKIO_dataOctet_In_i("data_in_0",&_sriListener );
    oid = ossie::corba::RootPOA()->activate_object(data_in_0);
    data_in_1 = new BULKIO_dataOctet_In_i("data_in_1",&_sriListener );
    oid = ossie::corba::RootPOA()->activate_object(data_in_1);
    data_out = new BULKIO_dataOctet_Out_i("data_out", this);
...
void and_bb_2i_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
    ...
    LOG_TRACE( and_bb_2i_base, "COMPLETED RELEASE OBJECT" );
}
...
void and_bb_2i_base::notifySRI( BULKIO_dataOctet_In_i *port, BULKIO::StreamSRI &sri ) {

  LOG_TRACE( and_bb_2i_base, "START NotifySRI  port:stream " << port->getName() << "/" << sri.streamID);
  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( port, sri ) );
  LOG_TRACE( and_bb_2i_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << port->getName() << "/" << sri.streamID); 
...
BULKIO::StreamSRI and_bb_2i_base::createOutputSRI( int32_t idx)
{
  ...
  while ( nout > -1 && !exitServiceFunction && serviceThread->threadRunning() ) {
	...
	if ( _input_ready[idx] ) {
	  ...
	  if ( nitems > istream->nitems() ) {
	       LOG_WARN( and_bb_2i_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE " << nitems << "/" << istream->nitems() );
											...
											typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
  ...
  if ( !eos ) {
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
      LOG_TRACE(  and_bb_2i_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << istream->nitems() << "/" << istream->_data.size() );
    ...
    if ( !gr_sptr->fixed_rate() )  {
      ...
      LOG_TRACE( and_bb_2i_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << noutput_items  << "/" << gr_sptr->output_multiple());
    }
	...
	LOG_TRACE( and_bb_2i_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << "/" << istream->nitems() << "/" << _ninput_items_required[idx] << "/" << _input_ready[idx]);
      }
    ...
    if ( !dataReady ) {
      LOG_TRACE( and_bb_2i_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << "/" << _ninput_items_required[0] );
      return -1;	 
    }
    ...
    if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
      LOG_TRACE( and_bb_2i_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems << " ISTREAMS:" << _input_items.size() << " OSTREAMS:" << _output_items.size());
