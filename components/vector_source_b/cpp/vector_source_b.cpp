...
vector_source_b_i::vector_source_b_i(const char *uuid, const char *label) :
vector_source_b_base(uuid, label)
{
}

vector_source_b_i::~vector_source_b_i()
{
}

void vector_source_b_i::changedSamplingFreq(const std::string &id)
{
...
//
void vector_source_b_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_vector_source_b(data, repeat, vlen);
  ...
  // maintainTimestamp( bool onoff );
   ...
   setupIOMappings();
   this->registerSetter("data", &gr_vector_source_b::set_data);
   this->setPropertyChangeListener("rewind", this, &vector_source_b_i::onRewind);
   this->setPropertyChangeListener("sampling_freq", this, &vector_source_b_i::changedSamplingFreq);
} 
...
//      
BULKIO::StreamSRI vector_source_b_i::createOutputSRI( int32_t idx)
{
  //
...
void vector_source_b_i::onRewind (const std::string&)
{
