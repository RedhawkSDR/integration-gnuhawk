...
slicer_fb_i::slicer_fb_i(const char *uuid, const char *label) :
slicer_fb_base(uuid, label)
{
}

slicer_fb_i::~slicer_fb_i()
{
}
...
void slicer_fb_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	try{
        gr_sptr = pager_make_slicer_fb (this->alpha);
	}
	catch(...){
		return;
	}
    this->registerGetter("avg", &pager_slicer_fb::dc_offset);
    // 
...
//      
BULKIO::StreamSRI slicer_fb_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
