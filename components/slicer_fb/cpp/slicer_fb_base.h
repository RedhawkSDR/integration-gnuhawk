...
#ifndef SLICER_FB_IMPL_BASE_H
#define SLICER_FB_IMPL_BASE_H
...
#include "slicer_fb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class slicer_fb_base;
...
class slicer_fb_base : public GnuHawkBlock
{
    public:
        slicer_fb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<slicer_fb_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( slicer_fb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( slicer_fb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( slicer_fb_base, " serviceFunction: retval:" << retval);
