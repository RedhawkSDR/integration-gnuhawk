...
#ifndef IIR_FILTER_FFD_IMPL_BASE_H
#define IIR_FILTER_FFD_IMPL_BASE_H
...
#include "iir_filter_ffd_GnuHawkBlock.h"


class iir_filter_ffd_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  iir_filter_ffd_base;
  ...
  SRIListener( iir_filter_ffd_base &parent ) : _parent(parent) {};

    iir_filter_ffd_base &_parent;
  };
    

    public:

        iir_filter_ffd_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<iir_filter_ffd_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( iir_filter_ffd_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( iir_filter_ffd_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( iir_filter_ffd_base, " serviceFunction: retval:" << retval);
