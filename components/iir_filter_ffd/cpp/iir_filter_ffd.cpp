...
iir_filter_ffd_i::iir_filter_ffd_i(const char *uuid, const char *label) :
iir_filter_ffd_base(uuid, label)
{
}

iir_filter_ffd_i::~iir_filter_ffd_i()
{
}

void iir_filter_ffd_i::setTaps(const std::string &id){
    gr_sptr->set_taps(this->fftaps, this->fbtaps);
}
...
//      
BULKIO::StreamSRI iir_filter_ffd_i::createOutputSRI( int32_t idx)
{
  //
