...
#ifndef SKIPHEAD_II_IMPL_BASE_H
#define SKIPHEAD_II_IMPL_BASE_H
...
#include "skiphead_ii_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class skiphead_ii_base;
...
class skiphead_ii_base : public GnuHawkBlock
{
    public:
        skiphead_ii_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<skiphead_ii_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::Long itemsize;
        CORBA::ULongLong nitems_to_skip;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( skiphead_ii_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( skiphead_ii_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( skiphead_ii_base, " serviceFunction: retval:" << retval);
