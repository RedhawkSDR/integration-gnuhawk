...
#ifndef CHUNKS_TO_SYMBOLS_SF_IMPL_BASE_H
#define CHUNKS_TO_SYMBOLS_SF_IMPL_BASE_H
...
#include "chunks_to_symbols_sf_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class chunks_to_symbols_sf_base;
...
class chunks_to_symbols_sf_base : public GnuHawkBlock
{
    public:
        chunks_to_symbols_sf_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<chunks_to_symbols_sf_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( chunks_to_symbols_sf_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( chunks_to_symbols_sf_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( chunks_to_symbols_sf_base, " serviceFunction: retval:" << retval);
