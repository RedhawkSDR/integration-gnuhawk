...
#ifndef ADD_CONST_SF_IMPL_BASE_H
#define ADD_CONST_SF_IMPL_BASE_H
...
#include "add_const_sf_GnuHawkBlock.h"


class add_const_sf_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataShort_In_i::SRIListener {
    ...
    friend class  add_const_sf_base;
  ...
  SRIListener( add_const_sf_base &parent ) : _parent(parent) {};

    add_const_sf_base &_parent;
  };
    

    public:

        add_const_sf_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataShort_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<add_const_sf_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( add_const_sf_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( add_const_sf_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( add_const_sf_base, " serviceFunction: retval:" << retval);
