...
add_const_sf_i::add_const_sf_i(const char *uuid, const char *label) :
add_const_sf_base(uuid, label)
{
}

add_const_sf_i::~add_const_sf_i()
{
}
...
//
void add_const_sf_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_add_const_sf(k);
  ...
  setupIOMappings();
  this->registerGetterSetter("k", &gr_add_const_sf::k, &gr_add_const_sf::set_k);
} 
...
//      
BULKIO::StreamSRI add_const_sf_i::createOutputSRI( int32_t idx)
{
  //
