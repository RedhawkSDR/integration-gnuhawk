...
wavelet_ff_i::wavelet_ff_i(const char *uuid, const char *label) :
wavelet_ff_base(uuid, label)
{
}

wavelet_ff_i::~wavelet_ff_i()
{
}
...
void wavelet_ff_i::createBlock()
{
    ...
    try {
        gr_sptr = wavelet_make_wavelet_ff( this->size, this->order, this->forward );
    } catch (...) {
    	return;
    }
...
//      
BULKIO::StreamSRI wavelet_ff_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
