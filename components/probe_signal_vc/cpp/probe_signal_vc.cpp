...
probe_signal_vc_i::probe_signal_vc_i(const char *uuid, const char *label) :
probe_signal_vc_base(uuid, label)
{
}

probe_signal_vc_i::~probe_signal_vc_i()
{
}
...
void probe_signal_vc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_probe_signal_vc( this->size );

    this->registerGetter("level",
                         &gr_probe_signal_vc::level);
