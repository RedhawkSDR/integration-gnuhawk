...
#ifndef MULTIPLY_CONST_VCC_IMPL_BASE_H
#define MULTIPLY_CONST_VCC_IMPL_BASE_H
...
#include "multiply_const_vcc_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class multiply_const_vcc_base;
...
class multiply_const_vcc_base : public GnuHawkBlock
{
    public:
        multiply_const_vcc_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<multiply_const_vcc_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        std::vector<std::complex<float> > k;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( multiply_const_vcc_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( multiply_const_vcc_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( multiply_const_vcc_base, " serviceFunction: retval:" << retval);
