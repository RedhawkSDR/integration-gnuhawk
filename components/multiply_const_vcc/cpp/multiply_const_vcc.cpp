...
multiply_const_vcc_i::multiply_const_vcc_i(const char *uuid, const char *label) :
multiply_const_vcc_base(uuid, label)
{
}

multiply_const_vcc_i::~multiply_const_vcc_i()
{
}
...
void multiply_const_vcc_i::createBlock()
{
  //
  ...
  try {
    gr_sptr = gr_make_multiply_const_vcc( k );
  }
  catch(...) {
    return;
  }
  ...
  this->registerGetterSetter("k",
			     &gr_multiply_const_vcc::k,
			     &gr_multiply_const_vcc::set_k);
...
//      
BULKIO::StreamSRI multiply_const_vcc_i::createOutputSRI( int32_t idx)
{
    //
