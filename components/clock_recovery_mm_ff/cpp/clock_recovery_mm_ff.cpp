...
#include "clock_recovery_mm_ff.h"
...
clock_recovery_mm_ff_i::clock_recovery_mm_ff_i(const char *uuid, const char *label) :
clock_recovery_mm_ff_base(uuid, label)
{
}

clock_recovery_mm_ff_i::~clock_recovery_mm_ff_i()
{
}
...
void clock_recovery_mm_ff_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = digital_make_clock_recovery_mm_ff( this->omega, this->gain_omega, this->mu, this->gain_mu, this->omega_relative_limit );
    } catch (...) {
        return;
    }

    this->registerGetterSetter("omega",
            &digital_clock_recovery_mm_ff::omega,
            &digital_clock_recovery_mm_ff::set_omega);

    this->registerGetterSetter("gain_omega",
            &digital_clock_recovery_mm_ff::gain_omega,
            &digital_clock_recovery_mm_ff::set_gain_omega);

    this->registerGetterSetter("mu",
            &digital_clock_recovery_mm_ff::mu,
            &digital_clock_recovery_mm_ff::set_mu);

    this->registerGetterSetter("gain_mu",
            &digital_clock_recovery_mm_ff::gain_mu,
            &digital_clock_recovery_mm_ff::set_gain_mu);
...
//      
BULKIO::StreamSRI clock_recovery_mm_ff_i::createOutputSRI( int32_t idx)
{
    //
