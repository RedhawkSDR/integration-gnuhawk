...
#ifndef HRPT_DECODER_IMPL_BASE_H
#define HRPT_DECODER_IMPL_BASE_H
...
#include "hrpt_decoder_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class hrpt_decoder_base;
...
class hrpt_decoder_base : public GnuHawkBlock
{
    public:
        hrpt_decoder_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<hrpt_decoder_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( hrpt_decoder_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( hrpt_decoder_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( hrpt_decoder_base, " serviceFunction: retval:" << retval);
