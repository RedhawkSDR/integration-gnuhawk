...
threshold_ff_i::threshold_ff_i(const char *uuid, const char *label) :
threshold_ff_base(uuid, label)
{
}

threshold_ff_i::~threshold_ff_i()
{
}
...
void threshold_ff_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_threshold_ff( this->lo, this->hi, this->last_state );
    } catch (...) {
        return;
    }

    this->registerGetterSetter("lo", &gr_threshold_ff::lo, &gr_threshold_ff::set_lo);
    this->registerGetterSetter("hi", &gr_threshold_ff::hi, &gr_threshold_ff::set_hi);
    this->registerGetterSetter("last_state", &gr_threshold_ff::last_state, &gr_threshold_ff::set_last_state);
...
//      
BULKIO::StreamSRI threshold_ff_i::createOutputSRI( int32_t idx)
{
    //
