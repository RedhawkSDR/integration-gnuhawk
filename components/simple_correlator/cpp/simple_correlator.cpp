...
simple_correlator_i::simple_correlator_i(const char *uuid, const char *label) :
simple_correlator_base(uuid, label)
{
}

simple_correlator_i::~simple_correlator_i()
{
}
...
void simple_correlator_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_simple_correlator( this->payload_bytesize );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI simple_correlator_i::createOutputSRI( int32_t idx)
{
    //
