...
#ifndef RS_DECODER_IMPL_BASE_H
#define RS_DECODER_IMPL_BASE_H
...
#include "rs_decoder_GnuHawkBlock.h"
...
class rs_decoder_base : public GnuHawkBlock
{
    public:
        rs_decoder_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<rs_decoder_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Ports
        bulkio::InOctetPort *atsc_mpeg_packet_rs_encoded_in;
        bulkio::OutOctetPort *atsc_mpeg_packet_no_sync_out;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( rs_decoder_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( rs_decoder_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( rs_decoder_base, " serviceFunction: retval:" << retval);
