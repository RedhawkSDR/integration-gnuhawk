...
stretch_ff_i::stretch_ff_i(const char *uuid, const char *label) :
stretch_ff_base(uuid, label)
{
}

stretch_ff_i::~stretch_ff_i()
{
}
...
void stretch_ff_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_stretch_ff( this->lo, this->vlen );

    this->registerGetterSetter("lo",
                               &gr_stretch_ff::lo,
                               &gr_stretch_ff::set_lo);
    this->registerGetter("vlen",
                         &gr_stretch_ff::vlen);
...
//      
BULKIO::StreamSRI stretch_ff_i::createOutputSRI( int32_t idx)
{
    //
