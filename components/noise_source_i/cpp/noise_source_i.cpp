...
noise_source_i_i::noise_source_i_i(const char *uuid, const char *label) :
noise_source_i_base(uuid, label)
{
}

noise_source_i_i::~noise_source_i_i()
{
}
...
//
void noise_source_i_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_noise_source_i((gr_noise_type_t)type, amplitude, seed);
  this->registerGetterSetter("type", &gr_noise_source_i::type, &gr_noise_source_i::set_type);
  this->registerGetterSetter("amplitude", &gr_noise_source_i::amplitude, &gr_noise_source_i::set_amplitude);
  this->setPropertyChangeListener("sampling_freq", this, &noise_source_i_i::changedSamplingFreq);
...
//      
BULKIO::StreamSRI noise_source_i_i::createOutputSRI( int32_t idx)
{
  //
...
void noise_source_i_i::changedSamplingFreq(const std::string &id)
{
