...
#ifndef NOISE_SOURCE_I_IMPL_BASE_H
#define NOISE_SOURCE_I_IMPL_BASE_H
...
#include "noise_source_i_GnuHawkBlock.h"


class noise_source_i_base : public GnuHawkBlock
{
    ...
    public:
        ...
        noise_source_i_base(const char *uuid, const char *label);
 ...
 template < typename OUT_PORT_TYPE > struct gr_ostream {
  ...
  typedef _OStream< BULKIO_dataLong_Out_i >::List   OStreamList;  
   ...
   ProcessThread<noise_source_i_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( noise_source_i_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( noise_source_i_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( noise_source_i_base, " serviceFunction: retval:" << retval);
