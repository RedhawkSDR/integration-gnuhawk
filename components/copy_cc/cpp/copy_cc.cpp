...
copy_cc_i::copy_cc_i(const char *uuid, const char *label) :
copy_cc_base(uuid, label)
{
}
...
void copy_cc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	gr_sptr = gr_make_copy (this->itemsize);
	this->registerGetterSetter("enable", &gr_copy::enabled,
		                                   &gr_copy::set_enabled);
...
//      
BULKIO::StreamSRI copy_cc_i::createOutputSRI( int32_t idx)
{
    //
