...
#ifndef DEINTERLEAVE_CC_1O_IMPL_BASE_H
#define DEINTERLEAVE_CC_1O_IMPL_BASE_H
...
#include "deinterleave_cc_1o_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class deinterleave_cc_1o_base;
...
class deinterleave_cc_1o_base : public GnuHawkBlock
{
    public:
        deinterleave_cc_1o_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<deinterleave_cc_1o_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( deinterleave_cc_1o_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( deinterleave_cc_1o_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( deinterleave_cc_1o_base, " serviceFunction: retval:" << retval);
