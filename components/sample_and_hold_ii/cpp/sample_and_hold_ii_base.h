...
#ifndef SAMPLE_AND_HOLD_II_IMPL_BASE_H
#define SAMPLE_AND_HOLD_II_IMPL_BASE_H
...
#include "sample_and_hold_ii_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class sample_and_hold_ii_base;
...
class sample_and_hold_ii_base : public GnuHawkBlock
{
    public:
        sample_and_hold_ii_base(const char *uuid, const char *label);
        ...
        ~sample_and_hold_ii_base();

        void loadProperties();

    protected:
        ProcessThread<sample_and_hold_ii_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Ports
        bulkio::InLongPort *long_in;
        bulkio::InOctetPort *ctrl_in;
    ...
    protected:
        ...
        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  ctrl_in_newStreamCallback( BULKIO::StreamSRI &sri );
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( sample_and_hold_ii_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( sample_and_hold_ii_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( sample_and_hold_ii_base, " serviceFunction: retval:" << retval);
