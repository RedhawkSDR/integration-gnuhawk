...
fmdet_cf_i::fmdet_cf_i(const char *uuid, const char *label) :
fmdet_cf_base(uuid, label)
{
}

fmdet_cf_i::~fmdet_cf_i()
{
}
...
void fmdet_cf_i::createBlock()
{
    ...
    try {
        gr_sptr = gr_make_fmdet_cf( this->samplerate, this->freq_low, this->freq_high, this->scl );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI fmdet_cf_i::createOutputSRI( int32_t idx)
{
    //
