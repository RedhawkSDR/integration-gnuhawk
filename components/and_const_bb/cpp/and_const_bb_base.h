...
#ifndef AND_CONST_BB_IMPL_BASE_H
#define AND_CONST_BB_IMPL_BASE_H
...
#include "and_const_bb_GnuHawkBlock.h"


class and_const_bb_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataOctet_In_i::SRIListener {
    ...
    friend class  and_const_bb_base;
  ...
  SRIListener( and_const_bb_base &parent ) : _parent(parent) {};

    and_const_bb_base &_parent;
  };
    

    public:

        and_const_bb_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataOctet_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataOctet_Out_i >::List   OStreamList;  
   ...
   ProcessThread<and_const_bb_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( and_const_bb_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( and_const_bb_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( and_const_bb_base, " serviceFunction: retval:" << retval);
