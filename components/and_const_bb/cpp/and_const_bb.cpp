...
and_const_bb_i::and_const_bb_i(const char *uuid, const char *label) :
and_const_bb_base(uuid, label)
{
}

and_const_bb_i::~and_const_bb_i()
{
}
...
//
void and_const_bb_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_and_const_bb(k);
  ...
  setupIOMappings();
  this->registerGetterSetter("k", &gr_and_const_bb::k, &gr_and_const_bb::set_k);
} 
...
//      
BULKIO::StreamSRI and_const_bb_i::createOutputSRI( int32_t idx)
{
  //
