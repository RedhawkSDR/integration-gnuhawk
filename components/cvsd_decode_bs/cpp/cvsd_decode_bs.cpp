...
cvsd_decode_bs_i::cvsd_decode_bs_i(const char *uuid, const char *label) :
cvsd_decode_bs_base(uuid, label)
{
}

cvsd_decode_bs_i::~cvsd_decode_bs_i()
{
}

short cvsd_decode_bs_i::get_min_step()
{
    return gr_sptr->min_step();
}

short cvsd_decode_bs_i::get_max_step()
{
    return gr_sptr->max_step();
}

double cvsd_decode_bs_i::get_step_decay()
{
    return gr_sptr->step_decay();
}

double cvsd_decode_bs_i::get_accum_decay()
{
    return gr_sptr->accum_decay();
}

int cvsd_decode_bs_i::get_K()
{
    return gr_sptr->K();
}

int cvsd_decode_bs_i::get_J()
{
    return gr_sptr->J();
}

short cvsd_decode_bs_i::get_pos_accum_max()
{
    return gr_sptr->pos_accum_max();
}

short cvsd_decode_bs_i::get_neg_accum_max()
{
    return gr_sptr->neg_accum_max();
}
...
void cvsd_decode_bs_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = vocoder_make_cvsd_decode_bs( min_step, max_step, step_decay, accum_decay, K, J, pos_accum_max, neg_accum_max);
    } catch (...) {
        return;
    }

    this->registerGetValue("min_step", this, &cvsd_decode_bs_i::get_min_step);
    this->registerGetValue("max_step", this, &cvsd_decode_bs_i::get_max_step);
    this->registerGetValue("step_decay", this, &cvsd_decode_bs_i::get_step_decay);
    this->registerGetValue("accum_decay", this, &cvsd_decode_bs_i::get_accum_decay);
    this->registerGetValue("K", this, &cvsd_decode_bs_i::get_K);
    this->registerGetValue("J", this, &cvsd_decode_bs_i::get_J);
    this->registerGetValue("pos_accum_max", this, &cvsd_decode_bs_i::get_pos_accum_max);
    this->registerGetValue("neg_accum_max", this, &cvsd_decode_bs_i::get_neg_accum_max);
...
//      
BULKIO::StreamSRI cvsd_decode_bs_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
