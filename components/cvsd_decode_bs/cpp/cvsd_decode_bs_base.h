...
#ifndef CVSD_DECODE_BS_IMPL_BASE_H
#define CVSD_DECODE_BS_IMPL_BASE_H
...
#include "cvsd_decode_bs_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class cvsd_decode_bs_base;
...
class cvsd_decode_bs_base : public GnuHawkBlock
{
    public:
        cvsd_decode_bs_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<cvsd_decode_bs_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( cvsd_decode_bs_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( cvsd_decode_bs_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( cvsd_decode_bs_base, " serviceFunction: retval:" << retval);
