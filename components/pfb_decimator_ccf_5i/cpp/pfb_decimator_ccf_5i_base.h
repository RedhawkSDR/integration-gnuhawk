...
#ifndef PFB_DECIMATOR_CCF_5I_IMPL_BASE_H
#define PFB_DECIMATOR_CCF_5I_IMPL_BASE_H
...
#include "pfb_decimator_ccf_5i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class pfb_decimator_ccf_5i_base;
...
class pfb_decimator_ccf_5i_base : public GnuHawkBlock
{
    public:
        pfb_decimator_ccf_5i_base(const char *uuid, const char *label);
        ...
        ~pfb_decimator_ccf_5i_base();

        void loadProperties();

    protected:
        ProcessThread<pfb_decimator_ccf_5i_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( pfb_decimator_ccf_5i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( pfb_decimator_ccf_5i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( pfb_decimator_ccf_5i_base, " serviceFunction: retval:" << retval);
