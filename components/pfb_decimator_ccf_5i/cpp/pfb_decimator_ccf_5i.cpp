...
#include "pfb_decimator_ccf_5i.h"
...
pfb_decimator_ccf_5i_i::pfb_decimator_ccf_5i_i(const char *uuid, const char *label) :
pfb_decimator_ccf_5i_base(uuid, label)
{
}

pfb_decimator_ccf_5i_i::~pfb_decimator_ccf_5i_i()
{
}
...
void pfb_decimator_ccf_5i_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr::filter::pfb_decimator_ccf::make( decim, taps, channel );
    } catch (... ) {
        return;
    }

    this->registerSetter("taps", &gr::filter::pfb_decimator_ccf::set_taps);
...
//      
BULKIO::StreamSRI pfb_decimator_ccf_5i_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
