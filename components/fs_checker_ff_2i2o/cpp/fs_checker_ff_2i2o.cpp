...
fs_checker_ff_2i2o_i::fs_checker_ff_2i2o_i(const char *uuid, const char *label) :
fs_checker_ff_2i2o_base(uuid, label)
{
}

fs_checker_ff_2i2o_i::~fs_checker_ff_2i2o_i()
{
}

void fs_checker_ff_2i2o_i::reset_data(const std::string &id)
{
	if(this->reset == true){
		gr_sptr->reset();
	}
...
void fs_checker_ff_2i2o_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	try {
		gr_sptr = atsc_make_fs_checker();
	}
	catch (...) {
		return;
	}
	this->setPropertyChangeListener("reset", this, &fs_checker_ff_2i2o_i::reset_data);
    // 
...
//      
BULKIO::StreamSRI fs_checker_ff_2i2o_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
