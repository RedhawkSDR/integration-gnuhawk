...
#ifndef RS_ENCODER_IMPL_BASE_H
#define RS_ENCODER_IMPL_BASE_H
...
#include "rs_encoder_GnuHawkBlock.h"
...
class rs_encoder_base : public GnuHawkBlock
{
    public:
        rs_encoder_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<rs_encoder_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( rs_encoder_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( rs_encoder_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( rs_encoder_base, " serviceFunction: retval:" << retval);
