...
keep_m_in_n_ss_i::keep_m_in_n_ss_i(const char *uuid, const char *label) :
keep_m_in_n_ss_base(uuid, label)
{
}

keep_m_in_n_ss_i::~keep_m_in_n_ss_i()
{
}
...
void keep_m_in_n_ss_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	gr_sptr = gr_make_keep_m_in_n (this->item_size, this->m, this->n, this->offset);
	this->registerSetter("offset", &gr_keep_m_in_n::set_offset);
	this->registerSetter("n", &gr_keep_m_in_n::set_n);
	this->registerSetter("m", &gr_keep_m_in_n::set_m);
...
//      
BULKIO::StreamSRI keep_m_in_n_ss_i::createOutputSRI( int32_t idx)
{
    //
