...
#ifndef INTERLEAVE_II_3I_IMPL_BASE_H
#define INTERLEAVE_II_3I_IMPL_BASE_H
...
#include "interleave_ii_3i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class interleave_ii_3i_base;
...
class interleave_ii_3i_base : public GnuHawkBlock
{
    public:
        interleave_ii_3i_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<interleave_ii_3i_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( interleave_ii_3i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( interleave_ii_3i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( interleave_ii_3i_base, " serviceFunction: retval:" << retval);
