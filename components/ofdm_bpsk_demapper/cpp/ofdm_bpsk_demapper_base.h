...
#ifndef OFDM_BPSK_DEMAPPER_IMPL_BASE_H
#define OFDM_BPSK_DEMAPPER_IMPL_BASE_H
...
#include "ofdm_bpsk_demapper_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class ofdm_bpsk_demapper_base;
...
class ofdm_bpsk_demapper_base : public GnuHawkBlock
{
    public:
        ofdm_bpsk_demapper_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<ofdm_bpsk_demapper_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( ofdm_bpsk_demapper_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( ofdm_bpsk_demapper_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( ofdm_bpsk_demapper_base, " serviceFunction: retval:" << retval);
