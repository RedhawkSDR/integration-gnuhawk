...
#ifndef VECTOR_SINK_F_IMPL_BASE_H
#define VECTOR_SINK_F_IMPL_BASE_H
...
#include "vector_sink_f_GnuHawkBlock.h"


class vector_sink_f_base : public GnuHawkBlock
{
    friend class BULKIO_dataFloat_In_i;

  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {

    friend class  vector_sink_f_base;
  ...
  SRIListener( vector_sink_f_base &parent ) : _parent(parent) {};

    vector_sink_f_base &_parent;
  };
    

    public:

        vector_sink_f_base(const char *uuid, const char *label);
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _IStream< BULKIO_dataFloat_In_i >::List    IStreamList;
   ...
   ProcessThread<vector_sink_f_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( vector_sink_f_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( vector_sink_f_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( vector_sink_f_base, " serviceFunction: retval:" << retval);
