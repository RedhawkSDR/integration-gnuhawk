...
vector_sink_f_i::vector_sink_f_i(const char *uuid, const char *label) :
vector_sink_f_base(uuid, label)
{
}

vector_sink_f_i::~vector_sink_f_i()
{
}
...
//
void vector_sink_f_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_vector_sink_f(vlen);
  ...
  setupIOMappings();
  this->registerGetter("data", &gr_vector_sink_f::data);
  this->setPropertyChangeListener("reset", this, &vector_sink_f_i::on_reset);
} 

void vector_sink_f_i::on_reset (const std::string&)
{
