...
head_bb_i::head_bb_i(const char *uuid, const char *label) :
head_bb_base(uuid, label)
{
}
...
void head_bb_i::reset_data(const std::string &id)
{
    if (this->reset == true) {
        gr_sptr->reset();
    }
...
void head_bb_i::createBlock()
{
    ...
    this->setPropertyChangeListener("reset", this, &head_bb_i::reset_data);
...
//      
BULKIO::StreamSRI head_bb_i::createOutputSRI( int32_t idx)
{
    //
