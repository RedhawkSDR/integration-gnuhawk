...
vector_source_c_i::vector_source_c_i(const char *uuid, const char *label) :
vector_source_c_base(uuid, label)
{
}

vector_source_c_i::~vector_source_c_i()
{
}

void vector_source_c_i::onRewind (const std::string&)
{
...
void vector_source_c_i::changedSamplingFreq(const std::string &id)
{
...
void vector_source_c_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_vector_source_c(this->data, this->repeat, this->vlen);
    } catch (...) {
        return;
    }

    this->registerSetter("data", &gr_vector_source_c::set_data);
    this->setPropertyChangeListener("rewind", this, &vector_source_c_i::onRewind);
    this->setPropertyChangeListener("sampling_freq", this, &vector_source_c_i::changedSamplingFreq);
...
//      
BULKIO::StreamSRI vector_source_c_i::createOutputSRI( int32_t idx)
{
    //
