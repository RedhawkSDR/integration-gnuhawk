...
#ifndef VECTOR_SOURCE_C_IMPL_BASE_H
#define VECTOR_SOURCE_C_IMPL_BASE_H
...
#include "vector_source_c_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class vector_source_c_base;
...
class vector_source_c_base : public GnuHawkBlock
{
    public:
        vector_source_c_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<vector_source_c_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( vector_source_c_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( vector_source_c_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( vector_source_c_base, " serviceFunction: retval:" << retval);
