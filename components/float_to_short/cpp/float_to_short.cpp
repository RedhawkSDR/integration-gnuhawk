...
float_to_short_i::float_to_short_i(const char *uuid, const char *label) :
float_to_short_base(uuid, label)
{
}

float_to_short_i::~float_to_short_i()
{
}
...
void float_to_short_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_float_to_short( this->vlen, this->scale );

    this->registerGetterSetter("scale",
                              &gr_float_to_short::scale,
                              &gr_float_to_short::set_scale);
...
//      
BULKIO::StreamSRI float_to_short_i::createOutputSRI( int32_t idx)
{
    //
