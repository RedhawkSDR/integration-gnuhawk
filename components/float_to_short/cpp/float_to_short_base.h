...
#ifndef FLOAT_TO_SHORT_IMPL_BASE_H
#define FLOAT_TO_SHORT_IMPL_BASE_H
...
#include "float_to_short_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class float_to_short_base;
...
class float_to_short_base : public GnuHawkBlock
{
    public:
        float_to_short_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<float_to_short_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( float_to_short_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( float_to_short_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( float_to_short_base, " serviceFunction: retval:" << retval);
