...
freq_xlating_fir_filter_scf_i::freq_xlating_fir_filter_scf_i(const char *uuid, const char *label) :
freq_xlating_fir_filter_scf_base(uuid, label)
{
}

freq_xlating_fir_filter_scf_i::~freq_xlating_fir_filter_scf_i()
{
}

void freq_xlating_fir_filter_scf_i::set_taps(const std::string &id)
{
    this->gr_sptr->set_taps(this->taps);
}

std::vector<float> freq_xlating_fir_filter_scf_i::get_taps()
{
...
void freq_xlating_fir_filter_scf_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr::filter::freq_xlating_fir_filter_scf::make( this->decimation, this->taps, this->center_freq, this->sampling_freq );

    this->registerGetterSetter("center_freq",
                               &gr::filter::freq_xlating_fir_filter_scf::center_freq,
                               &gr::filter::freq_xlating_fir_filter_scf::set_center_freq);

    this->setPropertyChangeListener("taps",
                                    this,
                                    &freq_xlating_fir_filter_scf_i::set_taps);
    this->registerGetValue("taps",
                           this,
                           &freq_xlating_fir_filter_scf_i::get_taps);
...
//      
BULKIO::StreamSRI freq_xlating_fir_filter_scf_i::createOutputSRI( int32_t idx)
{
    //
