...
#ifndef BURST_TAGGER_SS_IMPL_BASE_H
#define BURST_TAGGER_SS_IMPL_BASE_H
...
#include "burst_tagger_ss_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class burst_tagger_ss_base;
...
class burst_tagger_ss_base : public GnuHawkBlock
{
    public:
        burst_tagger_ss_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<burst_tagger_ss_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        unsigned char itemsize;
        true_tag_struct true_tag;
        false_tag_struct false_tag;

        // Ports
        bulkio::InShortPort *short_in;
        bulkio::InShortPort *trigger_in;
    ...
    protected:
        ...
        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  trigger_in_newStreamCallback( BULKIO::StreamSRI &sri );
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( burst_tagger_ss_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( burst_tagger_ss_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( burst_tagger_ss_base, " serviceFunction: retval:" << retval);
