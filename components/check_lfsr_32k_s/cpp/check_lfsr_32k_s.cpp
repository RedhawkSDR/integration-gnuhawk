...
check_lfsr_32k_s_i::check_lfsr_32k_s_i(const char *uuid, const char *label) :
check_lfsr_32k_s_base(uuid, label)
{
}

check_lfsr_32k_s_i::~check_lfsr_32k_s_i()
{
}
...
void check_lfsr_32k_s_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_check_lfsr_32k_s();
    } catch (...) {
        return;
    }

    this->registerGetter("ntotal", &gr_check_lfsr_32k_s::ntotal);
    this->registerGetter("nright", &gr_check_lfsr_32k_s::nright);
    this->registerGetter("runlength", &gr_check_lfsr_32k_s::runlength);
