...
sig_source_i_i::sig_source_i_i(const char *uuid, const char *label) :
sig_source_i_base(uuid, label)
{
}

sig_source_i_i::~sig_source_i_i()
{
}

void sig_source_i_i::changedSamplingFreq(const std::string &id)
{
...
//
void sig_source_i_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  this->gr_sptr = gr_make_sig_source_i(this->sampling_freq, (gr_waveform_t)this->waveform, this->frequency, this->amplitude, this->offset);
  this->setPropertyChangeListener("sampling_freq", this, &sig_source_i_i::changedSamplingFreq);
  this->registerGetter("sampling_freq", &gr_sig_source_i::sampling_freq);
  this->registerGetterSetter("waveform", &gr_sig_source_i::waveform, &gr_sig_source_i::set_waveform);
  this->registerGetterSetter("frequency", &gr_sig_source_i::frequency, &gr_sig_source_i::set_frequency);
  this->registerGetterSetter("amplitude", &gr_sig_source_i::amplitude, &gr_sig_source_i::set_amplitude);
  this->registerGetterSetter("offset", &gr_sig_source_i::offset, &gr_sig_source_i::set_offset);
...
//      
BULKIO::StreamSRI sig_source_i_i::createOutputSRI( int32_t idx)
{
  //
