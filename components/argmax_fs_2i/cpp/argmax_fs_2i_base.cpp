...
argmax_fs_2i_base::argmax_fs_2i_base(const char *uuid, const char *label) :
...
void argmax_fs_2i_base::construct()
{
    ...
    float_in_1->setNewStreamListener(this, &argmax_fs_2i_base::float_in_1_newStreamCallback);
    oid = ossie::corba::RootPOA()->activate_object(float_in_1);
    float_in_2 = new bulkio::InFloatPort("float_in_2");
    float_in_2->setNewStreamListener(this, &argmax_fs_2i_base::float_in_2_newStreamCallback);
...
*******************************************************************************************/
void argmax_fs_2i_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void argmax_fs_2i_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    boost::mutex::scoped_lock lock(serviceThreadLock);
    if (serviceThread == 0) {
        float_in_1->unblock();
        float_in_2->unblock();
        serviceThread = new ProcessThread<argmax_fs_2i_base>(this, 0.1);
        serviceThread->start();
    }
...
void argmax_fs_2i_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( argmax_fs_2i_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( argmax_fs_2i_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr argmax_fs_2i_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void argmax_fs_2i_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void argmax_fs_2i_base::loadProperties()
{
    addProperty(vlen,
                1,
                "vlen",
...
//  Allow for logging 
// 
PREPARE_LOGGING(argmax_fs_2i_base);
...
uint32_t argmax_fs_2i_base::getNOutputStreams() {
    return 0;
}
...
void argmax_fs_2i_base::float_in_1_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( argmax_fs_2i_base, "START NotifySRI  port:stream " << float_in_1->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( float_in_1, sri ) );

  LOG_TRACE( argmax_fs_2i_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << float_in_1->getName() << "/" << sri.streamID); 
}

void argmax_fs_2i_base::float_in_2_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( argmax_fs_2i_base, "START NotifySRI  port:stream " << float_in_2->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( float_in_2, sri ) );

  LOG_TRACE( argmax_fs_2i_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << float_in_2->getName() << "/" << sri.streamID); 
}


void argmax_fs_2i_base::processStreamIdChanges()
{
    boost::mutex::scoped_lock lock(_sriMutex);

    LOG_TRACE( argmax_fs_2i_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
    ...
    if ( validGRBlock() ) {
        ...
        for ( ; item != _sriQueue.end(); item++ ) {
            ...
            for ( ; istream != _istreams.end(); idx++, istream++ ) {
                ...
                if ( istream->port == item->first ) {
                    LOG_DEBUG( argmax_fs_2i_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
                    istream->sri(true);
                    istream->spe(mode);

                    LOG_DEBUG( argmax_fs_2i_base,  "  SETTING  OUT_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
        ...
        LOG_WARN( argmax_fs_2i_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
    }

}

BULKIO::StreamSRI argmax_fs_2i_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI argmax_fs_2i_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    return createOutputSRI( oidx );
}

void argmax_fs_2i_base::adjustOutputRate(BULKIO::StreamSRI &sri )
{
    if ( validGRBlock() ) {
        double ret=sri.xdelta*gr_sptr->relative_rate();
/**
**/
        LOG_TRACE( argmax_fs_2i_base, "ADJUSTING SRI.XDELTA FROM/TO: " << sri.xdelta << "/" << ret );
        sri.xdelta = ret;
    }
}

argmax_fs_2i_base::TimeDuration argmax_fs_2i_base::getTargetDuration()
{
    ...
    LOG_TRACE( argmax_fs_2i_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
              " target_duration " << t_drate );
    return t_drate;
}

argmax_fs_2i_base::TimeDuration argmax_fs_2i_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time )
{
    TimeDuration delta;
    ...
    if ( start_time.is_not_a_date_time() == false ) {
        ...
        delta /= 4;
        LOG_TRACE( argmax_fs_2i_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
                  " delta " << delta );
    }
...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int argmax_fs_2i_base::_transformerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int argmax_fs_2i_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
                                 typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
    typedef typename std::vector< gr_istream< IN_PORT_TYPE > >   _IStreamList;
    typedef typename std::vector< gr_ostream< OUT_PORT_TYPE > >  _OStreamList;
...
// use incoming packet's time stamp to forward
      ...
      LOG_DEBUG( argmax_fs_2i_base, "                  mode:" <<  pkt->T.tcmode );
      LOG_DEBUG( argmax_fs_2i_base, "                status:" <<  pkt->T.tcstatus );
      LOG_DEBUG( argmax_fs_2i_base, "                offset:" <<  pkt->T.toff );
      LOG_DEBUG( argmax_fs_2i_base, "                 whole:" <<  std::setprecision(10) << pkt->T.twsec );
      LOG_DEBUG( argmax_fs_2i_base, "PKT - TS          frac:" <<  std::setprecision(12) << pkt->T.tfsec );
#endif
