...
randomizer_i::randomizer_i(const char *uuid, const char *label) :
randomizer_base(uuid, label)
{
}

randomizer_i::~randomizer_i()
{
}

void randomizer_i::reset_state(const std::string &id)
{
    if (this->reset) {
        gr_sptr->reset();
    }
...
void randomizer_i::createBlock()
{
    ...
    try {
        gr_sptr = atsc_make_randomizer();
    } catch (...) {
        return;
    }

    this->setPropertyChangeListener("reset", this, &randomizer_i::reset_state);
...
//      
BULKIO::StreamSRI randomizer_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
