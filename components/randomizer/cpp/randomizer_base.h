...
#ifndef RANDOMIZER_IMPL_BASE_H
#define RANDOMIZER_IMPL_BASE_H
...
#include "randomizer_GnuHawkBlock.h"
...
class randomizer_base : public GnuHawkBlock
{
    public:
        randomizer_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<randomizer_base> *serviceThread; 
        ...
        // Ports
        bulkio::InOctetPort *atsc_mpeg_packet_in;
    ...
    protected:
        ...
        void  atsc_mpeg_packet_in_newStreamCallback( BULKIO::StreamSRI &sri );
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( randomizer_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( randomizer_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( randomizer_base, " serviceFunction: retval:" << retval);
