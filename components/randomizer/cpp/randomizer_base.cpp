...
randomizer_base::randomizer_base(const char *uuid, const char *label) :
...
*******************************************************************************************/
void randomizer_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void randomizer_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    boost::mutex::scoped_lock lock(serviceThreadLock);
    if (serviceThread == 0) {
        atsc_mpeg_packet_in->unblock();
        serviceThread = new ProcessThread<randomizer_base>(this, 0.1);
        serviceThread->start();
    }
...
void randomizer_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( randomizer_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( randomizer_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr randomizer_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void randomizer_base::loadProperties()
{
    addProperty(reset,
                false,
                "reset",
...
//  Allow for logging 
// 
PREPARE_LOGGING(randomizer_base);
...
uint32_t randomizer_base::getNOutputStreams() {
    return 0;
}
...
void randomizer_base::atsc_mpeg_packet_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( randomizer_base, "START NotifySRI  port:stream " << atsc_mpeg_packet_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( atsc_mpeg_packet_in, sri ) );

  LOG_TRACE( randomizer_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << atsc_mpeg_packet_in->getName() << "/" << sri.streamID); 
}


void randomizer_base::processStreamIdChanges()
{
    boost::mutex::scoped_lock lock(_sriMutex);

    LOG_TRACE( randomizer_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
    ...
    if ( validGRBlock() ) {
        ...
        for ( ; item != _sriQueue.end(); item++ ) {
            ...
            for ( ; istream != _istreams.end(); idx++, istream++ ) {
                ...
                if ( istream->port == item->first ) {
                    LOG_DEBUG( randomizer_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
                    istream->sri(true);
                    istream->spe(mode);

                    LOG_DEBUG( randomizer_base,  "  SETTING  OUT_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
        ...
        LOG_WARN( randomizer_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
    }

}

BULKIO::StreamSRI randomizer_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI randomizer_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    return createOutputSRI( oidx );
}

void randomizer_base::adjustOutputRate(BULKIO::StreamSRI &sri )
{
    if ( validGRBlock() ) {
        double ret=sri.xdelta*gr_sptr->relative_rate();
/**
**/
        LOG_TRACE( randomizer_base, "ADJUSTING SRI.XDELTA FROM/TO: " << sri.xdelta << "/" << ret );
        sri.xdelta = ret;
    }
}

randomizer_base::TimeDuration randomizer_base::getTargetDuration()
{
    ...
    LOG_TRACE( randomizer_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
              " target_duration " << t_drate );
    return t_drate;
}

randomizer_base::TimeDuration randomizer_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time )
{
    TimeDuration delta;
    ...
    if ( start_time.is_not_a_date_time() == false ) {
        ...
        delta /= 4;
        LOG_TRACE( randomizer_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
                  " delta " << delta );
    }
