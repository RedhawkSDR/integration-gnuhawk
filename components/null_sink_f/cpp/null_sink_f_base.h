...
#ifndef NULL_SINK_F_IMPL_BASE_H
#define NULL_SINK_F_IMPL_BASE_H
...
#include "null_sink_f_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class null_sink_f_base;
...
class null_sink_f_base : public GnuHawkBlock
{
    public:
        null_sink_f_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<null_sink_f_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( null_sink_f_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( null_sink_f_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( null_sink_f_base, " serviceFunction: retval:" << retval);
