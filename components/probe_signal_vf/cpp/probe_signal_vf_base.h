...
#ifndef PROBE_SIGNAL_VF_IMPL_BASE_H
#define PROBE_SIGNAL_VF_IMPL_BASE_H
...
#include "probe_signal_vf_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class probe_signal_vf_base;
...
class probe_signal_vf_base : public GnuHawkBlock
{
    public:
        probe_signal_vf_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<probe_signal_vf_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( probe_signal_vf_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( probe_signal_vf_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( probe_signal_vf_base, " serviceFunction: retval:" << retval);
