...
add_ff_2i_i::add_ff_2i_i(const char *uuid, const char *label) :
add_ff_2i_base(uuid, label)
{
}

add_ff_2i_i::~add_ff_2i_i()
{
}
...
//
void add_ff_2i_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_add_ff(vlen);
...
//      
BULKIO::StreamSRI add_ff_2i_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
  //
