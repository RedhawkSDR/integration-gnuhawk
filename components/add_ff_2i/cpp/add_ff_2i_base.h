...
#ifndef ADD_FF_2I_IMPL_BASE_H
#define ADD_FF_2I_IMPL_BASE_H
...
#include "add_ff_2i_GnuHawkBlock.h"
#include "gr_malloc.h"

class add_ff_2i_base : public GnuHawkBlock
{
  ...
  SRIListener( add_ff_2i_base &parent ) : _parent(parent) {};

    add_ff_2i_base &_parent;
  };
    

    public:

        add_ff_2i_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<add_ff_2i_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( add_ff_2i_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( add_ff_2i_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( add_ff_2i_base, " serviceFunction: retval:" << retval);
