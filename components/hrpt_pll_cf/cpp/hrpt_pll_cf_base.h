...
#ifndef HRPT_PLL_CF_IMPL_BASE_H
#define HRPT_PLL_CF_IMPL_BASE_H
...
#include "hrpt_pll_cf_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class hrpt_pll_cf_base;
...
class hrpt_pll_cf_base : public GnuHawkBlock
{
    public:
        hrpt_pll_cf_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<hrpt_pll_cf_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( hrpt_pll_cf_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( hrpt_pll_cf_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( hrpt_pll_cf_base, " serviceFunction: retval:" << retval);
