...
hrpt_pll_cf_i::hrpt_pll_cf_i(const char *uuid, const char *label) :
hrpt_pll_cf_base(uuid, label)
{
}

hrpt_pll_cf_i::~hrpt_pll_cf_i()
{
}
...
void hrpt_pll_cf_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = noaa_make_hrpt_pll_cf( this->alpha, this->beta, this->max_offset );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI hrpt_pll_cf_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
