...
/*******************************************************************************************
    ...
    AUTO-GENERATED CODE. DO NOT MODIFY
 	...
 	Source: copy_octet.spd.xml
 	Generated on: Tue Feb 26 11:36:32 EST 2013
 	Redhawk IDE
 	Version:M.1.8.3
...
void copy_octet_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
    ...
    LOG_TRACE( copy_octet_base, "COMPLETED RELEASE OBJECT" );
}
...
void copy_octet_base::notifySRI( BULKIO_dataOctet_In_i *port, BULKIO::StreamSRI &sri ) {

  LOG_TRACE( copy_octet_base, "START NotifySRI  port:stream " << port->getName() << "/" << sri.streamID);
  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( port, sri ) );
  LOG_TRACE( copy_octet_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << port->getName() << "/" << sri.streamID); 
...
BULKIO::StreamSRI copy_octet_base::createOutputSRI( int32_t idx)
{
											...
											typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
  ...
  if ( !eos ) {
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
      LOG_TRACE(  copy_octet_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << istream->nitems() << "/" << istream->_data.size() );
    ...
    if ( !gr_sptr->fixed_rate() )  {
      ...
      LOG_TRACE( copy_octet_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << noutput_items  << "/" << gr_sptr->output_multiple());
    }
	...
	LOG_TRACE( copy_octet_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << "/" << istream->nitems() << "/" << _ninput_items_required[idx] << "/" << _input_ready[idx]);
      }
    ...
    if ( !dataReady ) {
      LOG_TRACE( copy_octet_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << "/" << _ninput_items_required[0] );
      return -1;	 
    }
    ...
    if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
      LOG_TRACE( copy_octet_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems << " ISTREAMS:" << _input_items.size() << " OSTREAMS:" << _output_items.size());
