...
copy_octet_i::copy_octet_i(const char *uuid, const char *label) :
copy_octet_base(uuid, label)
{
}

copy_octet_i::~copy_octet_i()
{
}
...
//
void copy_octet_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_copy(sizeof(unsigned char));
  this->registerGetterSetter("enabled", &gr_copy::enabled, &gr_copy::set_enabled);
...
//      
BULKIO::StreamSRI copy_octet_i::createOutputSRI( int32_t idx)
{
  //
  ...
  sri.mode = 0;
  sri.streamID = CORBA::string_dup(naming_service_name.c_str());
...
//
uint32_t  copy_octet_i::getNOutputStreams() {

  // Called by setupIOMappings when the number of Gnu Radio output streams == -1 (variable) and number of 
  // Redhawk output ports == 1. Return the number of output streams that you want to create
