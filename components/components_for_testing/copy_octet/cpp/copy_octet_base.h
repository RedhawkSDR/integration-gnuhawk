...
#ifndef COPY_OCTET_IMPL_BASE_H
#define COPY_OCTET_IMPL_BASE_H
...
#include "copy_octet_GnuHawkBlock.h"


class copy_octet_base : public GnuHawkBlock
{
    friend class BULKIO_dataOctet_In_i;
    friend class BULKIO_dataOctet_Out_i;

  class SRIListener : BULKIO_dataOctet_In_i::SRIListener {

    friend class  copy_octet_base;

  public:
    void operator() ( BULKIO_dataOctet_In_i *port, BULKIO::StreamSRI &sri  ) {
      _parent.notifySRI( port, sri );
    };
  
  private:
    
  SRIListener( copy_octet_base &parent ) : _parent(parent) {};

    copy_octet_base &_parent;
  };
    

    public:

        copy_octet_base(const char *uuid, const char *label);
    ...
    //
    // Output port mapping is created as follows:
    // if GnuRadio output streams == -1 and RedHawk output ports == 1
    //   then  getNOutputStreams will be called to build the output mapping entries
    // If GnuRadio output streams > -1 or (  GnuRadio output streams == -1 && Redhawk Ports > 1 )
    //   A mapping object will be created for the minimum value betwween:
    //       GnuRadio output streams in make_io_mapping call
    //         and
    //       Number of defined Redhawk Output ports 
    //
    // For each defined output map created, a default SRI object is requested
    // using the createOutputSRI method.  As SRI changes occur on input mappings those changes will
    // propigate to the same indexed output mapping.
    //
    ...
    //   A mapping object will be created for the minimum value between:
    ...
    void  notifySRI( BULKIO_dataOctet_In_i *port, BULKIO::StreamSRI &sri );
 ...
 gr_ostream( OUT_PORT_TYPE *out_port, std::string &in_sid, GNU_RADIO_BLOCK_PTR ingrb=NULL, int idx=0, int mode=0 ) :
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataOctet_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  
  typedef _IStream< BULKIO_dataOctet_In_i >::List    IStreamList;

  typedef _OStream< BULKIO_dataOctet_Out_i >::List   OStreamList;  
    
    
   ProcessThread<copy_octet_base> *serviceThread; 
  ...
  // Ports
  BULKIO_dataOctet_In_i *octet_in;
  BULKIO_dataOctet_Out_i *octet_out;
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( copy_octet_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( copy_octet_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( copy_octet_base, " serviceFunction: retval:" << retval);
