...
#include "copy_float_base.h"

/*******************************************************************************************

    AUTO-GENERATED CODE. DO NOT MODIFY
    
 	Source: copy_float.spd.xml
 	Generated on: Tue Feb 26 11:25:39 EST 2013
 	Redhawk IDE
 	Version:M.1.8.3
...
//  Allow for logging 
// 
PREPARE_LOGGING(copy_float_base);
...
copy_float_base::copy_float_base(const char *uuid, const char *label) :
...
void copy_float_base::construct()
{
...
*******************************************************************************************/
void copy_float_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void copy_float_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
...
void copy_float_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( copy_float_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( copy_float_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr copy_float_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void copy_float_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
    ...
    LOG_TRACE( copy_float_base, "COMPLETED RELEASE OBJECT" );
}

void copy_float_base::loadProperties()
{
    addProperty(enabled,
                true, 
               "enabled",
...
uint32_t copy_float_base::getNOutputStreams() {
	return 0;
}


void copy_float_base::setupIOMappings()
{
  ...
  LOG_DEBUG( copy_float_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
  std::string sid("");
  ...
  //
   ...
   if ( _istreams.size() > 0 || _ostreams.size() > 0 ) {
    ...
    LOG_DEBUG(  copy_float_base, "RESET INPUT SIGNATURE SIZE:" << _istreams.size() );
    IStreamList::iterator istream;
    for ( int idx=0 ; istream != _istreams.end(); idx++, istream++ ) {
        // re-add existing stream definitons
      LOG_DEBUG(  copy_float_base, "ADD READ INDEX TO GNU RADIO BLOCK");
    ...
    LOG_DEBUG(  copy_float_base, "RESET OUTPUT SIGNATURE SIZE:" << _ostreams.size() );
  ...
  if ( noutput_streams != -1  || (noutput_streams == -1 && outPorts.size() > 1  ) ) {
    ...
    if ( noutput_streams != -1 ) {
      if ( nstreams != noutput_streams ) 
         LOG_WARN( copy_float_base, "Number of OUTPUT PORTS is different than number of GNU RADIO STREAMS  PORTS/STREAMS " <<  nstreams << "/" << noutput_streams );
      nstreams = std::min( nstreams, noutput_streams);
    }

    // add number of output streams based min ( gr block output streams, or output ports)
    LOG_TRACE( copy_float_base, "setupIOMappings OutputPorts: " << nstreams );
...
void copy_float_base::notifySRI( BULKIO_dataFloat_In_i *port, BULKIO::StreamSRI &sri ) {

  LOG_TRACE( copy_float_base, "START NotifySRI  port:stream " << port->getName() << "/" << sri.streamID);
  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( port, sri ) );
  LOG_TRACE( copy_float_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << port->getName() << "/" << sri.streamID); 
  
}
 
void copy_float_base::processStreamIdChanges() {

  boost::mutex::scoped_lock lock(_sriMutex);

  LOG_TRACE( copy_float_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
  ...
  if ( validGRBlock() ) {
    ...
    LOG_TRACE( copy_float_base, " IN_MAX=" << max_input << " N_IN:" << n_input);
    int max_output =  gr_sptr->get_max_output_streams();
    int n_output = gr_sptr->get_num_output_streams(); 
    LOG_TRACE( copy_float_base, " OUT_MAX=" << max_output << " N_OUT:" << n_output);          
    ...
    for ( ; item != _sriQueue.end(); item++ ) {
       ...
       if ( istream == _istreams.end() ) {
	     ...
	     _istreams.push_back( gr_istream< BULKIO_dataFloat_In_i >( item->first, sid, gr_sptr, idx, mode ) );
             LOG_TRACE( copy_float_base, "GR_ISTREAM::ADD  PORT:" << item->first->getName() << " STREAM_ID:" << sid << " NSTREAMS:" << _istreams.size());

	     if ( var_ostreams ) {
	     	RH_UsesPortMap::iterator p_out = outPorts.begin();
		_ostreams.push_back( gr_ostream < BULKIO_dataFloat_Out_i >( dynamic_cast< BULKIO_dataFloat_Out_i *>(p_out->second), sid, gr_sptr, idx ));
	        LOG_TRACE( copy_float_base, "GR_OSTREAM::ADD  PORT:" << p_out->second->getName() << " STREAM_ID:" << sid << " OSTREAMS:" << _ostreams.size());
             }    
	  }
	  else {
	       LOG_WARN( copy_float_base, " NEW STREAM ID, MISSING INPUT STREAM DEFINITION"  );
	  }
       }
       else if ( !var_istreams ) {
	    LOG_DEBUG(  copy_float_base,  "  SETTING GR_OSTREAM ID/STREAM_ID :" << idx << "/" << sid  );
	    istream->sri(true);
	    istream->spe(mode);
       }

       LOG_DEBUG(  copy_float_base,  "  GR_OSTREAM ID/STREAM_ID :" << idx << "/" << sid  );
 	...
 	LOG_WARN(  copy_float_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
  }
      

 LOG_TRACE(  copy_float_base,  "processStreamID,  GR_ISTREAM MAP SIZE:"  << _istreams.size() );

}



BULKIO::StreamSRI copy_float_base::createOutputSRI( int32_t idx)
{
...
void copy_float_base::adjustOutputRate(BULKIO::StreamSRI &sri ) {

   if ( validGRBlock() ) {
      double ret=sri.xdelta*gr_sptr->relative_rate();
/**      
**/
      LOG_TRACE(copy_float_base, "ADJUSTING SRI.XDELTA FROM/TO: " << sri.xdelta << "/" << ret );
      sri.xdelta = ret;
...
copy_float_base::TimeDuration copy_float_base::getTargetDuration() {
  ...
  LOG_TRACE( copy_float_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
	     " target_duration " << t_drate );
  return t_drate;
}

copy_float_base::TimeDuration copy_float_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time ) {
  ...
  if ( start_time.is_not_a_date_time() == false ) {
    ...
    delta /= 4;
    LOG_TRACE( copy_float_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
	      " delta " << delta );
  }
  return delta;
}
...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int copy_float_base::_transformerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
											typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
  ...
  if ( validGRBlock() == false ) {
    ...
    LOG_DEBUG( copy_float_base, " FINISHED BUILDING  GNU RADIO BLOCK");
  }
  ...
  if ( !validGRBlock() || istreams.size() == 0 || ostreams.size() == 0  ) {
    LOG_WARN(copy_float_base, "NO STREAMS ATTACHED TO BLOCK..." );
    return NOOP;
  }
  ...
  for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
    ...
    if ( istream->overrun() ) {
      LOG_WARN( copy_float_base, " NOT KEEPING UP WITH STREAM ID:" << istream->streamID );
    }

    if ( istream->sriChanged() ) {
      // RESOLVE - need to look at how SRI changes can affect Gnu Radio BLOCK state
      LOG_DEBUG( copy_float_base, "SRI CHANGED, STREAMD IDX/ID: " 
  ...
  LOG_TRACE( copy_float_base, "READ NITEMS: "  << nitems );
  ...
  while ( nout > -1 && !exitServiceFunction && serviceThread->threadRunning() ) {
	...
	if ( _input_ready[idx] ) {
	  ...
	  LOG_TRACE( copy_float_base, " CONSUME READ DATA  ITEMS/REMAIN " << nitems << "/" << istream->nitems());
	}
    ...
    if ( nout == -1 ) {
      ...
      if ( eos ) {
        LOG_TRACE(  copy_float_base, " DATA NOT READY, EOS:" << eos );
  ...
  if ( eos ) {
	...
	LOG_DEBUG( copy_float_base, " CLOSING INPUT STREAM IDX:" << idx );
         istream->close();
         //
         // if we are variable input list of input streams and fixed output...... humm
         //
         if ( gr_sptr->get_max_input_streams() == -1 ) {
            LOG_DEBUG( copy_float_base, " REMOVE VARIABLE INPUT STREAM IDX:" << idx );
	    ...
	    if ( gr_sptr->get_max_output_streams() == -1 && outPorts.size() == 1 ) {
	       ...
	       for ( int i=0; ostream != ostreams.end(); i++, ostream++) {
	          if ( i == idx ) {
                     LOG_DEBUG( copy_float_base, " REMOVE VARIABLE OUTPUT STREAM IDX:" << idx );
  ...
  LOG_TRACE( copy_float_base, " END OF TRANSFORM SERVICE FUNCTION....." << noutput_items );
...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int copy_float_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
											typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
  typedef typename std::vector< gr_istream< IN_PORT_TYPE > >   _IStreamList;
  typedef typename std::vector< gr_ostream< OUT_PORT_TYPE > >  _OStreamList;
  ...
  if ( !eos ) {
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
      LOG_TRACE(  copy_float_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << istream->nitems() << "/" << istream->_data.size() );
      max_items_avail = std::max( istream->nitems(), max_items_avail );
    }

    if ( max_items_avail == 0  ) {
       LOG_TRACE( copy_float_base, "DATA CHECK - MAX ITEMS  NOUTPUT/MAX_ITEMS:" <<   noutput_items << "/" << max_items_avail);
       return -1;
    }
    ...
    if ( !gr_sptr->fixed_rate() )  {
      ...
      LOG_TRACE( copy_float_base, " VARIABLE FORECAST NOUTPUT == " << noutput_items );
    }   
    else {
      istream = istreams.begin();
      ...
      LOG_TRACE( copy_float_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << noutput_items  << "/" << gr_sptr->output_multiple());
    }
    ...
    while ( !dataReady && noutput_items >= outMultiple  ) {
      ...
      LOG_TRACE( copy_float_base, "--> FORECAST IN/OUT " << _ninput_items_required[0]  << "/" << noutput_items  );
	...
	LOG_TRACE( copy_float_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << "/" << istream->nitems() << "/" << _ninput_items_required[idx] << "/" << _input_ready[idx]);
      }
      ...
      LOG_TRACE( copy_float_base, " TRIM FORECAST NOUTPUT/READY " << noutput_items << "/" << dataReady );
    }

    // check if data is ready...
    if ( !dataReady ) {
      LOG_TRACE( copy_float_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << "/" << _ninput_items_required[0] );
      return -1;	 
    }
	...
	LOG_ERROR( copy_float_base, "MISSING INPUT STREAM FOR GR BLOCK, STREAM ID:" <<   istream->streamID );
        return -2;
      } 
    
      nitems = istream->nitems() - ritems;
      LOG_TRACE( copy_float_base,  " ISTREAM: IDX:" << idx  << " ITEMS AVAIL/READ/REQ " << nitems << "/" 
    ...
    if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
      LOG_TRACE( copy_float_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems << " ISTREAMS:" << _input_items.size() << " OSTREAMS:" << _output_items.size());
      nout = gr_sptr->general_work( noutput_items, _ninput_items, _input_items, _output_items);
      LOG_TRACE( copy_float_base, "RETURN  WORK ..... N_OUT:" << nout);
    }

    // check for stop condition from work method
    if ( nout < gr_block::WORK_DONE ) {
      LOG_WARN( copy_float_base, "WORK RETURNED STOP CONDITION..." << nout );
      nout=0;
      eos = true;
    }
  }
  ...
  if (nout != 0 or eos ) {
    ...
    noutput_items = nout;
    LOG_TRACE( copy_float_base, " WORK RETURNED: NOUT : " << nout << " EOS:" << eos);
    ostream = ostreams.begin();
	...
	pkt = istreams[inputIdx].pkt;
      ...
      LOG_TRACE( copy_float_base,  "PUSHING DATA   ITEMS/STREAM_ID " << ostream->nitems() << "/" << ostream->streamID );    
      if ( _maintainTimeStamp ) {

	// set time stamp for output samples based on input time stamp
	if ( ostream->nelems() == 0 )  {
#ifdef TEST_TIME_STAMP
	  LOG_DEBUG(  copy_float_base, "SEED - TS SRI:  xdelta:" << std::setprecision(12) << ostream->sri.xdelta );
	  LOG_DEBUG(  copy_float_base, "OSTREAM WRITE:   maint:" << _maintainTimeStamp );
	  LOG_DEBUG(  copy_float_base, "                  mode:" <<  ostream->tstamp.tcmode );
	  LOG_DEBUG(  copy_float_base, "                status:" <<  ostream->tstamp.tcstatus );
	  LOG_DEBUG(  copy_float_base, "                offset:" <<  ostream->tstamp.toff );
	  LOG_DEBUG(  copy_float_base, "                 whole:" <<  std::setprecision(10) << ostream->tstamp.twsec );
	  LOG_DEBUG(  copy_float_base, "SEED - TS         frac:" <<  std::setprecision(12) << ostream->tstamp.tfsec );
#endif
	...
	if ( pkt ) {
#ifdef TEST_TIME_STAMP
	  LOG_DEBUG(  copy_float_base, "OSTREAM  SRI:  items/xdelta:" << noutput_items << "/" << std::setprecision(12) << ostream->sri.xdelta );
	  LOG_DEBUG(  copy_float_base, "PKT - TS         maint:" << _maintainTimeStamp );
	  LOG_DEBUG(  copy_float_base, "                  mode:" <<  pkt->T.tcmode );
	  LOG_DEBUG(  copy_float_base, "                status:" <<  pkt->T.tcstatus );
	  LOG_DEBUG(  copy_float_base, "                offset:" <<  pkt->T.toff );
	  LOG_DEBUG(  copy_float_base, "                 whole:" <<  std::setprecision(10) << pkt->T.twsec );
	  LOG_DEBUG(  copy_float_base, "PKT - TS          frac:" <<  std::setprecision(12) << pkt->T.tfsec );
#endif
...
#ifdef TEST_TIME_STAMP
	  LOG_DEBUG(  copy_float_base, "OSTREAM  SRI:  items/xdelta:" << noutput_items << "/" << std::setprecision(12) << ostream->sri.xdelta );
	  LOG_DEBUG(  copy_float_base, "OSTREAM TOD      maint:" << _maintainTimeStamp );
	  LOG_DEBUG(  copy_float_base, "                  mode:" <<  ostream->tstamp.tcmode );
	  LOG_DEBUG(  copy_float_base, "                status:" <<  ostream->tstamp.tcstatus );
	  LOG_DEBUG(  copy_float_base, "                offset:" <<  ostream->tstamp.toff );
	  LOG_DEBUG(  copy_float_base, "                 whole:" <<  std::setprecision(10) << ostream->tstamp.twsec );
	  LOG_DEBUG(  copy_float_base, "OSTREAM TOD       frac:" <<  std::setprecision(12) << ostream->tstamp.tfsec );
#endif
