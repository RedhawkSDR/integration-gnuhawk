...
#ifndef COPY_FLOAT_IMPL_BASE_H
#define COPY_FLOAT_IMPL_BASE_H
...
#include "copy_float_GnuHawkBlock.h"


class copy_float_base : public GnuHawkBlock
{
  ...
  SRIListener( copy_float_base &parent ) : _parent(parent) {};

    copy_float_base &_parent;
  };
    

    public:

        copy_float_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<copy_float_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( copy_float_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( copy_float_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( copy_float_base, " serviceFunction: retval:" << retval);
