...
void kludge_copy_float_base::construct()
{
    ...
    float_in->setNewStreamListener(this, &kludge_copy_float_base::float_in_newStreamCallback);
...
void kludge_copy_float_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        float_in->unblock();
        serviceThread = new ProcessThread<kludge_copy_float_base>(this, 0.1);
        serviceThread->start();
    }
...
void kludge_copy_float_base::setupIOMappings( )
{
   ...
   //
    LOG_TRACE( kludge_copy_float_base, "setupIOMappings INPUT PORTS: " << inPorts.size() );
    ...
    for( int i=0; pname != inputPortOrder.end(); pname++ ) {
        ...
        if ( p_in != inPorts.end() ) {
            ...
            LOG_DEBUG( kludge_copy_float_base, "ADDING INPUT MAP IDX:" << i << " SID:" << sid );
    ...
    // streams over a single connection.  We are mapping a single RH Port to a single GNU Radio stream.
    //
    LOG_TRACE( kludge_copy_float_base, "setupIOMappings OutputPorts: " << outPorts.size() );
    ...
    for( int i=0; pname != outputPortOrder.end(); pname++ ) {
        ...
        if ( p_out != outPorts.end() ) {
            ...
            LOG_DEBUG( kludge_copy_float_base, "ADDING OUTPUT MAP IDX:" << i << " SID:" << sid );
...
void kludge_copy_float_base::float_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( kludge_copy_float_base, "START NotifySRI  port:stream " << float_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( float_in, sri ) );

  LOG_TRACE( kludge_copy_float_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << float_in->getName() << "/" << sri.streamID); 
}
        ...
        for ( ; item != _sriQueue.end(); item++ ) {
            ...
            for ( ; istream != _istreams.end(); idx++, istream++ ) {
                ...
                if ( istream->port == item->first ) {
                    LOG_DEBUG( kludge_copy_float_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
                    istream->sri(true);
                    istream->spe(mode);

                    LOG_DEBUG( kludge_copy_float_base,  "  SETTING  OUT_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
...
BULKIO::StreamSRI kludge_copy_float_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI kludge_copy_float_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    ...
    typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
    ...
    while ( nout > -1 && serviceThread->threadRunning() ) {
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > istream->nitems() ) {
                        LOG_WARN( kludge_copy_float_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE "
        ...
        if ( nout == -1 ) {
            ...
            if ( eos ) {
                LOG_TRACE(  kludge_copy_float_base, "EOS SEEN, SENDING DOWNSTREAM " );
    ...
    if ( eos ) {
        ...
        for ( ; eos && ostream != ostreams.end(); ostream++ ) {
            ...
            LOG_DEBUG( kludge_copy_float_base, " CLOSING OUTPUT STREAM IDX:" << idx );
            ostream->close();
        }
    }
                                 ...
                                 typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( kludge_copy_float_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << 
        ...
        if ( !gr_sptr->fixed_rate() )  {
            ...
            LOG_TRACE( kludge_copy_float_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( kludge_copy_float_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << 
        ...
        if ( !dataReady ) {
            LOG_TRACE( kludge_copy_float_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( kludge_copy_float_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems 
