...
#ifndef KLUDGE_COPY_FLOAT_IMPL_BASE_H
#define KLUDGE_COPY_FLOAT_IMPL_BASE_H
...
#include "kludge_copy_float_GnuHawkBlock.h"


class kludge_copy_float_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  kludge_copy_float_base;
  ...
  SRIListener( kludge_copy_float_base &parent ) : _parent(parent) {};

    kludge_copy_float_base &_parent;
  };
    

    public:

        kludge_copy_float_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
    
    
   ProcessThread<kludge_copy_float_base> *serviceThread; 
  ...
  // Ports
  BULKIO_dataFloat_In_i *float_in;
  BULKIO_dataFloat_Out_i *float_out;
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( kludge_copy_float_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( kludge_copy_float_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( kludge_copy_float_base, " serviceFunction: retval:" << retval);
