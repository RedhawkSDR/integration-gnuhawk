...
#ifndef NULL_SINK_FLOAT_IMPL_BASE_H
#define NULL_SINK_FLOAT_IMPL_BASE_H
...
#include "null_sink_float_GnuHawkBlock.h"


class null_sink_float_base : public GnuHawkBlock
{
    friend class BULKIO_dataFloat_In_i;

  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {

    friend class  null_sink_float_base;
  ...
  SRIListener( null_sink_float_base &parent ) : _parent(parent) {};

    null_sink_float_base &_parent;
  };
    

    public:

        null_sink_float_base(const char *uuid, const char *label);
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _IStream< BULKIO_dataFloat_In_i >::List    IStreamList;
   ...
   ProcessThread<null_sink_float_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( null_sink_float_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( null_sink_float_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( null_sink_float_base, " serviceFunction: retval:" << retval);
