...
void null_sink_float_base::construct()
{
    ...
    float_in->setNewStreamListener(this, &null_sink_float_base::float_in_newStreamCallback);
...
void null_sink_float_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        float_in->unblock();
        serviceThread = new ProcessThread<null_sink_float_base>(this, 0.1);
        serviceThread->start();
    }
...
void null_sink_float_base::setupIOMappings( )
{
   ...
   //
    LOG_TRACE( null_sink_float_base, "setupIOMappings INPUT PORTS: " << inPorts.size() );
    ...
    for( int i=0; pname != inputPortOrder.end(); pname++ ) {
        ...
        if ( p_in != inPorts.end() ) {
            ...
            LOG_DEBUG( null_sink_float_base, "ADDING INPUT MAP IDX:" << i << " SID:" << sid );
...
void null_sink_float_base::float_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( null_sink_float_base, "START NotifySRI  port:stream " << float_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( float_in, sri ) );

  LOG_TRACE( null_sink_float_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << float_in->getName() << "/" << sri.streamID); 
}
        ...
        for ( ; item != _sriQueue.end(); item++ ) {
            ...
            for ( ; istream != _istreams.end(); idx++, istream++ ) {
                ...
                if ( istream->port == item->first ) {
                    LOG_DEBUG( null_sink_float_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
                    istream->sri(true);
                    istream->spe(mode);
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > istream->nitems() ) {
                        LOG_WARN( null_sink_float_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE " 
...
template <  typename IN_PORT_TYPE > int null_sink_float_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams )
{
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( null_sink_float_base, "GET MAX ITEMS: STREAM:" << idx << " NITEMS/SCALARS:"
        ...
        if ( gr_sptr->fixed_rate() ) {
            ...
            LOG_TRACE( null_sink_float_base, " FIXED FORECAST NOUTPUT/output_multiple == " 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( null_sink_float_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" << 
        ...
        if ( !dataReady ) {
            LOG_TRACE( null_sink_float_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( null_sink_float_base, " CALLING WORK.....N_OUT:" << noutput_items << 
