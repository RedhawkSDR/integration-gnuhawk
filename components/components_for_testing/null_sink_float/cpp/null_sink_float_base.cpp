...
#include "null_sink_float_base.h"

/*******************************************************************************************

    AUTO-GENERATED CODE. DO NOT MODIFY
    
 	Source: null_sink_float.spd.xml
 	Generated on: Tue Feb 26 12:06:34 EST 2013
 	Redhawk IDE
 	Version:M.1.8.3
...
//  Allow for logging 
// 
PREPARE_LOGGING(null_sink_float_base);
...
null_sink_float_base::null_sink_float_base(const char *uuid, const char *label) :
...
void null_sink_float_base::construct()
{
...
*******************************************************************************************/
void null_sink_float_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void null_sink_float_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
...
void null_sink_float_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( null_sink_float_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( null_sink_float_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr null_sink_float_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void null_sink_float_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
    ...
    LOG_TRACE( null_sink_float_base, "COMPLETED RELEASE OBJECT" );
}

void null_sink_float_base::loadProperties()
{
}
...
void null_sink_float_base::setupIOMappings()
{
  ...
  LOG_DEBUG( null_sink_float_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
  std::string sid("");
  ...
  //
   ...
   if ( _istreams.size() > 0 ) {
    ...
    LOG_DEBUG(  null_sink_float_base, "RESET INPUT SIGNATURE SIZE:" << _istreams.size() );
    IStreamList::iterator istream;
    for ( int idx=0 ; istream != _istreams.end(); idx++, istream++ ) {
        // re-add existing stream definitons
      LOG_DEBUG(  null_sink_float_base, "ADD READ INDEX TO GNU RADIO BLOCK");
...
void null_sink_float_base::notifySRI( BULKIO_dataFloat_In_i *port, BULKIO::StreamSRI &sri ) {

  LOG_TRACE( null_sink_float_base, "START NotifySRI  port:stream " << port->getName() << "/" << sri.streamID);
  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( port, sri ) );
  LOG_TRACE( null_sink_float_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << port->getName() << "/" << sri.streamID); 
  
}
 
void null_sink_float_base::processStreamIdChanges() {

  boost::mutex::scoped_lock lock(_sriMutex);

  LOG_TRACE( null_sink_float_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
  ...
  if ( validGRBlock() ) {
    ...
    LOG_TRACE( null_sink_float_base, " IN_MAX=" << max_input << " N_IN:" << n_input);    
    ...
    for ( ; item != _sriQueue.end(); item++ ) {
       ...
       if ( istream == _istreams.end() ) {
	     ...
	     _istreams.push_back( gr_istream< BULKIO_dataFloat_In_i >( item->first, sid, gr_sptr, idx, mode ) );
             LOG_TRACE( null_sink_float_base, "GR_ISTREAM::ADD  PORT:" << item->first->getName() << " STREAM_ID:" << sid << " NSTREAMS:" << _istreams.size());
    
	  }
	  else {
	       LOG_WARN( null_sink_float_base, " NEW STREAM ID, MISSING INPUT STREAM DEFINITION"  );
	  }
       }
       else if ( !var_istreams ) {
	    LOG_DEBUG(  null_sink_float_base,  "  SETTING GR_OSTREAM ID/STREAM_ID :" << idx << "/" << sid  );
	    istream->sri(true);
	    istream->spe(mode);
       }
 	...
 	LOG_WARN(  null_sink_float_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
  }
      

 LOG_TRACE(  null_sink_float_base,  "processStreamID,  GR_ISTREAM MAP SIZE:"  << _istreams.size() );
...
null_sink_float_base::TimeDuration null_sink_float_base::getTargetDuration() {
  ...
  LOG_TRACE( null_sink_float_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
	     " target_duration " << t_drate );
  return t_drate;
}

null_sink_float_base::TimeDuration null_sink_float_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time ) {
  ...
  if ( start_time.is_not_a_date_time() == false ) {
    ...
    delta /= 4;
    LOG_TRACE( null_sink_float_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
	      " delta " << delta );
  }
  return delta;
}
...
template < typename IN_PORT_TYPE > int null_sink_float_base::_analyzerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ) {
  ...
  if ( validGRBlock() == false ) {
    ...
    LOG_DEBUG(  null_sink_float_base, " FINISHED BUILDING  GNU RADIO BLOCK");
  }
  ...
  if ( !validGRBlock() || istreams.size() == 0 ) {
    LOG_WARN(null_sink_float_base, "NO STREAMS ATTACHED TO BLOCK..." );
    return NOOP;
  }
  ...
  for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
    ...
    if ( istream->overrun() ) {
        LOG_WARN( null_sink_float_base, " NOT KEEPING UP WITH STREAM ID:" << istream->streamID );
    }
    
    // RESOLVE issue when SRI changes that could affect the GNU Radio BLOCK
    if ( istream->sriChanged() ) {
      LOG_DEBUG( null_sink_float_base, "SRI CHANGED, STREAMD IDX/ID: " 
               << idx << "/" << istream->pkt->streamID );
    }
  }

  LOG_TRACE( null_sink_float_base, "READ NITEMS: "  << nitems );
  ...
  while ( nout > -1 && !exitServiceFunction && serviceThread->threadRunning() ) {
	...
	if ( _input_ready[idx] ) {
	  ...
	  istream->consume( nitems );
          LOG_TRACE( null_sink_float_base, " CONSUME READ DATA  ITEMS/REMAIN " << nitems << "/" << istream->nitems());
	}
    ...
    if ( nout == -1 ) {
      ...
      if ( eos ) {
        LOG_TRACE( null_sink_float_base, " DATA NOT READY, EOS:" << eos );
  ...
  if ( eos ) {
    ...
    for ( ; istream != istreams.end() ; ) {
      ...
      if (  istream->eos() || eos == true ) {
        LOG_TRACE( null_sink_float_base, " CLOSING INPUT STREAM IDX:" << idx );
	istream->close();

	// variable list of inputs then we need to close that stream
	if ( gr_sptr->get_max_input_streams() == -1 ) {
	  LOG_TRACE( null_sink_float_base," REMOVE VARIABLE INPUT STREAM IDX:" << idx );
  ...
  LOG_TRACE( null_sink_float_base, " END OF ANALYZER SERVICE FUNCTION....." << noutput_items );
...
template <  typename IN_PORT_TYPE > int null_sink_float_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams )
{
  typedef typename std::vector< gr_istream< IN_PORT_TYPE > >   _IStreamList;
  ...
  if ( !eos ) {
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
      LOG_TRACE(  null_sink_float_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << istream->nitems() << "/" << istream->_data.size() );
    ...
    if ( noutput_items <= 0  ) {
       LOG_TRACE( null_sink_float_base, "DATA CHECK - MAX ITEMS  NOUTPUT/MAX_ITEMS:" <<   noutput_items << "/" << max_items_avail);
       return -1;
    }
	...
	if ( gr_sptr->fixed_rate() ) {
      ...
      LOG_TRACE( null_sink_float_base, " FIXED FORECAST NOUTPUT/output_multiple == " << noutput_items  << "/" << gr_sptr->output_multiple());
    }
    ...
    while ( !dataReady && noutput_items >= outMultiple  ) {
      ...
      LOG_TRACE( null_sink_float_base, "--> FORECAST IN/OUT " << _ninput_items_required[0]  << "/" << noutput_items  );
	...
	LOG_TRACE( null_sink_float_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << "/" << istream->nitems() << "/" << _ninput_items_required[idx] << "/" << _input_ready[idx]);
      }
      ...
      LOG_TRACE( null_sink_float_base, " TRIM FORECAST NOUTPUT/READY " << noutput_items << "/" << dataReady );
    }

    // check if data is ready...
    if ( !dataReady ) {
      LOG_TRACE( null_sink_float_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << "/" << _ninput_items_required[0] );
      return -1;	 
    }
	...
	LOG_ERROR( null_sink_float_base, "MISSING INPUT STREAM FOR GR BLOCK, STREAM ID:" <<   istream->streamID );
        return -2;
      } 
    
      nitems = istream->nitems() - ritems;
      LOG_TRACE( null_sink_float_base,  " ISTREAM: IDX:" << idx  << " ITEMS AVAIL/READ/REQ " << nitems << "/" 
    ...
    if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
      LOG_TRACE( null_sink_float_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems << " ISTREAMS:" << _input_items.size() << " OSTREAMS:" << _output_items.size());
      ...
      LOG_TRACE( null_sink_float_base, "RETURN  WORK ..... N_OUT:" << nout);
    }

    // check for stop condition from work method
    if ( nout < gr_block::WORK_DONE ) {
      LOG_WARN( null_sink_float_base, "WORK RETURNED STOP CONDITION..." << nout );
      nout=0;
      eos = true;
    }
  }
