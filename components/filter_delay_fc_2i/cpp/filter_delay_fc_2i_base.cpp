...
/*******************************************************************************************
    ...
    AUTO-GENERATED CODE. DO NOT MODIFY
 	...
 	Source: filter_delay_fc_2i.spd.xml
 	Generated on: Thu Feb 28 14:19:00 EST 2013
 	Redhawk IDE
 	Version:N.1.8.3
...
void filter_delay_fc_2i_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        if ( float_in_0 ) float_in_0->unblock();
        if ( float_in_1 ) float_in_1->unblock();
...
void filter_delay_fc_2i_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
    ...
    LOG_TRACE( filter_delay_fc_2i_base, "COMPLETED RELEASE OBJECT" );
}
...
void filter_delay_fc_2i_base::notifySRI( BULKIO_dataFloat_In_i *port, BULKIO::StreamSRI &sri ) {

  LOG_TRACE( filter_delay_fc_2i_base, "START NotifySRI  port:stream " << port->getName() << "/" << sri.streamID);
  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( port, sri ) );
  LOG_TRACE( filter_delay_fc_2i_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << port->getName() << "/" << sri.streamID); 
...
BULKIO::StreamSRI filter_delay_fc_2i_base::createOutputSRI( int32_t idx)
{
  ...
  while ( nout > -1 && !exitServiceFunction && serviceThread->threadRunning() ) {
	...
	if ( _input_ready[idx] ) {
	  ...
	  if ( nitems > istream->nitems() ) {
	       LOG_WARN( filter_delay_fc_2i_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE " << nitems << "/" << istream->nitems() );
											...
											typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
  ...
  if ( !eos ) {
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
      LOG_TRACE(  filter_delay_fc_2i_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << istream->nitems() << "/" << istream->_data.size() );
    ...
    if ( !gr_sptr->fixed_rate() )  {
      ...
      LOG_TRACE( filter_delay_fc_2i_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << noutput_items  << "/" << gr_sptr->output_multiple());
    }
	...
	LOG_TRACE( filter_delay_fc_2i_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << "/" << istream->nitems() << "/" << _ninput_items_required[idx] << "/" << _input_ready[idx]);
      }
    ...
    if ( !dataReady ) {
      LOG_TRACE( filter_delay_fc_2i_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << "/" << _ninput_items_required[0] );
      return -1;	 
    }
    ...
    if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
      LOG_TRACE( filter_delay_fc_2i_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems << " ISTREAMS:" << _input_items.size() << " OSTREAMS:" << _output_items.size());
