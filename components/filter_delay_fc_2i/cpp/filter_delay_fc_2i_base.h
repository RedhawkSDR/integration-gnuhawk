...
#ifndef FILTER_DELAY_FC_2I_IMPL_BASE_H
#define FILTER_DELAY_FC_2I_IMPL_BASE_H
...
#include "filter_delay_fc_2i_GnuHawkBlock.h"
#include "gr_malloc.h"

class filter_delay_fc_2i_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  filter_delay_fc_2i_base;
  ...
  SRIListener( filter_delay_fc_2i_base &parent ) : _parent(parent) {};

    filter_delay_fc_2i_base &_parent;
  };
    

    public:

        filter_delay_fc_2i_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<filter_delay_fc_2i_base> *serviceThread; 
  ...
  // Ports
  BULKIO_dataFloat_In_i *float_in_0;
  BULKIO_dataFloat_In_i *float_in_1;
  BULKIO_dataFloat_Out_i *complex_out;
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( filter_delay_fc_2i_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( filter_delay_fc_2i_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( filter_delay_fc_2i_base, " serviceFunction: retval:" << retval);
