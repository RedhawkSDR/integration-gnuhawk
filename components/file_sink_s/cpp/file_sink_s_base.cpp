...
file_sink_s_base::file_sink_s_base(const char *uuid, const char *label) :
...
void file_sink_s_base::construct()
{
    ...
    short_in->setNewStreamListener(this, &file_sink_s_base::short_in_newStreamCallback);
...
*******************************************************************************************/
void file_sink_s_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void file_sink_s_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    boost::mutex::scoped_lock lock(serviceThreadLock);
    if (serviceThread == 0) {
        short_in->unblock();
        serviceThread = new ProcessThread<file_sink_s_base>(this, 0.1);
        serviceThread->start();
    }
...
void file_sink_s_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( file_sink_s_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( file_sink_s_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr file_sink_s_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void file_sink_s_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void file_sink_s_base::loadProperties()
{
...
//  Allow for logging 
// 
PREPARE_LOGGING(file_sink_s_base);
...
void file_sink_s_base::setupIOMappings( )
{
    ...
    LOG_DEBUG( file_sink_s_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
    ...
    if ( _istreams.size() > 0 ) {
        ...
        LOG_DEBUG( file_sink_s_base, "RESET INPUT SIGNATURE SIZE:" << _istreams.size() );
        IStreamList::iterator istream;
        for ( int idx=0 ; istream != _istreams.end(); idx++, istream++ ) {
            // re-add existing stream definitons
            LOG_DEBUG(  file_sink_s_base, "ADD READ INDEX TO GNU RADIO BLOCK");
   ...
   //
    LOG_TRACE( file_sink_s_base, "setupIOMappings INPUT PORTS: " << inPorts.size() );
    ...
    for( int i=0; pname != inputPortOrder.end(); pname++ ) {
        ...
        if ( p_in != inPorts.end() ) {
            ...
            LOG_DEBUG( file_sink_s_base, "ADDING INPUT MAP IDX:" << i << " SID:" << sid );
...
void file_sink_s_base::short_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( file_sink_s_base, "START NotifySRI  port:stream " << short_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( short_in, sri ) );

  LOG_TRACE( file_sink_s_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << short_in->getName() << "/" << sri.streamID); 
}


void file_sink_s_base::processStreamIdChanges()
{
    boost::mutex::scoped_lock lock(_sriMutex);

    LOG_TRACE( file_sink_s_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
    ...
    if ( validGRBlock() ) {
        ...
        for ( ; item != _sriQueue.end(); item++ ) {
            ...
            for ( ; istream != _istreams.end(); idx++, istream++ ) {
                ...
                if ( istream->port == item->first ) {
                    LOG_DEBUG( file_sink_s_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
                    istream->sri(true);
                    istream->spe(mode);
        ...
        LOG_WARN( file_sink_s_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
    }

}

file_sink_s_base::TimeDuration file_sink_s_base::getTargetDuration()
{
    ...
    LOG_TRACE( file_sink_s_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
              " target_duration " << t_drate );
    return t_drate;
}

file_sink_s_base::TimeDuration file_sink_s_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time )
{
    TimeDuration delta;
    ...
    if ( start_time.is_not_a_date_time() == false ) {
        ...
        delta /= 4;
        LOG_TRACE( file_sink_s_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
                  " delta " << delta );
    }
...
template < typename IN_PORT_TYPE > int file_sink_s_base::_analyzerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams )
{
    ...
    if ( validGRBlock() == false ) {
        ...
        LOG_DEBUG( file_sink_s_base, " FINISHED BUILDING  GNU RADIO BLOCK");
    }
    ...
    if ( !validGRBlock() || istreams.size() == 0 ) {
        LOG_WARN( file_sink_s_base, "NO STREAMS ATTACHED TO BLOCK..." );
        return NOOP;
    }
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
        ...
        if ( istream->overrun() ) {
            LOG_WARN( file_sink_s_base, " NOT KEEPING UP WITH STREAM ID:" << istream->streamID );
        }
    
        // RESOLVE issue when SRI changes that could affect the GNU Radio BLOCK
        if ( istream->sriChanged() ) {
            LOG_DEBUG( file_sink_s_base, "SRI CHANGED, STREAMD IDX/ID: " 
                   << idx << "/" << istream->pkt->streamID );
        }
    }

    LOG_TRACE( file_sink_s_base, "READ NITEMS: "  << nitems );
    ...
    while ( nout > -1 && serviceThread->threadRunning() ) {
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > istream->nitems() ) {
                        LOG_WARN( file_sink_s_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE " 
                                 << nitems << "/" << istream->nitems() );
                        nitems = istream->nitems();
                    }
                    istream->consume( nitems );
                    LOG_TRACE( file_sink_s_base, " CONSUME READ DATA  ITEMS/REMAIN " << nitems << "/" << istream->nitems());
                }
            }
        ...
        if ( nout == -1 ) {
            ...
            if ( eos ) {
                LOG_TRACE( file_sink_s_base, " DATA NOT READY, EOS:" << eos );
    ...
    if ( eos ) {
        ...
        for ( ; istream != istreams.end() ; istream++) {
            int idx=std::distance( istreams.begin(), istream );
            LOG_TRACE( file_sink_s_base, " CLOSING INPUT STREAM IDX:" << idx );
            istream->close();
        }
    }
    ...
    LOG_TRACE( file_sink_s_base, " END OF ANALYZER SERVICE FUNCTION....." << noutput_items );
