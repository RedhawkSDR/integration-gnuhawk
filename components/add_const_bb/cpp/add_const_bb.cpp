...
add_const_bb_i::add_const_bb_i(const char *uuid, const char *label) :
add_const_bb_base(uuid, label)
{
}

add_const_bb_i::~add_const_bb_i()
{
}
...
void add_const_bb_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_add_const_bb( this->k );
    } catch (...) {
        return;
    }

    this->registerGetterSetter("k", &gr_add_const_bb::k, &gr_add_const_bb::set_k);
...
//      
BULKIO::StreamSRI add_const_bb_i::createOutputSRI( int32_t idx)
{
    //
