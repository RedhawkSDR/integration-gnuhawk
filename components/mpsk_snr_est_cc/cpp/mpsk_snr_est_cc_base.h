...
#ifndef MPSK_SNR_EST_CC_IMPL_BASE_H
#define MPSK_SNR_EST_CC_IMPL_BASE_H
...
#include "mpsk_snr_est_cc_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class mpsk_snr_est_cc_base;
...
class mpsk_snr_est_cc_base : public GnuHawkBlock
{
    public:
        mpsk_snr_est_cc_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<mpsk_snr_est_cc_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::Long type;
        CORBA::Long tag_nsamples;
        double alpha;
        double snr;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( mpsk_snr_est_cc_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( mpsk_snr_est_cc_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( mpsk_snr_est_cc_base, " serviceFunction: retval:" << retval);
