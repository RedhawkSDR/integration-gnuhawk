...
fir_filter_fff_i::fir_filter_fff_i(const char *uuid, const char *label) :
fir_filter_fff_base(uuid, label)
{
}

fir_filter_fff_i::~fir_filter_fff_i()
{
}

void fir_filter_fff_i::setTaps(const std::string &id){
    this->gr_sptr->set_taps(taps);
}

std::vector<float> fir_filter_fff_i::getTaps(){
...
//      
BULKIO::StreamSRI fir_filter_fff_i::createOutputSRI( int32_t idx)
{
  //
