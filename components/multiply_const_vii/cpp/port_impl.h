...
//RESOLVE    ENABLE_LOGGING;
    public:
        ...
        BULKIO_dataLong_Out_i(std::string port_name, multiply_const_vii_base *_parent);
       ...
       template <typename ALLOCATOR>
        void pushPacket(std::vector<CORBA::Long, ALLOCATOR>& data, BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID) {
				 ...
				 for (port = outConnections.begin(); port != outConnections.end(); port++) {
					 ...
					 //RESOLVE LOG_ERROR(BULKIO_dataLong_Out_i, "Call to pushPacket by BULKIO_dataLong_Out_i failed");
				     }
				 }
			     }
			 };
...
class BULKIO_dataLong_In_i : public POA_BULKIO::dataLong, public Port_Provides_base_impl
{
    public:
     ...
     //BULKIO_dataLong_In_i(std::string port_name, multiply_const_vii_base *_parent);
