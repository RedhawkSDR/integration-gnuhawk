...
#ifndef FIELD_SYNC_DEMUX_IMPL_BASE_H
#define FIELD_SYNC_DEMUX_IMPL_BASE_H
...
#include "field_sync_demux_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class field_sync_demux_base;
...
class field_sync_demux_base : public GnuHawkBlock
{
    public:
        field_sync_demux_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<field_sync_demux_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( field_sync_demux_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( field_sync_demux_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( field_sync_demux_base, " serviceFunction: retval:" << retval);
