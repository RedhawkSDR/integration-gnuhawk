...
field_sync_demux_i::field_sync_demux_i(const char *uuid, const char *label) :
field_sync_demux_base(uuid, label)
{
}

field_sync_demux_i::~field_sync_demux_i()
{
}
...
void field_sync_demux_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = atsc_make_field_sync_demux();
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI field_sync_demux_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
