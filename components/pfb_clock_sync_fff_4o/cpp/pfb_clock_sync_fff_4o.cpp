...
#include "pfb_clock_sync_fff_4o.h"
...
pfb_clock_sync_fff_4o_i::pfb_clock_sync_fff_4o_i(const char *uuid, const char *label) :
pfb_clock_sync_fff_4o_base(uuid, label)
{
    setPropertyChangeListener ("stream_id_map" , this, &pfb_clock_sync_fff_4o_i::streamIdChanged );
}

pfb_clock_sync_fff_4o_i::~pfb_clock_sync_fff_4o_i()
{
}

void pfb_clock_sync_fff_4o_i::streamIdChanged(const std::string& id)
{ 
...
void pfb_clock_sync_fff_4o_i::createBlock()
{
    //
    //

    try {
        gr_sptr = digital_make_pfb_clock_sync_fff( this->sps, this->gain, this->taps,
                this->filter_size, this->init_phase, this->max_rate_deviation, this->osps);
    } catch (...) {
        return;
    }

    this->registerGetterSetter("loop_bandwidth",
                               &digital_pfb_clock_sync_fff::get_loop_bandwidth,
                               &digital_pfb_clock_sync_fff::set_loop_bandwidth);

    this->registerGetterSetter("damping_factor",
                               &digital_pfb_clock_sync_fff::get_damping_factor,
                               &digital_pfb_clock_sync_fff::set_damping_factor);

     this->registerGetterSetter("alpha",
                                &digital_pfb_clock_sync_fff::get_alpha,
                                &digital_pfb_clock_sync_fff::set_alpha);

     this->registerGetterSetter("beta",
                                &digital_pfb_clock_sync_fff::get_beta,
                                &digital_pfb_clock_sync_fff::set_beta);

     this->registerGetter("clock_rate",
                          &digital_pfb_clock_sync_fff::get_clock_rate);

     this->registerSetter("max_rate_deviation",
                          &digital_pfb_clock_sync_fff::set_max_rate_deviation);
...
//      
BULKIO::StreamSRI pfb_clock_sync_fff_4o_i::createOutputSRI( int32_t oidx, int32_t &in_idx, std::string &ext)
{
    //
