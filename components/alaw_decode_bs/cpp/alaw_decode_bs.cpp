...
alaw_decode_bs_i::alaw_decode_bs_i(const char *uuid, const char *label) :
alaw_decode_bs_base(uuid, label)
{
}

alaw_decode_bs_i::~alaw_decode_bs_i()
{
}
...
void alaw_decode_bs_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = vocoder_make_alaw_decode_bs();
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI alaw_decode_bs_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
