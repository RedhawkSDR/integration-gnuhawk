...
#ifndef AGC_FF_IMPL_BASE_H
#define AGC_FF_IMPL_BASE_H
...
#include "agc_ff_GnuHawkBlock.h"


class agc_ff_base : public GnuHawkBlock
{
  ...
  SRIListener( agc_ff_base &parent ) : _parent(parent) {};
    ...
    public:
        ...
        agc_ff_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<agc_ff_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( agc_ff_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( agc_ff_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( agc_ff_base, " serviceFunction: retval:" << retval);
