...
agc_ff_i::agc_ff_i(const char *uuid, const char *label) :
agc_ff_base(uuid, label)
{
}
...
//
void agc_ff_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_agc_ff(this->rate, this->reference, this->gain, this->max_gain);
...
//      
BULKIO::StreamSRI agc_ff_i::createOutputSRI( int32_t idx)
{
  //
