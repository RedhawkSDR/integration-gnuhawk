...
fir_filter_fcc_i::fir_filter_fcc_i(const char *uuid, const char *label) :
fir_filter_fcc_base(uuid, label)
{
}

fir_filter_fcc_i::~fir_filter_fcc_i()
{
}

void fir_filter_fcc_i::set_taps(const std::string &id)
{
    gr_sptr->set_taps(this->taps);
}

std::vector<std::complex<float> > fir_filter_fcc_i::get_taps()
{
    return gr_sptr->taps();
}
...
void fir_filter_fcc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr::filter::fir_filter_fcc::make( this->decimation, this->taps );

    this->setPropertyChangeListener("taps", this, &fir_filter_fcc_i::set_taps);
    this->registerGetValue("taps", this, &fir_filter_fcc_i::get_taps);
...
//      
BULKIO::StreamSRI fir_filter_fcc_i::createOutputSRI( int32_t idx)
{
    //
