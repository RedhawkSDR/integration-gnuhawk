...
#ifndef REGENERATE_BB_IMPL_BASE_H
#define REGENERATE_BB_IMPL_BASE_H
...
#include "regenerate_bb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class regenerate_bb_base;
...
class regenerate_bb_base : public GnuHawkBlock
{
    public:
        regenerate_bb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<regenerate_bb_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::Long period;
        CORBA::ULong max_regen;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( regenerate_bb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( regenerate_bb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( regenerate_bb_base, " serviceFunction: retval:" << retval);
