...
#include "probe_signal_vb_base.h"
...
probe_signal_vb_base::probe_signal_vb_base(const char *uuid, const char *label) :
...
void probe_signal_vb_base::construct()
{
    ...
    byte_in->setNewStreamListener(this, &probe_signal_vb_base::byte_in_newStreamCallback);
...
*******************************************************************************************/
void probe_signal_vb_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void probe_signal_vb_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    boost::mutex::scoped_lock lock(serviceThreadLock);
    if (serviceThread == 0) {
        byte_in->unblock();
        serviceThread = new ProcessThread<probe_signal_vb_base>(this, 0.1);
        serviceThread->start();
    }
...
void probe_signal_vb_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( probe_signal_vb_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( probe_signal_vb_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr probe_signal_vb_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void probe_signal_vb_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void probe_signal_vb_base::loadProperties()
{
    addProperty(size,
                1,
                "size",
...
//  Allow for logging 
// 
PREPARE_LOGGING(probe_signal_vb_base);
...
void probe_signal_vb_base::setupIOMappings( )
{
    ...
    LOG_DEBUG( probe_signal_vb_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
    ...
    if ( _istreams.size() > 0 ) {
        ...
        LOG_DEBUG( probe_signal_vb_base, "RESET INPUT SIGNATURE SIZE:" << _istreams.size() );
        IStreamList::iterator istream;
        for ( int idx=0 ; istream != _istreams.end(); idx++, istream++ ) {
            // re-add existing stream definitons
            LOG_DEBUG(  probe_signal_vb_base, "ADD READ INDEX TO GNU RADIO BLOCK");
   ...
   //
    LOG_TRACE( probe_signal_vb_base, "setupIOMappings INPUT PORTS: " << inPorts.size() );
    ...
    for( int i=0; pname != inputPortOrder.end(); pname++ ) {
        ...
        if ( p_in != inPorts.end() ) {
            ...
            LOG_DEBUG( probe_signal_vb_base, "ADDING INPUT MAP IDX:" << i << " SID:" << sid );
...
void probe_signal_vb_base::byte_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( probe_signal_vb_base, "START NotifySRI  port:stream " << byte_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( byte_in, sri ) );

  LOG_TRACE( probe_signal_vb_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << byte_in->getName() << "/" << sri.streamID); 
}


void probe_signal_vb_base::processStreamIdChanges()
{
    boost::mutex::scoped_lock lock(_sriMutex);

    LOG_TRACE( probe_signal_vb_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
    ...
    if ( validGRBlock() ) {
        ...
        for ( ; item != _sriQueue.end(); item++ ) {
            ...
            for ( ; istream != _istreams.end(); idx++, istream++ ) {
                ...
                if ( istream->port == item->first ) {
                    LOG_DEBUG( probe_signal_vb_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
                    istream->sri(true);
                    istream->spe(mode);
        ...
        LOG_WARN( probe_signal_vb_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
    }

}

probe_signal_vb_base::TimeDuration probe_signal_vb_base::getTargetDuration()
{
    ...
    LOG_TRACE( probe_signal_vb_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
              " target_duration " << t_drate );
    return t_drate;
}

probe_signal_vb_base::TimeDuration probe_signal_vb_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time )
{
    TimeDuration delta;
    ...
    if ( start_time.is_not_a_date_time() == false ) {
        ...
        delta /= 4;
        LOG_TRACE( probe_signal_vb_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
                  " delta " << delta );
    }
...
template < typename IN_PORT_TYPE > int probe_signal_vb_base::_analyzerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams )
{
    ...
    if ( validGRBlock() == false ) {
        ...
        LOG_DEBUG( probe_signal_vb_base, " FINISHED BUILDING  GNU RADIO BLOCK");
    }
    ...
    if ( !validGRBlock() || istreams.size() == 0 ) {
        LOG_WARN( probe_signal_vb_base, "NO STREAMS ATTACHED TO BLOCK..." );
        return NOOP;
    }
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
        ...
        if ( istream->overrun() ) {
            LOG_WARN( probe_signal_vb_base, " NOT KEEPING UP WITH STREAM ID:" << istream->streamID );
        }
    
        // RESOLVE issue when SRI changes that could affect the GNU Radio BLOCK
        if ( istream->sriChanged() ) {
            LOG_DEBUG( probe_signal_vb_base, "SRI CHANGED, STREAMD IDX/ID: " 
                   << idx << "/" << istream->pkt->streamID );
        }
    }

    LOG_TRACE( probe_signal_vb_base, "READ NITEMS: "  << nitems );
    ...
    while ( nout > -1 && serviceThread->threadRunning() ) {
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > istream->nitems() ) {
                        LOG_WARN( probe_signal_vb_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE " 
                                 << nitems << "/" << istream->nitems() );
                        nitems = istream->nitems();
                    }
                    istream->consume( nitems );
                    LOG_TRACE( probe_signal_vb_base, " CONSUME READ DATA  ITEMS/REMAIN " << nitems << "/" << istream->nitems());
                }
            }
        ...
        if ( nout == -1 ) {
            ...
            if ( eos ) {
                LOG_TRACE( probe_signal_vb_base, " DATA NOT READY, EOS:" << eos );
    ...
    if ( eos ) {
        ...
        for ( ; istream != istreams.end() ; istream++) {
            int idx=std::distance( istreams.begin(), istream );
            LOG_TRACE( probe_signal_vb_base, " CLOSING INPUT STREAM IDX:" << idx );
            istream->close();
        }
    }
    ...
    LOG_TRACE( probe_signal_vb_base, " END OF ANALYZER SERVICE FUNCTION....." << noutput_items );
...
template <  typename IN_PORT_TYPE > int probe_signal_vb_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams )
{
    typedef typename std::vector< gr_istream< IN_PORT_TYPE > >   _IStreamList;
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( probe_signal_vb_base, "GET MAX ITEMS: STREAM:" << idx << " NITEMS/SCALARS:"
        ...
        if ( noutput_items <= 0  ) {
           LOG_TRACE( probe_signal_vb_base, "DATA CHECK - MAX ITEMS  NOUTPUT/MAX_ITEMS:" <<   noutput_items << "/" << max_items_avail);
           return -1;
        }
        ...
        if ( gr_sptr->fixed_rate() ) {
            ...
            LOG_TRACE( probe_signal_vb_base, " FIXED FORECAST NOUTPUT/output_multiple == " 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            LOG_TRACE( probe_signal_vb_base, "--> FORECAST IN/OUT " << _ninput_items_required[0]  << "/" << noutput_items  );
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( probe_signal_vb_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" << 
            ...
            LOG_TRACE( probe_signal_vb_base, " TRIM FORECAST NOUTPUT/READY " << noutput_items << "/" << dataReady );
        }

        // check if data is ready...
        if ( !dataReady ) {
            LOG_TRACE( probe_signal_vb_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" 
        ...
        for ( int idx=0 ; istream != istreams.end(); idx++, istream++ ) {
            ...
            try {
                ...
                LOG_ERROR( probe_signal_vb_base, "MISSING INPUT STREAM FOR GR BLOCK, STREAM ID:" <<   istream->streamID );
                return -2;
            } 

            nitems = istream->nitems() - ritems;
            LOG_TRACE( probe_signal_vb_base,  " ISTREAM: IDX:" << idx  << " ITEMS AVAIL/READ/REQ " << nitems << "/" 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( probe_signal_vb_base, " CALLING WORK.....N_OUT:" << noutput_items << 
            ...
            LOG_TRACE( probe_signal_vb_base, "RETURN  WORK ..... N_OUT:" << nout);
        }

        // check for stop condition from work method
        if ( nout < gr_block::WORK_DONE ) {
            LOG_WARN( probe_signal_vb_base, "WORK RETURNED STOP CONDITION..." << nout );
            nout=0;
            eos = true;
        }
    }
