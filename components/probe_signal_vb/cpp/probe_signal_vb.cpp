...
probe_signal_vb_i::probe_signal_vb_i(const char *uuid, const char *label) :
probe_signal_vb_base(uuid, label)
{
}

probe_signal_vb_i::~probe_signal_vb_i()
{
}
...
void probe_signal_vb_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_probe_signal_vb( this->size );

    this->registerGetter("level",
                         &gr_probe_signal_vb::level);
