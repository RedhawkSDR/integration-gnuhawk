...
cvsd_encode_sb_i::cvsd_encode_sb_i(const char *uuid, const char *label) :
cvsd_encode_sb_base(uuid, label)
{
}

cvsd_encode_sb_i::~cvsd_encode_sb_i()
{
}

short cvsd_encode_sb_i::get_min_step()
{
    return gr_sptr->min_step();
}

short cvsd_encode_sb_i::get_max_step()
{
    return gr_sptr->max_step();
}

double cvsd_encode_sb_i::get_step_decay()
{
    return gr_sptr->step_decay();
}

double cvsd_encode_sb_i::get_accum_decay()
{
    return gr_sptr->accum_decay();
}

int cvsd_encode_sb_i::get_K()
{
    return gr_sptr->K();
}

int cvsd_encode_sb_i::get_J()
{
    return gr_sptr->J();
}

short cvsd_encode_sb_i::get_pos_accum_max()
{
    return gr_sptr->pos_accum_max();
}

short cvsd_encode_sb_i::get_neg_accum_max()
{
...
//      
BULKIO::StreamSRI cvsd_encode_sb_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
