...
#ifndef CVSD_ENCODE_SB_IMPL_H
#define CVSD_ENCODE_SB_IMPL_H

#include "cvsd_encode_sb_base.h"

class cvsd_encode_sb_i : public cvsd_encode_sb_base
{
    public:
        cvsd_encode_sb_i(const char *uuid, const char *label);
        ~cvsd_encode_sb_i();
        ...
        short get_min_step() ;
        short get_max_step();
        double get_step_decay();
        double get_accum_decay();
        int get_K();
        int get_J();
        short get_pos_accum_max();
        short get_neg_accum_max();
};
