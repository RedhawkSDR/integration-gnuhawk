...
#ifndef CVSD_ENCODE_SB_IMPL_BASE_H
#define CVSD_ENCODE_SB_IMPL_BASE_H
...
#include "cvsd_encode_sb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class cvsd_encode_sb_base;
...
class cvsd_encode_sb_base : public GnuHawkBlock
{
    public:
        cvsd_encode_sb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<cvsd_encode_sb_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( cvsd_encode_sb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( cvsd_encode_sb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( cvsd_encode_sb_base, " serviceFunction: retval:" << retval);
