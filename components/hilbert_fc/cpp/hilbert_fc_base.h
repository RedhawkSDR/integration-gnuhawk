...
#ifndef HILBERT_FC_IMPL_BASE_H
#define HILBERT_FC_IMPL_BASE_H
...
#include "hilbert_fc_GnuHawkBlock.h"
#include "gr_malloc.h"

class hilbert_fc_base : public GnuHawkBlock
{
  ...
  SRIListener( hilbert_fc_base &parent ) : _parent(parent) {};

    hilbert_fc_base &_parent;
  };
    

    public:

        hilbert_fc_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<hilbert_fc_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( hilbert_fc_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( hilbert_fc_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( hilbert_fc_base, " serviceFunction: retval:" << retval);
