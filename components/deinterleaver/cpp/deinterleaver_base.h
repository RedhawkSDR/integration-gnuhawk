...
#ifndef DEINTERLEAVER_IMPL_BASE_H
#define DEINTERLEAVER_IMPL_BASE_H
...
#include "deinterleaver_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class deinterleaver_base;
...
class deinterleaver_base : public GnuHawkBlock
{
    public:
        deinterleaver_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<deinterleaver_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( deinterleaver_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( deinterleaver_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( deinterleaver_base, " serviceFunction: retval:" << retval);
