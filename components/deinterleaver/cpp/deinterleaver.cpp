...
deinterleaver_i::deinterleaver_i(const char *uuid, const char *label) :
deinterleaver_base(uuid, label)
{
}

deinterleaver_i::~deinterleaver_i()
{
}
...
void deinterleaver_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = atsc_make_deinterleaver();
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI deinterleaver_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
