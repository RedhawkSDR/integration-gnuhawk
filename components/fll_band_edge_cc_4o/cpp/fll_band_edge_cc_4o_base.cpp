...
void fll_band_edge_cc_4o_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        ...
        serviceThread = new ProcessThread<fll_band_edge_cc_4o_base>(this, 0.1);
        serviceThread->start();
    }
...
void fll_band_edge_cc_4o_base::data_complex_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( fll_band_edge_cc_4o_base, "START NotifySRI  port:stream " << data_complex_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( data_complex_in, sri ) );

  LOG_TRACE( fll_band_edge_cc_4o_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << data_complex_in->getName() << "/" << sri.streamID); 
}
...
BULKIO::StreamSRI fll_band_edge_cc_4o_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI fll_band_edge_cc_4o_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
                                 ...
                                 typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( fll_band_edge_cc_4o_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << 
        ...
        if ( !gr_sptr->fixed_rate() )  {
            ...
            LOG_TRACE( fll_band_edge_cc_4o_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( fll_band_edge_cc_4o_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << 
        ...
        if ( !dataReady ) {
            LOG_TRACE( fll_band_edge_cc_4o_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( fll_band_edge_cc_4o_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems 
