...
// Destructor
fll_band_edge_cc_4o_base::~fll_band_edge_cc_4o_base()
{
    // Free input streams
...
void fll_band_edge_cc_4o_base::setupIOMappings( )
{
   ...
   //
    ...
    // grab ports based on their order in the scd.xml file
    p_out = outPorts.find("data_frequency_out");
    if ( p_out != outPorts.end() ) {
        bulkio::OutFloatPort *port = dynamic_cast< bulkio::OutFloatPort * >(p_out->second);
        int idx = -1;
        std::string ext;
    ...
    // grab ports based on their order in the scd.xml file
    p_out = outPorts.find("data_phase_out");
    if ( p_out != outPorts.end() ) {
        bulkio::OutFloatPort *port = dynamic_cast< bulkio::OutFloatPort * >(p_out->second);
        int idx = -1;
        std::string ext;
...
BULKIO::StreamSRI fll_band_edge_cc_4o_base::createOutputSRI( int32_t oidx, int32_t &in_idx, std::string &ext)
{
...
int fll_band_edge_cc_4o_base::_forecastAndProcess( bool &eos, std::vector< gr_istream_base * > &istreams ,
                                 std::vector< gr_ostream_base * > &ostreams  )
{
    typedef std::vector< gr_istream_base * >   _IStreamList;
    typedef std::vector< gr_ostream_base * >  _OStreamList;
...
// use incoming packet's time stamp to forward
      ...
      LOG_DEBUG( fll_band_edge_cc_4o_base, "                  mode:" <<  pktTs.tcmode );
      LOG_DEBUG( fll_band_edge_cc_4o_base, "                status:" <<  pktTs.tcstatus );
      LOG_DEBUG( fll_band_edge_cc_4o_base, "                offset:" <<  pktTs.toff );
      LOG_DEBUG( fll_band_edge_cc_4o_base, "                 whole:" <<  std::setprecision(10) << pktTs.twsec );
      LOG_DEBUG( fll_band_edge_cc_4o_base, "PKT - TS          frac:" <<  std::setprecision(12) << pktTs.tfsec );
#endif
