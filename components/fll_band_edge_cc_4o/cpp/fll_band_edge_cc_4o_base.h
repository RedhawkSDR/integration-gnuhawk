...
#ifndef FLL_BAND_EDGE_CC_4O_IMPL_BASE_H
#define FLL_BAND_EDGE_CC_4O_IMPL_BASE_H
...
#include "fll_band_edge_cc_4o_GnuHawkBlock.h"


class fll_band_edge_cc_4o_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  fll_band_edge_cc_4o_base;
  ...
  SRIListener( fll_band_edge_cc_4o_base &parent ) : _parent(parent) {};

    fll_band_edge_cc_4o_base &_parent;
  };
    

    public:

        fll_band_edge_cc_4o_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<fll_band_edge_cc_4o_base> *serviceThread; 
  ...
  // Member variables exposed as properties
   ...
   float rolloff;
   CORBA::Long filter_size;
   float bandwidth;

  // Ports
  BULKIO_dataFloat_In_i *data_complex_in;
  BULKIO_dataFloat_Out_i *data_complex_out;
  BULKIO_dataFloat_Out_i *data_frequency_out;
  BULKIO_dataFloat_Out_i *data_phase_out;
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( fll_band_edge_cc_4o_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( fll_band_edge_cc_4o_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( fll_band_edge_cc_4o_base, " serviceFunction: retval:" << retval);
