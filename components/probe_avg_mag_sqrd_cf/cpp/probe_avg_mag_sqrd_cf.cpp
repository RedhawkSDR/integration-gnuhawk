...
probe_avg_mag_sqrd_cf_i::probe_avg_mag_sqrd_cf_i(const char *uuid, const char *label) :
probe_avg_mag_sqrd_cf_base(uuid, label)
{
}

probe_avg_mag_sqrd_cf_i::~probe_avg_mag_sqrd_cf_i()
{
}
...
void probe_avg_mag_sqrd_cf_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_probe_avg_mag_sqrd_cf( this->threshold_db, this->alpha );
    } catch (...) {
        return;
    }

    this->registerGetterSetter("threshold_db", &gr_probe_avg_mag_sqrd_cf::threshold, &gr_probe_avg_mag_sqrd_cf::set_threshold );
    this->registerSetter("alpha", &gr_probe_avg_mag_sqrd_cf::set_alpha);
    this->registerGetter("unmuted", &gr_probe_avg_mag_sqrd_cf::unmuted);
    this->registerGetter("level", &gr_probe_avg_mag_sqrd_cf::level);
...
//      
BULKIO::StreamSRI probe_avg_mag_sqrd_cf_i::createOutputSRI( int32_t idx)
{
    //
