...
#ifndef PLL_CARRIERTRACKING_CC_IMPL_H
#define PLL_CARRIERTRACKING_CC_IMPL_H

#include "pll_carriertracking_cc_base.h"

class pll_carriertracking_cc_i : public pll_carriertracking_cc_base
{
    public:
        pll_carriertracking_cc_i(const char *uuid, const char *label);
        ~pll_carriertracking_cc_i();
        ...
        bool lock_detector();
        void squelch_enable(const std::string &id);
        void set_lock_threshold(const std::string &id);
};
