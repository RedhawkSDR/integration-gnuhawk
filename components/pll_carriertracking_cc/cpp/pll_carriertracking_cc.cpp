...
pll_carriertracking_cc_i::pll_carriertracking_cc_i(const char *uuid, const char *label) :
pll_carriertracking_cc_base(uuid, label)
{
}

pll_carriertracking_cc_i::~pll_carriertracking_cc_i()
{
}

bool pll_carriertracking_cc_i::lock_detector()
{
    return gr_sptr->lock_detector();
}

void pll_carriertracking_cc_i::squelch_enable(const std::string &id)
{
    gr_sptr->squelch_enable(this->d_squelch_enable);
}

void pll_carriertracking_cc_i::set_lock_threshold(const std::string &id)
{
    gr_sptr->set_lock_threshold(this->d_lock_threshold);
}
...
//      
BULKIO::StreamSRI pll_carriertracking_cc_i::createOutputSRI( int32_t idx)
{
    //
