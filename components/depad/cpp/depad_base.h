...
#ifndef DEPAD_IMPL_BASE_H
#define DEPAD_IMPL_BASE_H
...
class depad_base : public GnuHawkBlock
{
    public:
        depad_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<depad_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( depad_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( depad_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( depad_base, " serviceFunction: retval:" << retval);
