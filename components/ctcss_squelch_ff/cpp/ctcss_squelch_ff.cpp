...
ctcss_squelch_ff_i::ctcss_squelch_ff_i(const char *uuid, const char *label) :
ctcss_squelch_ff_base(uuid, label)
{
}

ctcss_squelch_ff_i::~ctcss_squelch_ff_i()
{
}

bool ctcss_squelch_ff_i::get_unmuted()
{
    return gr_sptr->unmuted();
}

int ctcss_squelch_ff_i::get_ramp()
{
    return gr_sptr->ramp();
}

bool ctcss_squelch_ff_i::get_gate()
{
    return gr_sptr->gate();
}

void ctcss_squelch_ff_i::set_ramp(const std::string &id)
{
    gr_sptr->set_ramp(this->ramp);
}

void ctcss_squelch_ff_i::set_gate(const std::string &id)
{
    gr_sptr->set_gate(this->gate);
}
...
void ctcss_squelch_ff_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_ctcss_squelch_ff( this->rate, this->freq, this->level, this->len, this->ramp, this->gate );
    } catch (...) {
        return;
    }

    this->registerGetterSetter("level", &gr_ctcss_squelch_ff::level, &gr_ctcss_squelch_ff::set_level);
    this->registerGetter("len", &gr_ctcss_squelch_ff::len);
    this->registerGetter("squelch_range", &gr_ctcss_squelch_ff::squelch_range);
    this->registerGetValue("unmuted", this, &ctcss_squelch_ff_i::get_unmuted);
    this->registerGetValue("ramp", this, &ctcss_squelch_ff_i::get_ramp);
    this->registerGetValue("gate", this, &ctcss_squelch_ff_i::get_gate);
    this->setPropertyChangeListener("ramp", this, &ctcss_squelch_ff_i::set_ramp);
    this->setPropertyChangeListener("gate", this, &ctcss_squelch_ff_i::set_gate);
...
//      
BULKIO::StreamSRI ctcss_squelch_ff_i::createOutputSRI( int32_t idx)
{
    //
