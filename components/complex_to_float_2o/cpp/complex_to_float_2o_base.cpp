...
void complex_to_float_2o_base::construct()
{
    ...
    data_complex_in->setNewStreamListener(this, &complex_to_float_2o_base::data_complex_in_newStreamCallback);
    oid = ossie::corba::RootPOA()->activate_object(data_complex_in);
    data_real_out = new bulkio::OutFloatPort("data_real_out");
    oid = ossie::corba::RootPOA()->activate_object(data_real_out);
    data_imag_out = new bulkio::OutFloatPort("data_imag_out");
...
void complex_to_float_2o_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        ...
        serviceThread = new ProcessThread<complex_to_float_2o_base>(this, 0.1);
        serviceThread->start();
    }
...
// Destructor
complex_to_float_2o_base::~complex_to_float_2o_base()
{
    // Free input streams
...
void complex_to_float_2o_base::setupIOMappings( )
{
   ...
   //
    ...
    i = 0;
    // grab ports based on their order in the scd.xml file
    p_out = outPorts.find("data_real_out");
    ...
    // grab ports based on their order in the scd.xml file
    p_out = outPorts.find("data_imag_out");
    if ( p_out != outPorts.end() ) {
        bulkio::OutFloatPort *port = dynamic_cast< bulkio::OutFloatPort * >(p_out->second);
        int idx = -1;
        BULKIO::StreamSRI sri = createOutputSRI( i, idx );
...
void complex_to_float_2o_base::data_complex_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( complex_to_float_2o_base, "START NotifySRI  port:stream " << data_complex_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( data_complex_in, sri ) );

  LOG_TRACE( complex_to_float_2o_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << data_complex_in->getName() << "/" << sri.streamID); 
}
...
BULKIO::StreamSRI complex_to_float_2o_base::createOutputSRI( int32_t oidx ) {
...
int complex_to_float_2o_base::_transformerServiceFunction( std::vector< gr_istream_base * > &istreams ,
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > (*istream)->nitems() ) {
                        LOG_WARN( complex_to_float_2o_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE "
...
int complex_to_float_2o_base::_forecastAndProcess( bool &eos, std::vector< gr_istream_base * > &istreams ,
                                 std::vector< gr_ostream_base * > &ostreams  )
{
    typedef std::vector< gr_istream_base * >   _IStreamList;
    typedef std::vector< gr_ostream_base * >  _OStreamList;
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( complex_to_float_2o_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << 
        ...
        if ( !gr_sptr->fixed_rate() )  {
            ...
            LOG_TRACE( complex_to_float_2o_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( complex_to_float_2o_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   (*istream)->nelems() << 
        ...
        if ( !dataReady ) {
            LOG_TRACE( complex_to_float_2o_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0]->nitems() << 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( complex_to_float_2o_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems 
...
// use incoming packet's time stamp to forward
      ...
      LOG_DEBUG( complex_to_float_2o_base, "                  mode:" <<  pktTs.tcmode );
      LOG_DEBUG( complex_to_float_2o_base, "                status:" <<  pktTs.tcstatus );
      LOG_DEBUG( complex_to_float_2o_base, "                offset:" <<  pktTs.toff );
      LOG_DEBUG( complex_to_float_2o_base, "                 whole:" <<  std::setprecision(10) << pktTs.twsec );
      LOG_DEBUG( complex_to_float_2o_base, "PKT - TS          frac:" <<  std::setprecision(12) << pktTs.tfsec );
#endif
