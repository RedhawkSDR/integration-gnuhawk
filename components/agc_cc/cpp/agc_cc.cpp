...
agc_cc_i::agc_cc_i(const char *uuid, const char *label) :
agc_cc_base(uuid, label)
{
}
...
//
void agc_cc_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
	gr_sptr = gr_make_agc_cc(this->rate, this->reference, this->gain, this->max_gain);
...
//      
BULKIO::StreamSRI agc_cc_i::createOutputSRI( int32_t idx)
{
  //
