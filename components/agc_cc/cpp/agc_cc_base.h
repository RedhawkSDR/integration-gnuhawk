...
#ifndef AGC_CC_IMPL_BASE_H
#define AGC_CC_IMPL_BASE_H
...
#include "agc_cc_GnuHawkBlock.h"


class agc_cc_base : public GnuHawkBlock
{
  ...
  SRIListener( agc_cc_base &parent ) : _parent(parent) {};
    ...
    public:
        ...
        agc_cc_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<agc_cc_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( agc_cc_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( agc_cc_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( agc_cc_base, " serviceFunction: retval:" << retval);
