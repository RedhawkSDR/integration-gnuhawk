...
and_const_ii_i::and_const_ii_i(const char *uuid, const char *label) :
and_const_ii_base(uuid, label)
{
}

and_const_ii_i::~and_const_ii_i()
{
}
...
void and_const_ii_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_and_const_ii (this->k);
    } catch (...) {
        return;
    }

    this->registerGetterSetter("k", &gr_and_const_ii::k, &gr_and_const_ii::set_k);
...
//      
BULKIO::StreamSRI and_const_ii_i::createOutputSRI( int32_t idx)
{
    //
