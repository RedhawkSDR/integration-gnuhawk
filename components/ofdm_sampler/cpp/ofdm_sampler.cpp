...
ofdm_sampler_i::ofdm_sampler_i(const char *uuid, const char *label) :
ofdm_sampler_base(uuid, label)
{
}

ofdm_sampler_i::~ofdm_sampler_i()
{
}
...
void ofdm_sampler_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = digital_make_ofdm_sampler( fft_length, symbol_length, timeout );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI ofdm_sampler_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
