...
#ifndef OFDM_SAMPLER_IMPL_BASE_H
#define OFDM_SAMPLER_IMPL_BASE_H
...
#include "ofdm_sampler_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class ofdm_sampler_base;
...
class ofdm_sampler_base : public GnuHawkBlock
{
    public:
        ofdm_sampler_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<ofdm_sampler_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::ULong fft_length;
        CORBA::ULong symbol_length;
        CORBA::ULong timeout;

        // Ports
        bulkio::InFloatPort *complex_in;
        bulkio::InOctetPort *trigger_in;
        bulkio::OutFloatPort *complex_out;
        bulkio::OutOctetPort *sig_out;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( ofdm_sampler_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( ofdm_sampler_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( ofdm_sampler_base, " serviceFunction: retval:" << retval);
