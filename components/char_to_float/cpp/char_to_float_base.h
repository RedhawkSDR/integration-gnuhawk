...
#ifndef CHAR_TO_FLOAT_IMPL_BASE_H
#define CHAR_TO_FLOAT_IMPL_BASE_H
...
#include "char_to_float_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class char_to_float_base;
...
class char_to_float_base : public GnuHawkBlock
{
    public:
        char_to_float_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<char_to_float_base> *serviceThread; 
    ...
    protected:
        ...
        void  char_in_newStreamCallback( BULKIO::StreamSRI &sri );
    ...
    typedef std::deque< std::pair< bulkio::InCharPort *, BULKIO::StreamSRI > >  SRIQueue;
    typedef _IStream< bulkio::InCharPort >::List    IStreamList;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( char_to_float_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( char_to_float_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( char_to_float_base, " serviceFunction: retval:" << retval);
