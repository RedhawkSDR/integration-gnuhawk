...
// Destructor
char_to_float_base::~char_to_float_base()
{
    // Free input streams
...
void char_to_float_base::setupIOMappings( )
{
   ...
   //
    ...
    if ( p_in != inPorts.end() ) {
        ...
        _istreams.push_back( new gr_istream< bulkio::InCharPort > ( port, gr_sptr, i, mode, sid ));
...
int char_to_float_base::_transformerServiceFunction( std::vector< gr_istream_base * > &istreams ,
...
int char_to_float_base::_forecastAndProcess( bool &eos, std::vector< gr_istream_base * > &istreams ,
                                 std::vector< gr_ostream_base * > &ostreams  )
{
    typedef std::vector< gr_istream_base * >   _IStreamList;
    typedef std::vector< gr_ostream_base * >  _OStreamList;
...
// use incoming packet's time stamp to forward
      ...
      LOG_DEBUG( char_to_float_base, "                  mode:" <<  pktTs.tcmode );
      LOG_DEBUG( char_to_float_base, "                status:" <<  pktTs.tcstatus );
      LOG_DEBUG( char_to_float_base, "                offset:" <<  pktTs.toff );
      LOG_DEBUG( char_to_float_base, "                 whole:" <<  std::setprecision(10) << pktTs.twsec );
      LOG_DEBUG( char_to_float_base, "PKT - TS          frac:" <<  std::setprecision(12) << pktTs.tfsec );
#endif
