...
char_to_float_i::char_to_float_i(const char *uuid, const char *label) :
char_to_float_base(uuid, label)
{
}

char_to_float_i::~char_to_float_i()
{
}
...
void char_to_float_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_char_to_float( this->vlen, this->scale );
    } catch (...) {
        return;
    }

    this->registerGetterSetter("scale", &gr_char_to_float::scale, &gr_char_to_float::set_scale);
...
//      
BULKIO::StreamSRI char_to_float_i::createOutputSRI( int32_t idx)
{
    //
