...
#ifndef NULL_SOURCE_I_IMPL_BASE_H
#define NULL_SOURCE_I_IMPL_BASE_H
...
#include "null_source_i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class null_source_i_base;
...
class null_source_i_base : public GnuHawkBlock
{
    public:
        null_source_i_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<null_source_i_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( null_source_i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( null_source_i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( null_source_i_base, " serviceFunction: retval:" << retval);
