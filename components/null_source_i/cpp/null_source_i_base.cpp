...
null_source_i_base::null_source_i_base(const char *uuid, const char *label) :
...
void null_source_i_base::construct()
{
...
*******************************************************************************************/
void null_source_i_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void null_source_i_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    boost::mutex::scoped_lock lock(serviceThreadLock);
    if (serviceThread == 0) {
        serviceThread = new ProcessThread<null_source_i_base>(this, 0.1);
        serviceThread->start();
    }
...
void null_source_i_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    // release the child thread (if it exists)
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( null_source_i_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( null_source_i_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr null_source_i_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void null_source_i_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void null_source_i_base::loadProperties()
{
...
//  Allow for logging 
// 
PREPARE_LOGGING(null_source_i_base);
...
uint32_t null_source_i_base::getNOutputStreams() {
    return 0;
}

void null_source_i_base::setupIOMappings( )
{
    ...
    LOG_DEBUG( null_source_i_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
    ...
    if ( _ostreams.size() > 0 ) {
        ...
        LOG_DEBUG( null_source_i_base, "RESET OUTPUT SIGNATURE SIZE:" << _ostreams.size() );
    ...
    //
    LOG_TRACE( null_source_i_base, "setupIOMappings OutputPorts: " << outPorts.size() );
    ...
    for( int i=0; pname != outputPortOrder.end(); pname++ ) {
        ...
        if ( p_out != outPorts.end() ) {
            ...
            LOG_DEBUG( null_source_i_base, "ADDING OUTPUT MAP IDX:" << i << " SID:" << sid );
...
BULKIO::StreamSRI null_source_i_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI null_source_i_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    return createOutputSRI( oidx );
}

void null_source_i_base::adjustOutputRate(BULKIO::StreamSRI &sri )
{
    if ( validGRBlock() ) {
        double ret=sri.xdelta*gr_sptr->relative_rate();
/**
**/
        LOG_TRACE( null_source_i_base, "ADJUSTING SRI.XDELTA FROM/TO: " << sri.xdelta << "/" << ret );
        sri.xdelta = ret;
    }
}

null_source_i_base::TimeDuration null_source_i_base::getTargetDuration()
{
    ...
    LOG_TRACE( null_source_i_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
              " target_duration " << t_drate );
    return t_drate;
}

null_source_i_base::TimeDuration null_source_i_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time )
{
    TimeDuration delta;
    ...
    if ( start_time.is_not_a_date_time() == false ) {
        ...
        delta /= 4;
        LOG_TRACE( null_source_i_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
                  " delta " << delta );
    }
...
template < typename OUT_PORT_TYPE > int null_source_i_base::_generatorServiceFunction( typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams ) 
{
    ...
    if ( validGRBlock() == false ) {
        ...
        LOG_DEBUG( null_source_i_base, "FINISHED BUILDING  GNU RADIO BLOCK");
    }

    if ( !validGRBlock() || ostreams.size() == 0  ) {
        LOG_WARN( null_source_i_base, "NO OUTPUT STREAMS DEFINED FOR GNU RADIO BLOCK..." );
        return NOOP;
    }
    ...
    LOG_TRACE( null_source_i_base, " FORECAST == " << noutput_items );
    ...
    if (numOut != 0 or (eos and !sentEOS)){
        ...
        ostream = ostreams.begin();
        for ( ; ostream != ostreams.end(); ostream++ ) {
            LOG_TRACE( null_source_i_base, "PUSHING DATA   NOUT/NITEMS/OITEMS/STREAM_ID " << numOut << 
                      "/" << ostream->nitems()  << "/" << ostream->oitems() << "/" << ostream->streamID );
#ifdef TEST_TIME_STAMP
      LOG_DEBUG( null_source_i_base, "OSTREAM SRI:    xdelta:" << std::setprecision(12) << ostream->sri.xdelta );
      LOG_DEBUG( null_source_i_base, "OSTREAM WRITE:   maint:" << _maintainTimeStamp );
      LOG_DEBUG( null_source_i_base, "                  mode:" <<  ostream->tstamp.tcmode );
      LOG_DEBUG( null_source_i_base, "                status:" <<  ostream->tstamp.tcstatus );
      LOG_DEBUG( null_source_i_base, "                offset:" <<  ostream->tstamp.toff );
      LOG_DEBUG( null_source_i_base, "                 whole:" <<  std::setprecision(10) << ostream->tstamp.twsec );
      LOG_DEBUG( null_source_i_base, "                  frac:" <<  std::setprecision(12) << ostream->tstamp.tfsec );
#endif
