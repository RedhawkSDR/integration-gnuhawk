...
argmax_is_1i_i::argmax_is_1i_i(const char *uuid, const char *label) :
argmax_is_1i_base(uuid, label)
{
    setPropertyChangeListener("stream_id_map", this, &argmax_is_1i_i::streamIdChanged);
}

argmax_is_1i_i::~argmax_is_1i_i()
{
}

void argmax_is_1i_i::streamIdChanged(const std::string& id) {
...
void argmax_is_1i_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	gr_sptr = gr_make_argmax_is (this->vlen);
...
//      
BULKIO::StreamSRI argmax_is_1i_i::createOutputSRI( int32_t oidx, int32_t &in_idx, std::string &ext)
{
    //
