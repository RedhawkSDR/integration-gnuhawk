...
float_to_int_i::float_to_int_i(const char *uuid, const char *label) :
float_to_int_base(uuid, label)
{
}

float_to_int_i::~float_to_int_i()
{
}
...
void float_to_int_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_float_to_int( this->vlen, this->scale );

    this->registerGetterSetter("scale",
                               &gr_float_to_int::scale,
                               &gr_float_to_int::set_scale);
...
//      
BULKIO::StreamSRI float_to_int_i::createOutputSRI( int32_t idx)
{
    //
