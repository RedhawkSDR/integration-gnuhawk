...
adaptive_fir_ccf_i::adaptive_fir_ccf_i(const char *uuid, const char *label) :
adaptive_fir_ccf_base(uuid, label)
{
}

adaptive_fir_ccf_i::~adaptive_fir_ccf_i()
{
}

void adaptive_fir_ccf_i::set_taps(const std::string &id)
{
    this->gr_sptr->set_taps(this->taps);
}

std::vector<float> adaptive_fir_ccf_i::get_taps()
{
...
void adaptive_fir_ccf_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr::filter::adaptive_fir_ccf::make( this->name.c_str(), this->decimation, this->taps );

    this->registerGetValue("taps",
                         this,
                         &adaptive_fir_ccf_i::get_taps);

    this->setPropertyChangeListener("taps",
                                    this,
                                    &adaptive_fir_ccf_i::set_taps);
...
//      
BULKIO::StreamSRI adaptive_fir_ccf_i::createOutputSRI( int32_t idx)
{
    //
