...
#ifndef ADAPTIVE_FIR_CCF_IMPL_BASE_H
#define ADAPTIVE_FIR_CCF_IMPL_BASE_H
...
#include "adaptive_fir_ccf_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class adaptive_fir_ccf_base;
...
class adaptive_fir_ccf_base : public GnuHawkBlock
{
    public:
        adaptive_fir_ccf_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<adaptive_fir_ccf_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( adaptive_fir_ccf_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( adaptive_fir_ccf_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( adaptive_fir_ccf_base, " serviceFunction: retval:" << retval);
