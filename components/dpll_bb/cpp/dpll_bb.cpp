...
dpll_bb_i::dpll_bb_i(const char *uuid, const char *label) :
dpll_bb_base(uuid, label)
{
}
...
void dpll_bb_i::createBlock()
{
    ...
    try {
        gr_sptr = gr_make_dpll_bb( this->period, this->gain );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI dpll_bb_i::createOutputSRI( int32_t idx)
{
    //
