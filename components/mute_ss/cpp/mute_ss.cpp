...
mute_ss_i::mute_ss_i(const char *uuid, const char *label) :
mute_ss_base(uuid, label)
{
}
...
void mute_ss_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_mute_ss( this->mute);
    this->registerGetterSetter("mute",
                                &gr_mute_ss::mute,
                                &gr_mute_ss::set_mute);
...
//      
BULKIO::StreamSRI mute_ss_i::createOutputSRI( int32_t idx)
{
    //
