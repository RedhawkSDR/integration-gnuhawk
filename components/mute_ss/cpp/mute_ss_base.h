...
#ifndef MUTE_SS_IMPL_BASE_H
#define MUTE_SS_IMPL_BASE_H
...
#include "mute_ss_GnuHawkBlock.h"
...
class mute_ss_base : public GnuHawkBlock
{
    public:
        mute_ss_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<mute_ss_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( mute_ss_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( mute_ss_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( mute_ss_base, " serviceFunction: retval:" << retval);
