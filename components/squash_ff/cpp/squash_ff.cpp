...
squash_ff_i::squash_ff_i(const char *uuid, const char *label) :
squash_ff_base(uuid, label)
{
}

squash_ff_i::~squash_ff_i()
{
}
...
void squash_ff_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
    try {
        gr_sptr = wavelet_make_squash_ff( this->igrid, this->ogrid );
    } catch (...) {
        return;
    }
    // 
...
//      
BULKIO::StreamSRI squash_ff_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
