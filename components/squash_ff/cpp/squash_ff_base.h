...
#ifndef SQUASH_FF_IMPL_BASE_H
#define SQUASH_FF_IMPL_BASE_H
...
#include "squash_ff_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class squash_ff_base;
...
class squash_ff_base : public GnuHawkBlock
{
    public:
        squash_ff_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<squash_ff_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        std::vector<float> igrid;
        std::vector<float> ogrid;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( squash_ff_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( squash_ff_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( squash_ff_base, " serviceFunction: retval:" << retval);
