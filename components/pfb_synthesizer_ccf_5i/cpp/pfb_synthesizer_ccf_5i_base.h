...
#ifndef PFB_SYNTHESIZER_CCF_5I_IMPL_BASE_H
#define PFB_SYNTHESIZER_CCF_5I_IMPL_BASE_H
...
#include "pfb_synthesizer_ccf_5i_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class pfb_synthesizer_ccf_5i_base;
...
class pfb_synthesizer_ccf_5i_base : public GnuHawkBlock
{
    public:
        pfb_synthesizer_ccf_5i_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<pfb_synthesizer_ccf_5i_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::ULong numchans;
        bool twox;
        std::vector<float> taps;
        std::vector<float> channel_map;

        // Ports
        bulkio::InFloatPort *complex_in_0;
        bulkio::InFloatPort *complex_in_1;
        bulkio::InFloatPort *complex_in_2;
        bulkio::InFloatPort *complex_in_3;
        bulkio::InFloatPort *complex_in_4;
        bulkio::OutFloatPort *complex_out;
    ...
    protected:
        ...
        void  complex_in_0_newStreamCallback( BULKIO::StreamSRI &sri );

        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  complex_in_1_newStreamCallback( BULKIO::StreamSRI &sri );

        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  complex_in_2_newStreamCallback( BULKIO::StreamSRI &sri );

        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  complex_in_3_newStreamCallback( BULKIO::StreamSRI &sri );

        // callback when a new Stream ID is detected on the port so we can add to istream/ostream mapping list
        void  complex_in_4_newStreamCallback( BULKIO::StreamSRI &sri );
    ...
    typedef _OStream< bulkio::OutFloatPort >::List   OStreamList;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( pfb_synthesizer_ccf_5i_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( pfb_synthesizer_ccf_5i_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( pfb_synthesizer_ccf_5i_base, " serviceFunction: retval:" << retval);
