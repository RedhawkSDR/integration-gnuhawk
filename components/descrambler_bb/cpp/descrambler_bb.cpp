...
descrambler_bb_i::descrambler_bb_i(const char *uuid, const char *label) :
descrambler_bb_base(uuid, label)
{
}

descrambler_bb_i::~descrambler_bb_i()
{
}
...
void descrambler_bb_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = digital_make_descrambler_bb( this->mask, this->seed, this->len );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI descrambler_bb_i::createOutputSRI( int32_t idx)
{
    //
