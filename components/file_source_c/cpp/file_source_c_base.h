...
#ifndef FILE_SOURCE_C_IMPL_BASE_H
#define FILE_SOURCE_C_IMPL_BASE_H
...
#include "file_source_c_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class file_source_c_base;
...
class file_source_c_base : public GnuHawkBlock
{
    public:
        file_source_c_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<file_source_c_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::Long itemsize;
        bool repeat;
        CORBA::Long seek_point;
        CORBA::Long whence;
        std::string filename;
        bool seek;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( file_source_c_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( file_source_c_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( file_source_c_base, " serviceFunction: retval:" << retval);
