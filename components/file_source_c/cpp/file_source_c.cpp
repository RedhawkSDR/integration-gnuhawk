...
file_source_c_i::file_source_c_i(const char *uuid, const char *label) :
file_source_c_base(uuid, label)
{
}

file_source_c_i::~file_source_c_i()
{
}

void file_source_c_i::changedSeek(const std::string &id){
	this->seek = gr_sptr->seek(this->seek_point, this->whence);
}

bool file_source_c_i::getSeek(){
	return this->seek;
}
...
void file_source_c_i::createBlock()
{
    //
	...
	this->setPropertyChangeListener("seek_point", this, &file_source_c_i::changedSeek);
	this->setPropertyChangeListener("whence", this, &file_source_c_i::changedSeek);
    // 
...
//      
BULKIO::StreamSRI file_source_c_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
