...
file_source_c_base::file_source_c_base(const char *uuid, const char *label) :
...
void file_source_c_base::construct()
{
...
*******************************************************************************************/
void file_source_c_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void file_source_c_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    boost::mutex::scoped_lock lock(serviceThreadLock);
    if (serviceThread == 0) {
        serviceThread = new ProcessThread<file_source_c_base>(this, 0.1);
        serviceThread->start();
    }
...
void file_source_c_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    // release the child thread (if it exists)
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( file_source_c_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( file_source_c_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr file_source_c_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void file_source_c_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void file_source_c_base::loadProperties()
{
...
//  Allow for logging 
// 
PREPARE_LOGGING(file_source_c_base);
...
uint32_t file_source_c_base::getNOutputStreams() {
    return 0;
}

void file_source_c_base::setupIOMappings( )
{
    ...
    LOG_DEBUG( file_source_c_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
    ...
    if ( _ostreams.size() > 0 ) {
        ...
        LOG_DEBUG( file_source_c_base, "RESET OUTPUT SIGNATURE SIZE:" << _ostreams.size() );
    ...
    //
    LOG_TRACE( file_source_c_base, "setupIOMappings OutputPorts: " << outPorts.size() );
    ...
    for( int i=0; pname != outputPortOrder.end(); pname++ ) {
        ...
        if ( p_out != outPorts.end() ) {
            ...
            LOG_DEBUG( file_source_c_base, "ADDING OUTPUT MAP IDX:" << i << " SID:" << sid );
...
BULKIO::StreamSRI file_source_c_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI file_source_c_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    return createOutputSRI( oidx );
}

void file_source_c_base::adjustOutputRate(BULKIO::StreamSRI &sri )
{
    if ( validGRBlock() ) {
        double ret=sri.xdelta*gr_sptr->relative_rate();
/**
**/
        LOG_TRACE( file_source_c_base, "ADJUSTING SRI.XDELTA FROM/TO: " << sri.xdelta << "/" << ret );
        sri.xdelta = ret;
    }
}

file_source_c_base::TimeDuration file_source_c_base::getTargetDuration()
{
    ...
    LOG_TRACE( file_source_c_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
              " target_duration " << t_drate );
    return t_drate;
}

file_source_c_base::TimeDuration file_source_c_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time )
{
    TimeDuration delta;
    ...
    if ( start_time.is_not_a_date_time() == false ) {
        ...
        delta /= 4;
        LOG_TRACE( file_source_c_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
                  " delta " << delta );
    }
...
template < typename OUT_PORT_TYPE > int file_source_c_base::_generatorServiceFunction( typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams ) 
{
    ...
    if ( validGRBlock() == false ) {
        ...
        LOG_DEBUG( file_source_c_base, "FINISHED BUILDING  GNU RADIO BLOCK");
    }

    if ( !validGRBlock() || ostreams.size() == 0  ) {
        LOG_WARN( file_source_c_base, "NO OUTPUT STREAMS DEFINED FOR GNU RADIO BLOCK..." );
        return NOOP;
    }
    ...
    LOG_TRACE( file_source_c_base, " FORECAST == " << noutput_items );
    ...
    if (numOut != 0 or (eos and !sentEOS)){
        ...
        ostream = ostreams.begin();
        for ( ; ostream != ostreams.end(); ostream++ ) {
            LOG_TRACE( file_source_c_base, "PUSHING DATA   NOUT/NITEMS/OITEMS/STREAM_ID " << numOut << 
                      "/" << ostream->nitems()  << "/" << ostream->oitems() << "/" << ostream->streamID );
#ifdef TEST_TIME_STAMP
      LOG_DEBUG( file_source_c_base, "OSTREAM SRI:    xdelta:" << std::setprecision(12) << ostream->sri.xdelta );
      LOG_DEBUG( file_source_c_base, "OSTREAM WRITE:   maint:" << _maintainTimeStamp );
      LOG_DEBUG( file_source_c_base, "                  mode:" <<  ostream->tstamp.tcmode );
      LOG_DEBUG( file_source_c_base, "                status:" <<  ostream->tstamp.tcstatus );
      LOG_DEBUG( file_source_c_base, "                offset:" <<  ostream->tstamp.toff );
      LOG_DEBUG( file_source_c_base, "                 whole:" <<  std::setprecision(10) << ostream->tstamp.twsec );
      LOG_DEBUG( file_source_c_base, "                  frac:" <<  std::setprecision(12) << ostream->tstamp.tfsec );
#endif
