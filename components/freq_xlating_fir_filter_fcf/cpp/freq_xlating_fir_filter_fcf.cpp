...
freq_xlating_fir_filter_fcf_i::freq_xlating_fir_filter_fcf_i(const char *uuid, const char *label) :
freq_xlating_fir_filter_fcf_base(uuid, label)
{
}

freq_xlating_fir_filter_fcf_i::~freq_xlating_fir_filter_fcf_i()
{
}

void freq_xlating_fir_filter_fcf_i::set_taps(const std::string &id)
{
    this->gr_sptr->set_taps(this->taps);
}

std::vector<float> freq_xlating_fir_filter_fcf_i::get_taps()
{
...
void freq_xlating_fir_filter_fcf_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr::filter::freq_xlating_fir_filter_fcf::make( this->decimation, this->taps, this->center_freq, this->sampling_freq );

    this->registerGetterSetter("center_freq",
                               &gr::filter::freq_xlating_fir_filter_fcf::center_freq,
                               &gr::filter::freq_xlating_fir_filter_fcf::set_center_freq);

    this->setPropertyChangeListener("taps",
                                    this,
                                    &freq_xlating_fir_filter_fcf_i::set_taps);
    this->registerGetValue("taps",
                           this,
                           &freq_xlating_fir_filter_fcf_i::get_taps);
...
//      
BULKIO::StreamSRI freq_xlating_fir_filter_fcf_i::createOutputSRI( int32_t idx)
{
    //
