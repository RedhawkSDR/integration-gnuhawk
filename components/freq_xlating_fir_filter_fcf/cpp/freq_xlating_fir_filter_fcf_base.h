...
#ifndef FREQ_XLATING_FIR_FILTER_FCF_IMPL_BASE_H
#define FREQ_XLATING_FIR_FILTER_FCF_IMPL_BASE_H
...
#include "freq_xlating_fir_filter_fcf_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class freq_xlating_fir_filter_fcf_base;
...
class freq_xlating_fir_filter_fcf_base : public GnuHawkBlock
{
    public:
        freq_xlating_fir_filter_fcf_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<freq_xlating_fir_filter_fcf_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( freq_xlating_fir_filter_fcf_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( freq_xlating_fir_filter_fcf_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( freq_xlating_fir_filter_fcf_base, " serviceFunction: retval:" << retval);
