...
peak_detector_fb_i::peak_detector_fb_i(const char *uuid, const char *label) :
peak_detector_fb_base(uuid, label)
{
}

peak_detector_fb_i::~peak_detector_fb_i()
{
}

// Getters
float peak_detector_fb_i::get_threshold_factor_rise()
{
    return gr_sptr->threshold_factor_rise();
}

float peak_detector_fb_i::get_threshold_factor_fall()
{
    return gr_sptr->threshold_factor_fall();
}

int peak_detector_fb_i::get_look_ahead()
{
    return gr_sptr->look_ahead();
}

float peak_detector_fb_i::get_alpha()
{
...
void peak_detector_fb_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_peak_detector_fb( this->threshold_factor_rise, this->threshold_factor_fall, this->look_ahead, this->alpha );
    } catch (...) {
        return;
    }

    this->registerSetter("threshold_factor_rise", &gr_peak_detector_fb::set_threshold_factor_rise);
    this->registerSetter("threshold_factor_fall", &gr_peak_detector_fb::set_threshold_factor_fall);
    this->registerSetter("look_ahead", &gr_peak_detector_fb::set_look_ahead);
    this->registerSetter("alpha", &gr_peak_detector_fb::set_alpha);

    // Note, need to use redhawk getValue because GNURadio versions are not const
    this->registerGetValue("threshold_factor_rise", this, &peak_detector_fb_i::get_threshold_factor_rise);
    this->registerGetValue("threshold_factor_fall", this, &peak_detector_fb_i::get_threshold_factor_fall);
    this->registerGetValue("look_ahead", this, &peak_detector_fb_i::get_look_ahead);
    this->registerGetValue("alpha", this, &peak_detector_fb_i::get_alpha);
...
//      
BULKIO::StreamSRI peak_detector_fb_i::createOutputSRI( int32_t idx)
{
    //
