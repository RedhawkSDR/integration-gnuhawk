...
#include "stream_to_streams_cc_4o.h"
...
stream_to_streams_cc_4o_i::stream_to_streams_cc_4o_i(const char *uuid, const char *label) :
stream_to_streams_cc_4o_base(uuid, label)
{
    setPropertyChangeListener ("stream_id_map" , this, &stream_to_streams_cc_4o_i::streamIdChanged );
}

stream_to_streams_cc_4o_i::~stream_to_streams_cc_4o_i()
{
}

void stream_to_streams_cc_4o_i::streamIdChanged(const std::string& id)
{ 
...
void stream_to_streams_cc_4o_i::createBlock()
{
    //
    //

    try{
    	this->gr_sptr = gr_make_stream_to_streams(this->item_size, this->nstreams);
    }
    catch(...){
    	return;
    }
...
//      
BULKIO::StreamSRI stream_to_streams_cc_4o_i::createOutputSRI( int32_t oidx, int32_t &in_idx, std::string &ext)
{
    //
