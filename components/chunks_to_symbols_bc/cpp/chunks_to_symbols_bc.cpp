...
#include "chunks_to_symbols_bc.h"
...
chunks_to_symbols_bc_i::chunks_to_symbols_bc_i(const char *uuid, const char *label) :
chunks_to_symbols_bc_base(uuid, label)
{
}

chunks_to_symbols_bc_i::~chunks_to_symbols_bc_i()
{
}
...
void chunks_to_symbols_bc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = gr_make_chunks_to_symbols_bc( this->symbol_table, this->D );

    this->registerGetter("symbol_table", &gr_chunks_to_symbols_bc::symbol_table);
    this->registerGetter("D", &gr_chunks_to_symbols_bc::D);
...
//      
BULKIO::StreamSRI chunks_to_symbols_bc_i::createOutputSRI( int32_t idx)
{
    //
