...
moving_average_cc_i::moving_average_cc_i(const char *uuid, const char *label) :
moving_average_cc_base(uuid, label)
{
}

moving_average_cc_i::~moving_average_cc_i()
{
}

void moving_average_cc_i::set_length_and_scale(const std::string &id)
{
    gr_sptr->set_length_and_scale(this->length, this->length);
}
...
void moving_average_cc_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_moving_average_cc( this->length, this->scale, this->max_iter );
    } catch (...) {
        return;
    }

    this->registerGetter("length", &gr_moving_average_cc::length);
    this->registerGetter("scale", &gr_moving_average_cc::scale);

    this->setPropertyChangeListener("length", this, &moving_average_cc_i::set_length_and_scale);
    this->setPropertyChangeListener("scale", this, &moving_average_cc_i::set_length_and_scale);
...
//      
BULKIO::StreamSRI moving_average_cc_i::createOutputSRI( int32_t idx)
{
    //
