...
fir_filter_fsf_i::fir_filter_fsf_i(const char *uuid, const char *label) :
fir_filter_fsf_base(uuid, label)
{
}

fir_filter_fsf_i::~fir_filter_fsf_i()
{
}

void fir_filter_fsf_i::setTaps(const std::string &id)
{
    this->gr_sptr->set_taps(this->taps);
}

std::vector<float> fir_filter_fsf_i::getTaps(){
...
//
void fir_filter_fsf_i::createBlock()
{
  //
...
//      
BULKIO::StreamSRI fir_filter_fsf_i::createOutputSRI( int32_t idx)
{
  //
