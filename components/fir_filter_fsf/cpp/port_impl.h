...
//RESOLVE    ENABLE_LOGGING;
    public:
        ...
        BULKIO_dataShort_Out_i(std::string port_name, fir_filter_fsf_base *_parent);
        ~BULKIO_dataShort_Out_i();

	     void pushSRI(const BULKIO::StreamSRI& H);
     typedef CORBA::Short    RH_TransportType;
     typedef Int16         RH_NativeType;
       ...
       template <typename ALLOCATOR>
        void pushPacket(std::vector<CORBA::Short, ALLOCATOR>& data, BULKIO::PrecisionUTCTime& T, bool EOS, const std::string& streamID) {
				 ...
				 for (port = outConnections.begin(); port != outConnections.end(); port++) {
					 ...
					 //RESOLVE LOG_ERROR(BULKIO_dataShort_Out_i, "Call to pushPacket by BULKIO_dataShort_Out_i failed");
				     }
				 }
			     }
			 };
    ...
    public:
     ...
     class SRIListener {
       ...
       public:
          ...
          virtual void operator() ( BULKIO_dataFloat_In_i *, BULKIO::StreamSRI &  ) = 0;
     };
      
     //BULKIO_dataFloat_In_i(std::string port_name, fir_filter_fsf_base *_parent);
     BULKIO_dataFloat_In_i(std::string port_name, SRIListener *listener=NULL, SRICompare cmp=CompareSRI);
     ~BULKIO_dataFloat_In_i();  
        ...
        void pushPacket(const PortTypes::FloatSequence& data, const BULKIO::PrecisionUTCTime& T, CORBA::Boolean EOS, const char* streamID);
     	...
     	typedef CORBA::Float         RH_TransportType;
        typedef Float        RH_NativeType;
