...
#ifndef SIMPLE_FRAMER_IMPL_BASE_H
#define SIMPLE_FRAMER_IMPL_BASE_H
...
#include "simple_framer_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class simple_framer_base;
...
class simple_framer_base : public GnuHawkBlock
{
    public:
        simple_framer_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<simple_framer_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::Long payload_bytesize;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( simple_framer_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( simple_framer_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( simple_framer_base, " serviceFunction: retval:" << retval);
