...
PREPARE_LOGGING(correlate_access_code_bb_i);

correlate_access_code_bb_i::correlate_access_code_bb_i(const char *uuid, const char *label) :
correlate_access_code_bb_base(uuid, label)
{
}

correlate_access_code_bb_i::~correlate_access_code_bb_i()
{
}


void correlate_access_code_bb_i::set_access_code(const std::string &id)
{
    bool ret_val = gr_sptr->set_access_code( this->access_code );

    if (!ret_val) {
        LOG_WARN(correlate_access_code_bb_i, "Could not set access_code");
    }
}
...
void correlate_access_code_bb_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    gr_sptr = digital_make_correlate_access_code_bb( this->access_code, this->threshold );

    this->setPropertyChangeListener("access_code", this, &correlate_access_code_bb_i::set_access_code);
...
//      
BULKIO::StreamSRI correlate_access_code_bb_i::createOutputSRI( int32_t idx)
{
    //
