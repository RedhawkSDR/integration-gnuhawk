...
add_const_vcc_i::add_const_vcc_i(const char *uuid, const char *label) :
add_const_vcc_base(uuid, label)
{
}

add_const_vcc_i::~add_const_vcc_i()
{
}


void add_const_vcc_i::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
  // setup object for processing
  add_const_vcc_base::initialize();

}

void add_const_vcc_i::setK( const std::string &id)
{
  std::cout << " setConst :" << std::endl;
  if ( validGRBlock() ) {
    std::cout << "calling add_const_vcc setK"  << std::endl;
    std::copy( k.begin(), k.end(), std::ostream_iterator< std::complex<float> >(std::cout,"--") );
    std::cout << std::endl;
    this->gr_sptr->set_k(k);
  }
}

std::vector<gr_complex> add_const_vcc_i::getK()
{
  std::vector<gr_complex> ret;
  if ( validGRBlock() ) return this->gr_sptr->k();
  return ret;
}
...
void add_const_vcc_i::createBlock()
{
  // need to have at least 1 item in the list... bad things happend if not
  if ( k.size() < 1 )  return;

  try {
    gr_sptr = gr_make_add_const_vcc( k );
  }
  catch(...) {
    return;
  }

  //  setPropertyChangeListener("k", this, &add_const_vcc_i::setK);
  //  registerGetValue("k", this, &add_const_vcc_i::getK);

  this->registerGetterSetter("k",
			     &gr_add_const_vcc::k,
			     &gr_add_const_vcc::set_k);
...
//      
BULKIO::StreamSRI add_const_vcc_i::createOutputSRI( int32_t idx)
{
    //
