...
throttle_bb_i::throttle_bb_i(const char *uuid, const char *label) :
throttle_bb_base(uuid, label)
{
}

throttle_bb_i::~throttle_bb_i()
{
}

void throttle_bb_i::changeFrequency(const std::string &id)
{
    boost::mutex::scoped_lock lock(serviceThreadLock);
    BULKIO::StreamSRI sri = BULKIO::StreamSRI();
    sri.hversion = 1;
    sri.xstart = 0.0;
    if (this->samples_per_sec > 0.0)
        sri.xdelta = 1/this->samples_per_sec;
    else
        sri.xdelta = 1.0;
...
void throttle_bb_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_throttle( this->itemsize, this->samples_per_sec );
    } catch (...) {
        return;
    }

    this->registerSetter("samples_per_sec", &gr_throttle::set_sample_rate);
    this->setPropertyChangeListener("samples_per_sec", this, &throttle_bb_i::changeFrequency);
...
//      
BULKIO::StreamSRI throttle_bb_i::createOutputSRI( int32_t idx)
{
    //
