...
#ifndef DECODE_CCSDS_27_FB_IMPL_BASE_H
#define DECODE_CCSDS_27_FB_IMPL_BASE_H
...
#include "decode_ccsds_27_fb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class decode_ccsds_27_fb_base;
...
class decode_ccsds_27_fb_base : public GnuHawkBlock
{
    public:
        decode_ccsds_27_fb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<decode_ccsds_27_fb_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( decode_ccsds_27_fb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( decode_ccsds_27_fb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( decode_ccsds_27_fb_base, " serviceFunction: retval:" << retval);
