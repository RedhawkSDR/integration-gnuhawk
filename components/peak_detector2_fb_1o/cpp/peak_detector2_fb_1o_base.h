...
#ifndef PEAK_DETECTOR2_FB_1O_IMPL_BASE_H
#define PEAK_DETECTOR2_FB_1O_IMPL_BASE_H
...
#include "peak_detector2_fb_1o_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class peak_detector2_fb_1o_base;
...
class peak_detector2_fb_1o_base : public GnuHawkBlock
{
    public:
        peak_detector2_fb_1o_base(const char *uuid, const char *label);
        ...
        ~peak_detector2_fb_1o_base();

        void loadProperties();

    protected:
        ProcessThread<peak_detector2_fb_1o_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( peak_detector2_fb_1o_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( peak_detector2_fb_1o_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( peak_detector2_fb_1o_base, " serviceFunction: retval:" << retval);
