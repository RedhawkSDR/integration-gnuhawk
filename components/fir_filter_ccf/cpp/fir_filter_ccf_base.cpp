...
void fir_filter_ccf_base::construct()
{
    ...
    complex_in->setNewStreamListener(this, &fir_filter_ccf_base::complex_in_newStreamCallback);
...
void fir_filter_ccf_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        complex_in->unblock();
        serviceThread = new ProcessThread<fir_filter_ccf_base>(this, 0.1);
        serviceThread->start();
    }
...
void fir_filter_ccf_base::complex_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( fir_filter_ccf_base, "START NotifySRI  port:stream " << complex_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( complex_in, sri ) );

  LOG_TRACE( fir_filter_ccf_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << complex_in->getName() << "/" << sri.streamID); 
}
...
BULKIO::StreamSRI fir_filter_ccf_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI fir_filter_ccf_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > istream->nitems() ) {
                        LOG_WARN( fir_filter_ccf_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE "
                                 ...
                                 typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( fir_filter_ccf_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << 
        ...
        if ( !gr_sptr->fixed_rate() )  {
            ...
            LOG_TRACE( fir_filter_ccf_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( fir_filter_ccf_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << 
        ...
        if ( !dataReady ) {
            LOG_TRACE( fir_filter_ccf_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( fir_filter_ccf_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems 
