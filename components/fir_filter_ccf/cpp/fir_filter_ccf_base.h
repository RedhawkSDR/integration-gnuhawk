...
#ifndef FIR_FILTER_CCF_IMPL_BASE_H
#define FIR_FILTER_CCF_IMPL_BASE_H
...
#include "fir_filter_ccf_GnuHawkBlock.h"
#include "gr_malloc.h"

class fir_filter_ccf_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  fir_filter_ccf_base;
  ...
  SRIListener( fir_filter_ccf_base &parent ) : _parent(parent) {};

    fir_filter_ccf_base &_parent;
  };
    

    public:

        fir_filter_ccf_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<fir_filter_ccf_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( fir_filter_ccf_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( fir_filter_ccf_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( fir_filter_ccf_base, " serviceFunction: retval:" << retval);
