...
fpll_ff_i::fpll_ff_i(const char *uuid, const char *label) :
fpll_ff_base(uuid, label)
{
}
...
void fpll_ff_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	try{
        gr_sptr = atsc_make_fpll();
	}
	catch (...) {
...
//      
BULKIO::StreamSRI fpll_ff_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
