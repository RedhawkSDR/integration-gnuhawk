...
ulaw_decode_bs_i::ulaw_decode_bs_i(const char *uuid, const char *label) :
ulaw_decode_bs_base(uuid, label)
{
}

ulaw_decode_bs_i::~ulaw_decode_bs_i()
{
}
...
void ulaw_decode_bs_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = vocoder_make_ulaw_decode_bs();
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI ulaw_decode_bs_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
