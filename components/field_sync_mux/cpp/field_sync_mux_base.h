...
#ifndef FIELD_SYNC_MUX_IMPL_BASE_H
#define FIELD_SYNC_MUX_IMPL_BASE_H
...
#include "field_sync_mux_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class field_sync_mux_base;
...
class field_sync_mux_base : public GnuHawkBlock
{
    public:
        field_sync_mux_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<field_sync_mux_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Ports
        bulkio::InOctetPort *atsc_data_segment_in;
    ...
    protected:
        ...
        void  atsc_data_segment_in_newStreamCallback( BULKIO::StreamSRI &sri );
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( field_sync_mux_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( field_sync_mux_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( field_sync_mux_base, " serviceFunction: retval:" << retval);
