...
field_sync_mux_i::field_sync_mux_i(const char *uuid, const char *label) :
field_sync_mux_base(uuid, label)
{
}

field_sync_mux_i::~field_sync_mux_i()
{
}
...
void field_sync_mux_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = atsc_make_field_sync_mux();
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI field_sync_mux_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
