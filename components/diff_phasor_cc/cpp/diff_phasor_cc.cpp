...
diff_phasor_cc_i::diff_phasor_cc_i(const char *uuid, const char *label) :
diff_phasor_cc_base(uuid, label)
{
}

diff_phasor_cc_i::~diff_phasor_cc_i()
{
}
...
void diff_phasor_cc_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
    try {
        gr_sptr = digital_make_diff_phasor_cc( );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI diff_phasor_cc_i::createOutputSRI( int32_t idx)
{
    //
