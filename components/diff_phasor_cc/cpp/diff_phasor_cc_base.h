...
#ifndef DIFF_PHASOR_CC_IMPL_BASE_H
#define DIFF_PHASOR_CC_IMPL_BASE_H
...
#include "diff_phasor_cc_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class diff_phasor_cc_base;
...
class diff_phasor_cc_base : public GnuHawkBlock
{
    public:
        diff_phasor_cc_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<diff_phasor_cc_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( diff_phasor_cc_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( diff_phasor_cc_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( diff_phasor_cc_base, " serviceFunction: retval:" << retval);
