...
#ifndef UCHAR_TO_FLOAT_IMPL_BASE_H
#define UCHAR_TO_FLOAT_IMPL_BASE_H
...
#include "uchar_to_float_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class uchar_to_float_base;
...
class uchar_to_float_base : public GnuHawkBlock
{
    public:
        uchar_to_float_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<uchar_to_float_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( uchar_to_float_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( uchar_to_float_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( uchar_to_float_base, " serviceFunction: retval:" << retval);
