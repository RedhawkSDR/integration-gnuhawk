...
add_const_ss_i::add_const_ss_i(const char *uuid, const char *label) :
add_const_ss_base(uuid, label)
{
}

add_const_ss_i::~add_const_ss_i()
{
}
...
//
void add_const_ss_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_add_const_ss(k);
  ...
  setupIOMappings();
  this->registerGetterSetter("k", &gr_add_const_ss::k, &gr_add_const_ss::set_k);
} 
...
//      
BULKIO::StreamSRI add_const_ss_i::createOutputSRI( int32_t idx)
{
  //
