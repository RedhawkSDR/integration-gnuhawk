...
noise_source_c_i::noise_source_c_i(const char *uuid, const char *label) :
noise_source_c_base(uuid, label)
{
}

noise_source_c_i::~noise_source_c_i()
{
}
...
//
void noise_source_c_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_noise_source_c((gr_noise_type_t)type, amplitude, seed);
  this->registerGetterSetter("type", &gr_noise_source_c::type, &gr_noise_source_c::set_type);
  this->registerGetterSetter("amplitude", &gr_noise_source_c::amplitude, &gr_noise_source_c::set_amplitude);
  this->setPropertyChangeListener("sampling_freq", this, &noise_source_c_i::changedSamplingFreq);
...
//      
BULKIO::StreamSRI noise_source_c_i::createOutputSRI( int32_t idx)
{
  //
...
void noise_source_c_i::changedSamplingFreq(const std::string &id)
{
