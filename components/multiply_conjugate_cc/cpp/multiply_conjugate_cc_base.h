...
#ifndef MULTIPLY_CONJUGATE_CC_IMPL_BASE_H
#define MULTIPLY_CONJUGATE_CC_IMPL_BASE_H
...
#include "multiply_conjugate_cc_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class multiply_conjugate_cc_base;
...
class multiply_conjugate_cc_base : public GnuHawkBlock
{
    public:
        multiply_conjugate_cc_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<multiply_conjugate_cc_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( multiply_conjugate_cc_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( multiply_conjugate_cc_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( multiply_conjugate_cc_base, " serviceFunction: retval:" << retval);
