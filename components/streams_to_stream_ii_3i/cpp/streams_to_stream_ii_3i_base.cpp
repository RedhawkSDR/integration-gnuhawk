...
#include "streams_to_stream_ii_3i_base.h"
...
streams_to_stream_ii_3i_base::streams_to_stream_ii_3i_base(const char *uuid, const char *label) :
...
void streams_to_stream_ii_3i_base::construct()
{
    ...
    long_in_0->setNewStreamListener(this, &streams_to_stream_ii_3i_base::long_in_0_newStreamCallback);
    oid = ossie::corba::RootPOA()->activate_object(long_in_0);
    long_in_1 = new bulkio::InLongPort("long_in_1");
    long_in_1->setNewStreamListener(this, &streams_to_stream_ii_3i_base::long_in_1_newStreamCallback);
    oid = ossie::corba::RootPOA()->activate_object(long_in_1);
    long_in_2 = new bulkio::InLongPort("long_in_2");
    long_in_2->setNewStreamListener(this, &streams_to_stream_ii_3i_base::long_in_2_newStreamCallback);
...
*******************************************************************************************/
void streams_to_stream_ii_3i_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void streams_to_stream_ii_3i_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        ...
        long_in_1->unblock();
        long_in_2->unblock();
        serviceThread = new ProcessThread<streams_to_stream_ii_3i_base>(this, 0.1);
        serviceThread->start();
    }
...
void streams_to_stream_ii_3i_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( streams_to_stream_ii_3i_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( streams_to_stream_ii_3i_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr streams_to_stream_ii_3i_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void streams_to_stream_ii_3i_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void streams_to_stream_ii_3i_base::loadProperties()
{
...
//  Allow for logging 
// 
PREPARE_LOGGING(streams_to_stream_ii_3i_base);
...
uint32_t streams_to_stream_ii_3i_base::getNOutputStreams() {
    return 0;
}

void streams_to_stream_ii_3i_base::setupIOMappings( )
{
    ...
    LOG_DEBUG( streams_to_stream_ii_3i_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
    ...
    if ( _istreams.size() > 0 || _ostreams.size() > 0 ) {
        ...
        LOG_DEBUG( streams_to_stream_ii_3i_base, "RESET INPUT SIGNATURE SIZE:" << _istreams.size() );
        IStreamList::iterator istream;
        for ( int idx=0 ; istream != _istreams.end(); idx++, istream++ ) {
            // re-add existing stream definitons
            LOG_DEBUG(  streams_to_stream_ii_3i_base, "ADD READ INDEX TO GNU RADIO BLOCK");
        ...
        LOG_DEBUG( streams_to_stream_ii_3i_base, "RESET OUTPUT SIGNATURE SIZE:" << _ostreams.size() );
   ...
   //
    LOG_TRACE( streams_to_stream_ii_3i_base, "setupIOMappings INPUT PORTS: " << inPorts.size() );
    ...
    for( int i=0; pname != inputPortOrder.end(); pname++ ) {
        ...
        if ( p_in != inPorts.end() ) {
            ...
            LOG_DEBUG( streams_to_stream_ii_3i_base, "ADDING INPUT MAP IDX:" << i << " SID:" << sid );
    ...
    // streams over a single connection.  We are mapping a single RH Port to a single GNU Radio stream.
    //
    LOG_TRACE( streams_to_stream_ii_3i_base, "setupIOMappings OutputPorts: " << outPorts.size() );
    ...
    for( int i=0; pname != outputPortOrder.end(); pname++ ) {
        ...
        if ( p_out != outPorts.end() ) {
            ...
            LOG_DEBUG( streams_to_stream_ii_3i_base, "ADDING OUTPUT MAP IDX:" << i << " SID:" << sid );
...
void streams_to_stream_ii_3i_base::long_in_0_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( streams_to_stream_ii_3i_base, "START NotifySRI  port:stream " << long_in_0->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( long_in_0, sri ) );

  LOG_TRACE( streams_to_stream_ii_3i_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << long_in_0->getName() << "/" << sri.streamID); 
}

void streams_to_stream_ii_3i_base::long_in_1_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( streams_to_stream_ii_3i_base, "START NotifySRI  port:stream " << long_in_1->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( long_in_1, sri ) );

  LOG_TRACE( streams_to_stream_ii_3i_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << long_in_1->getName() << "/" << sri.streamID); 
}

void streams_to_stream_ii_3i_base::long_in_2_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( streams_to_stream_ii_3i_base, "START NotifySRI  port:stream " << long_in_2->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( long_in_2, sri ) );

  LOG_TRACE( streams_to_stream_ii_3i_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << long_in_2->getName() << "/" << sri.streamID); 
}


void streams_to_stream_ii_3i_base::processStreamIdChanges()
{
    boost::mutex::scoped_lock lock(_sriMutex);

    LOG_TRACE( streams_to_stream_ii_3i_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
    ...
    if ( validGRBlock() ) {
        ...
        for ( ; item != _sriQueue.end(); item++ ) {
            ...
            for ( ; istream != _istreams.end(); idx++, istream++ ) {
                ...
                if ( istream->port == item->first ) {
                    LOG_DEBUG( streams_to_stream_ii_3i_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
                    istream->sri(true);
                    istream->spe(mode);

                    LOG_DEBUG( streams_to_stream_ii_3i_base,  "  SETTING  OUT_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
        ...
        LOG_WARN( streams_to_stream_ii_3i_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
    }

}

BULKIO::StreamSRI streams_to_stream_ii_3i_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI streams_to_stream_ii_3i_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    return createOutputSRI( oidx );
}

void streams_to_stream_ii_3i_base::adjustOutputRate(BULKIO::StreamSRI &sri )
{
    if ( validGRBlock() ) {
...
**/
        LOG_TRACE( streams_to_stream_ii_3i_base, "ADJUSTING SRI.XDELTA FROM/TO: " << sri.xdelta << "/" << ret );
        sri.xdelta = ret;
    }
}

streams_to_stream_ii_3i_base::TimeDuration streams_to_stream_ii_3i_base::getTargetDuration()
{
    ...
    LOG_TRACE( streams_to_stream_ii_3i_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
              " target_duration " << t_drate );
    return t_drate;
}

streams_to_stream_ii_3i_base::TimeDuration streams_to_stream_ii_3i_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time )
{
    TimeDuration delta;
    ...
    if ( start_time.is_not_a_date_time() == false ) {
        ...
        delta /= 4;
        LOG_TRACE( streams_to_stream_ii_3i_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
                  " delta " << delta );
    }
...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int streams_to_stream_ii_3i_base::_transformerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
    typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
    ...
    if ( validGRBlock() == false ) {
        ...
        LOG_DEBUG( streams_to_stream_ii_3i_base, " FINISHED BUILDING  GNU RADIO BLOCK");
    }
    ...
    if ( !validGRBlock() || istreams.size() == 0 || ostreams.size() == 0  ) {
        LOG_WARN( streams_to_stream_ii_3i_base, "NO STREAMS ATTACHED TO BLOCK..." );
        return NOOP;
    }
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
        ...
        if ( istream->overrun() ) {
            LOG_WARN( streams_to_stream_ii_3i_base, " NOT KEEPING UP WITH STREAM ID:" << istream->streamID );
        }

        if ( istream->sriChanged() ) {
            // RESOLVE - need to look at how SRI changes can affect Gnu Radio BLOCK state
            LOG_DEBUG( streams_to_stream_ii_3i_base, "SRI CHANGED, STREAMD IDX/ID: " 
                      << idx << "/" << istream->pkt->streamID );
            setOutputStreamSRI( idx, istream->pkt->SRI );
        }
    }

    LOG_TRACE( streams_to_stream_ii_3i_base, "READ NITEMS: "  << nitems );
    ...
    while ( nout > -1 && serviceThread->threadRunning() ) {
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > istream->nitems() ) {
                        LOG_WARN( streams_to_stream_ii_3i_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE "
                                 << nitems << "/" << istream->nitems() );
                        nitems = istream->nitems();
                    }
                    istream->consume( nitems );
                    LOG_TRACE( streams_to_stream_ii_3i_base, " CONSUME READ DATA  ITEMS/REMAIN " << nitems << "/" << istream->nitems());
                }
            }
        ...
        if ( nout == -1 ) {
            ...
            if ( eos ) {
                LOG_TRACE(  streams_to_stream_ii_3i_base, "EOS SEEN, SENDING DOWNSTREAM " );
    ...
    if ( eos ) {
        ...
        for ( ; istream != istreams.end() ; istream++ ) {
            int idx=std::distance( istreams.begin(), istream );
            LOG_DEBUG( streams_to_stream_ii_3i_base, " CLOSING INPUT STREAM IDX:" << idx );
            istream->close();
        }
        ...
        for ( ; eos && ostream != ostreams.end(); ostream++ ) {
            ...
            LOG_DEBUG( streams_to_stream_ii_3i_base, " CLOSING OUTPUT STREAM IDX:" << idx );
            ostream->close();
        }
    }
    ...
    LOG_TRACE( streams_to_stream_ii_3i_base, " END OF TRANSFORM SERVICE FUNCTION....." << noutput_items );
...
template <  typename IN_PORT_TYPE, typename OUT_PORT_TYPE > int streams_to_stream_ii_3i_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams ,
                                 typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams  )
{
    typedef typename std::vector< gr_istream< IN_PORT_TYPE > >   _IStreamList;
    typedef typename std::vector< gr_ostream< OUT_PORT_TYPE > >  _OStreamList;
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( streams_to_stream_ii_3i_base, "GET MAX ITEMS: STREAM:"<< idx << " NITEMS/SCALARS:" << 
                       istream->nitems() << "/" << istream->_data.size() );
            max_items_avail = std::max( istream->nitems(), max_items_avail );
        }

        if ( max_items_avail == 0  ) {
            LOG_TRACE( streams_to_stream_ii_3i_base, "DATA CHECK - MAX ITEMS  NOUTPUT/MAX_ITEMS:" <<   noutput_items << "/" << max_items_avail);
            return -1;
        }
        ...
        if ( !gr_sptr->fixed_rate() )  {
            ...
            LOG_TRACE( streams_to_stream_ii_3i_base, " VARIABLE FORECAST NOUTPUT == " << noutput_items );
        } else {
            istream = istreams.begin();
            ...
            LOG_TRACE( streams_to_stream_ii_3i_base,  " FIXED FORECAST NOUTPUT/output_multiple == " << 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            LOG_TRACE( streams_to_stream_ii_3i_base, "--> FORECAST IN/OUT " << _ninput_items_required[0]  << "/" << noutput_items  );
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( streams_to_stream_ii_3i_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" <<   istream->nelems() << 
            ...
            LOG_TRACE( streams_to_stream_ii_3i_base, " TRIM FORECAST NOUTPUT/READY " << noutput_items << "/" << dataReady );
        }

        // check if data is ready...
        if ( !dataReady ) {
            LOG_TRACE( streams_to_stream_ii_3i_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" <<   _istreams[0].nitems() << 
        ...
        for ( int idx=0 ; istream != istreams.end(); idx++, istream++ ) {
            ...
            try {
                ...
                LOG_ERROR( streams_to_stream_ii_3i_base, "MISSING INPUT STREAM FOR GR BLOCK, STREAM ID:" <<   istream->streamID );
                return -2;
            } 
    
            nitems = istream->nitems() - ritems;
            LOG_TRACE( streams_to_stream_ii_3i_base,  " ISTREAM: IDX:" << idx  << " ITEMS AVAIL/READ/REQ " << nitems << "/" 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( streams_to_stream_ii_3i_base, " CALLING WORK.....N_OUT:" << noutput_items << " N_IN:" << nitems 
            ...
            LOG_TRACE( streams_to_stream_ii_3i_base, "RETURN  WORK ..... N_OUT:" << nout);
        }

        // check for stop condition from work method
        if ( nout < gr_block::WORK_DONE ) {
            LOG_WARN( streams_to_stream_ii_3i_base, "WORK RETURNED STOP CONDITION..." << nout );
            nout=0;
            eos = true;
        }
    }
    ...
    if (nout != 0 or eos ) {
        noutput_items = nout;
        LOG_TRACE( streams_to_stream_ii_3i_base, " WORK RETURNED: NOUT : " << nout << " EOS:" << eos);
        ostream = ostreams.begin();
        ...
        for ( int idx=0 ; ostream != ostreams.end(); idx++, ostream++ ) {
            ...
            LOG_TRACE( streams_to_stream_ii_3i_base,  "PUSHING DATA   ITEMS/STREAM_ID " << ostream->nitems() << "/" << ostream->streamID );    
            if ( _maintainTimeStamp ) {

                // set time stamp for output samples based on input time stamp
                if ( ostream->nelems() == 0 )  {
#ifdef TEST_TIME_STAMP
      LOG_DEBUG( streams_to_stream_ii_3i_base, "SEED - TS SRI:  xdelta:" << std::setprecision(12) << ostream->sri.xdelta );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "OSTREAM WRITE:   maint:" << _maintainTimeStamp );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                  mode:" <<  ostream->tstamp.tcmode );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                status:" <<  ostream->tstamp.tcstatus );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                offset:" <<  ostream->tstamp.toff );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                 whole:" <<  std::setprecision(10) << ostream->tstamp.twsec );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "SEED - TS         frac:" <<  std::setprecision(12) << ostream->tstamp.tfsec );
#endif
...
// use incoming packet's time stamp to forward
                if ( pkt ) {
#ifdef TEST_TIME_STAMP
      LOG_DEBUG( streams_to_stream_ii_3i_base, "OSTREAM  SRI:  items/xdelta:" << noutput_items << "/" << std::setprecision(12) << ostream->sri.xdelta );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "PKT - TS         maint:" << _maintainTimeStamp );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                  mode:" <<  pkt->T.tcmode );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                status:" <<  pkt->T.tcstatus );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                offset:" <<  pkt->T.toff );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                 whole:" <<  std::setprecision(10) << pkt->T.twsec );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "PKT - TS          frac:" <<  std::setprecision(12) << pkt->T.tfsec );
#endif
                    ostream->write( noutput_items, eos, pkt->T  );
                } else {
#ifdef TEST_TIME_STAMP
      LOG_DEBUG( streams_to_stream_ii_3i_base, "OSTREAM  SRI:  items/xdelta:" << noutput_items << "/" << std::setprecision(12) << ostream->sri.xdelta );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "OSTREAM TOD      maint:" << _maintainTimeStamp );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                  mode:" <<  ostream->tstamp.tcmode );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                status:" <<  ostream->tstamp.tcstatus );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                offset:" <<  ostream->tstamp.toff );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "                 whole:" <<  std::setprecision(10) << ostream->tstamp.twsec );
      LOG_DEBUG( streams_to_stream_ii_3i_base, "OSTREAM TOD       frac:" <<  std::setprecision(12) << ostream->tstamp.tfsec );
#endif
