...
#ifndef BIT_TIMING_LOOP_FF_2O_IMPL_BASE_H
#define BIT_TIMING_LOOP_FF_2O_IMPL_BASE_H
...
#include "bit_timing_loop_ff_2o_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class bit_timing_loop_ff_2o_base;
...
class bit_timing_loop_ff_2o_base : public GnuHawkBlock
{
    public:
        bit_timing_loop_ff_2o_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<bit_timing_loop_ff_2o_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::Long noutput_item;
        double a_mu;
        bool a_no_update;
        double tap;
        double rate;
        std::vector<CORBA::Long> ninput_items_required;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( bit_timing_loop_ff_2o_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( bit_timing_loop_ff_2o_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( bit_timing_loop_ff_2o_base, " serviceFunction: retval:" << retval);
