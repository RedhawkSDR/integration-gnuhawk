...
#ifndef SIG_SOURCE_F_IMPL_BASE_H
#define SIG_SOURCE_F_IMPL_BASE_H
...
#include "sig_source_f_GnuHawkBlock.h"


class sig_source_f_base : public GnuHawkBlock
{
    ...
    public:
        ...
        sig_source_f_base(const char *uuid, const char *label);
 ...
 template < typename OUT_PORT_TYPE > struct gr_ostream {
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<sig_source_f_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( sig_source_f_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( sig_source_f_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( sig_source_f_base, " serviceFunction: retval:" << retval);
