...
void sig_source_f_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    ...
    if (serviceThread == 0) {
        serviceThread = new ProcessThread<sig_source_f_base>(this, 0.1);
        serviceThread->start();
    }
...
BULKIO::StreamSRI sig_source_f_base::createOutputSRI( int32_t oidx ) {
...
BULKIO::StreamSRI sig_source_f_base::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
...
template < typename OUT_PORT_TYPE > int sig_source_f_base::_generatorServiceFunction( typename  std::vector< gr_ostream< OUT_PORT_TYPE > > &ostreams ) 
{
    ...
    if (numOut != 0 or (eos and !sentEOS)){
        ...
        ostream = ostreams.begin();
        for ( ; ostream != ostreams.end(); ostream++ ) {
            LOG_TRACE( sig_source_f_base, "PUSHING DATA   NOUT/NITEMS/OITEMS/STREAM_ID " << numOut << 
