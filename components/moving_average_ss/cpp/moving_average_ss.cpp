...
moving_average_ss_i::moving_average_ss_i(const char *uuid, const char *label) :
moving_average_ss_base(uuid, label)
{
}

moving_average_ss_i::~moving_average_ss_i()
{
}

void moving_average_ss_i::set_length_and_scale(const std::string &id)
{
    gr_sptr->set_length_and_scale(this->length, this->scale);
}
...
void moving_average_ss_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_moving_average_ss( this->length, this->scale, this->max_iter );
    } catch (...) {
        return;
    }

    this->registerGetter("length", &gr_moving_average_ss::length);
    this->registerGetter("scale", &gr_moving_average_ss::scale);

    this->setPropertyChangeListener("length", this, &moving_average_ss_i::set_length_and_scale);
    this->setPropertyChangeListener("scale", this, &moving_average_ss_i::set_length_and_scale);
...
//      
BULKIO::StreamSRI moving_average_ss_i::createOutputSRI( int32_t idx)
{
    //
