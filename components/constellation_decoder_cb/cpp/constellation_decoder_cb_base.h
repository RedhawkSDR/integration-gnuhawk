...
#ifndef CONSTELLATION_DECODER_CB_IMPL_BASE_H
#define CONSTELLATION_DECODER_CB_IMPL_BASE_H
...
#include "constellation_decoder_cb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class constellation_decoder_cb_base;
...
class constellation_decoder_cb_base : public GnuHawkBlock
{
    public:
        constellation_decoder_cb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<constellation_decoder_cb_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( constellation_decoder_cb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( constellation_decoder_cb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( constellation_decoder_cb_base, " serviceFunction: retval:" << retval);
