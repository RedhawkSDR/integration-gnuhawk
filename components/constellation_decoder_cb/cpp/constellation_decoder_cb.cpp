...
//  Allow for logging
//
PREPARE_LOGGING(constellation_decoder_cb_i);

constellation_decoder_cb_i::constellation_decoder_cb_i(const char *uuid, const char *label) :
constellation_decoder_cb_base(uuid, label)
{
}

constellation_decoder_cb_i::~constellation_decoder_cb_i()
{
}
...
void constellation_decoder_cb_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //

    digital_constellation_sptr c;
    if (constellation == 1) {
        c = digital_make_constellation_bpsk();
    } else if (constellation == 2) {
        c = digital_make_constellation_qpsk();
    } else if (constellation == 3) {
        c = digital_make_constellation_dqpsk();
    } else if (constellation == 4) {
        c = digital_make_constellation_8psk();
    } else {
        LOG_WARN(constellation_decoder_cb_i, "Invalid constellation");
        return;
    }

    try {
        gr_sptr = digital_make_constellation_decoder_cb( c );
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI constellation_decoder_cb_i::createOutputSRI( int32_t idx)
{
    //
