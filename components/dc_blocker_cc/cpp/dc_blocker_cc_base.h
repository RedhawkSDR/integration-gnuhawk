...
#ifndef DC_BLOCKER_CC_IMPL_BASE_H
#define DC_BLOCKER_CC_IMPL_BASE_H
...
#include "dc_blocker_cc_GnuHawkBlock.h"


class dc_blocker_cc_base : public GnuHawkBlock
{
  ...
  class SRIListener : BULKIO_dataFloat_In_i::SRIListener {
    ...
    friend class  dc_blocker_cc_base;
  ...
  SRIListener( dc_blocker_cc_base &parent ) : _parent(parent) {};

    dc_blocker_cc_base &_parent;
  };
    

    public:

        dc_blocker_cc_base(const char *uuid, const char *label);
...
template < typename IN_PORT_TYPE, typename OUT_PORT_TYPE >  int  _forecastAndProcess( bool &eos, std::vector< gr_istream< IN_PORT_TYPE > > &istreams,
 ...
 typedef std::deque< std::pair< BULKIO_dataFloat_In_i *, BULKIO::StreamSRI > >  SRIQueue;
  ...
  typedef _OStream< BULKIO_dataFloat_Out_i >::List   OStreamList;  
   ...
   ProcessThread<dc_blocker_cc_base> *serviceThread; 
 ...
 private:
  ...
  SF_State serviceFunction()
  {
	...
	LOG_TRACE( dc_blocker_cc_base, " SLEEP ...." << delta );
	boost::this_thread::sleep( delta );
      }
      else {
	LOG_TRACE( dc_blocker_cc_base, " NO SLEEPING...." );
      }
    }
    p_start_time = p_end_time;
       
    LOG_TRACE( dc_blocker_cc_base, " serviceFunction: retval:" << retval);
