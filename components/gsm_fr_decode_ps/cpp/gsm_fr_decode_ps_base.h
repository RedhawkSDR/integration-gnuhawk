...
#ifndef GSM_FR_DECODE_PS_IMPL_BASE_H
#define GSM_FR_DECODE_PS_IMPL_BASE_H
...
#include "gsm_fr_decode_ps_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class gsm_fr_decode_ps_base;
...
class gsm_fr_decode_ps_base : public GnuHawkBlock
{
    public:
        gsm_fr_decode_ps_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<gsm_fr_decode_ps_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Ports
        bulkio::InOctetPort *packet_in;
    ...
    protected:
        ...
        void  packet_in_newStreamCallback( BULKIO::StreamSRI &sri );
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( gsm_fr_decode_ps_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( gsm_fr_decode_ps_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( gsm_fr_decode_ps_base, " serviceFunction: retval:" << retval);
