...
gsm_fr_decode_ps_i::gsm_fr_decode_ps_i(const char *uuid, const char *label) :
gsm_fr_decode_ps_base(uuid, label)
{
}

gsm_fr_decode_ps_i::~gsm_fr_decode_ps_i()
{
}
...
void gsm_fr_decode_ps_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = vocoder_make_gsm_fr_decode_ps();
    } catch (...){
        return;
    }
...
//      
BULKIO::StreamSRI gsm_fr_decode_ps_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
