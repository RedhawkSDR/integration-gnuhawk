...
derandomizer_i::derandomizer_i(const char *uuid, const char *label) :
derandomizer_base(uuid, label)
{
}

derandomizer_i::~derandomizer_i()
{
}

void derandomizer_i::reset_state(const std::string &id)
{
    if (this->reset) {
        gr_sptr->reset();
    }
...
void derandomizer_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = atsc_make_derandomizer();
    } catch (...) {
        return;
    }

    this->setPropertyChangeListener("reset", this, &derandomizer_i::reset_state);
...
//      
BULKIO::StreamSRI derandomizer_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
