...
#ifndef OFDM_CYCLIC_PREFIXER_IMPL_BASE_H
#define OFDM_CYCLIC_PREFIXER_IMPL_BASE_H
...
#include "ofdm_cyclic_prefixer_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class ofdm_cyclic_prefixer_base;
...
class ofdm_cyclic_prefixer_base : public GnuHawkBlock
{
    public:
        ofdm_cyclic_prefixer_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<ofdm_cyclic_prefixer_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Member variables exposed as properties
        CORBA::ULong input_size;
        CORBA::ULong output_size;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( ofdm_cyclic_prefixer_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( ofdm_cyclic_prefixer_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( ofdm_cyclic_prefixer_base, " serviceFunction: retval:" << retval);
