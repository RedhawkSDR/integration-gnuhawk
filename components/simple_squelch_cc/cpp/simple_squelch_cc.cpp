...
simple_squelch_cc_i::simple_squelch_cc_i(const char *uuid, const char *label) :
simple_squelch_cc_base(uuid, label)
{
}

simple_squelch_cc_i::~simple_squelch_cc_i()
{
}
...
void simple_squelch_cc_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = gr_make_simple_squelch_cc( this->threshold_db, this->alpha );
    } catch (...) {
        return;
    }

    this->registerGetterSetter("threshold_db", &gr_simple_squelch_cc::threshold, &gr_simple_squelch_cc::set_threshold);
    this->registerSetter("alpha", &gr_simple_squelch_cc::set_alpha);
    this->registerGetter("unmuted", &gr_simple_squelch_cc::unmuted);
    this->registerGetter("squelch_range", &gr_simple_squelch_cc::squelch_range);
...
//      
BULKIO::StreamSRI simple_squelch_cc_i::createOutputSRI( int32_t idx)
{
    //
