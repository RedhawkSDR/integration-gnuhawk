...
#ifndef FLEX_DEINTERLEAVE_BI_IMPL_BASE_H
#define FLEX_DEINTERLEAVE_BI_IMPL_BASE_H
...
#include "flex_deinterleave_bi_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class flex_deinterleave_bi_base;
...
class flex_deinterleave_bi_base : public GnuHawkBlock
{
    public:
        flex_deinterleave_bi_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<flex_deinterleave_bi_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( flex_deinterleave_bi_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( flex_deinterleave_bi_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( flex_deinterleave_bi_base, " serviceFunction: retval:" << retval);
