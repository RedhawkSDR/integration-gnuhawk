...
#include "flex_deinterleave_bi.h"
...
flex_deinterleave_bi_i::flex_deinterleave_bi_i(const char *uuid, const char *label) :
flex_deinterleave_bi_base(uuid, label)
{
}

flex_deinterleave_bi_i::~flex_deinterleave_bi_i()
{
}
...
void flex_deinterleave_bi_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	try{
        gr_sptr = pager_make_flex_deinterleave();
	}
	catch (...){
		return;
	}
...
//      
BULKIO::StreamSRI flex_deinterleave_bi_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
