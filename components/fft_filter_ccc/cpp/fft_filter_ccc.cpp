...
fft_filter_ccc_i::fft_filter_ccc_i(const char *uuid, const char *label) :
fft_filter_ccc_base(uuid, label)
{
}

fft_filter_ccc_i::~fft_filter_ccc_i()
{
}

void fft_filter_ccc_i::setTaps(const std::string &id){
    this->gr_sptr->set_taps(taps);
}

std::vector<std::complex<float> > fft_filter_ccc_i::getTaps(){
...
void fft_filter_ccc_i::createBlock()
{
    //
    ...
    try {
       gr_sptr = gr::filter::fft_filter_ccc::make( this->decimation, this->taps, this->nthreads );
    }
    catch(...){
        return;
    }

    this->registerGetterSetter("nthreads",
                               &gr::filter::fft_filter_ccc::nthreads,
                               &gr::filter::fft_filter_ccc::set_nthreads);
    this->setPropertyChangeListener("taps", this, &fft_filter_ccc_i::setTaps);
    this->registerGetValue("taps", this, &fft_filter_ccc_i::getTaps);
...
//      
BULKIO::StreamSRI fft_filter_ccc_i::createOutputSRI( int32_t idx)
{
    //
