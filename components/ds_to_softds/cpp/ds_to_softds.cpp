...
ds_to_softds_i::ds_to_softds_i(const char *uuid, const char *label) :
ds_to_softds_base(uuid, label)
{
}

ds_to_softds_i::~ds_to_softds_i()
{
}
...
void ds_to_softds_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = atsc_make_ds_to_softds();
    } catch (...) {
        return;
    }
...
//      
BULKIO::StreamSRI ds_to_softds_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
