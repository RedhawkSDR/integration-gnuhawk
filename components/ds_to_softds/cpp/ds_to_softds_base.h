...
#ifndef DS_TO_SOFTDS_IMPL_BASE_H
#define DS_TO_SOFTDS_IMPL_BASE_H
...
#include "ds_to_softds_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class ds_to_softds_base;
...
class ds_to_softds_base : public GnuHawkBlock
{
    public:
        ds_to_softds_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<ds_to_softds_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Ports
        bulkio::InOctetPort *atsc_data_segment_in;
        bulkio::OutOctetPort *atsc_soft_data_segment_out;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( ds_to_softds_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( ds_to_softds_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( ds_to_softds_base, " serviceFunction: retval:" << retval);
