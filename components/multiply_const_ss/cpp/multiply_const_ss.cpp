...
multiply_const_ss_i::multiply_const_ss_i(const char *uuid, const char *label) :
multiply_const_ss_base(uuid, label)
{
}

multiply_const_ss_i::~multiply_const_ss_i()
{
}
...
//
void multiply_const_ss_i::createBlock()
{
  //
  // gr_sptr = xxx_make_xxx( args );
  //
  gr_sptr = gr_make_multiply_const_ss(k);
  ...
  setupIOMappings();
  this->registerGetterSetter("k", &gr_multiply_const_ss::k, &gr_multiply_const_ss::set_k);
} 
...
//      
BULKIO::StreamSRI multiply_const_ss_i::createOutputSRI( int32_t idx)
{
  //
