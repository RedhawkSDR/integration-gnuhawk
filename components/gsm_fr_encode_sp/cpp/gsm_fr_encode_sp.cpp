...
gsm_fr_encode_sp_i::gsm_fr_encode_sp_i(const char *uuid, const char *label) :
gsm_fr_encode_sp_base(uuid, label)
{
}

gsm_fr_encode_sp_i::~gsm_fr_encode_sp_i()
{
}
...
void gsm_fr_encode_sp_i::createBlock()
{
    //
    ...
    try {
        gr_sptr = vocoder_make_gsm_fr_encode_sp();
    } catch (...){
        return;
    }
...
//      
BULKIO::StreamSRI gsm_fr_encode_sp_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
