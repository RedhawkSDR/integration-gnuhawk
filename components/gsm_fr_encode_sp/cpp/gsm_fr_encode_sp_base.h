...
#ifndef GSM_FR_ENCODE_SP_IMPL_BASE_H
#define GSM_FR_ENCODE_SP_IMPL_BASE_H
...
#include "gsm_fr_encode_sp_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class gsm_fr_encode_sp_base;
...
class gsm_fr_encode_sp_base : public GnuHawkBlock
{
    public:
        gsm_fr_encode_sp_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<gsm_fr_encode_sp_base> *serviceThread; 
        boost::mutex serviceThreadLock;

        // Ports
        bulkio::InShortPort *short_in;
        bulkio::OutOctetPort *packet_out;
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( gsm_fr_encode_sp_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( gsm_fr_encode_sp_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( gsm_fr_encode_sp_base, " serviceFunction: retval:" << retval);
