...
#ifndef PA_2X2_PHASE_COMBINER_IMPL_BASE_H
#define PA_2X2_PHASE_COMBINER_IMPL_BASE_H
...
#include "pa_2x2_phase_combiner_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class pa_2x2_phase_combiner_base;
...
class pa_2x2_phase_combiner_base : public GnuHawkBlock
{
    public:
        pa_2x2_phase_combiner_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<pa_2x2_phase_combiner_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( pa_2x2_phase_combiner_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( pa_2x2_phase_combiner_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( pa_2x2_phase_combiner_base, " serviceFunction: retval:" << retval);
