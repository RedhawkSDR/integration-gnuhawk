...
#ifndef KEEP_M_IN_N_BB_IMPL_BASE_H
#define KEEP_M_IN_N_BB_IMPL_BASE_H
...
#include "keep_m_in_n_bb_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class keep_m_in_n_bb_base;
...
class keep_m_in_n_bb_base : public GnuHawkBlock
{
    public:
        keep_m_in_n_bb_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<keep_m_in_n_bb_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( keep_m_in_n_bb_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( keep_m_in_n_bb_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( keep_m_in_n_bb_base, " serviceFunction: retval:" << retval);
