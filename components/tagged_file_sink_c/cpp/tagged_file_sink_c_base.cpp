...
#include "tagged_file_sink_c_base.h"
...
tagged_file_sink_c_base::tagged_file_sink_c_base(const char *uuid, const char *label) :
...
void tagged_file_sink_c_base::construct()
{
    ...
    complex_in->setNewStreamListener(this, &tagged_file_sink_c_base::complex_in_newStreamCallback);
...
*******************************************************************************************/
void tagged_file_sink_c_base::initialize() throw (CF::LifeCycle::InitializeError, CORBA::SystemException)
{
}

void tagged_file_sink_c_base::start() throw (CORBA::SystemException, CF::Resource::StartError)
{
    boost::mutex::scoped_lock lock(serviceThreadLock);
    if (serviceThread == 0) {
        complex_in->unblock();
        serviceThread = new ProcessThread<tagged_file_sink_c_base>(this, 0.1);
        serviceThread->start();
    }
...
void tagged_file_sink_c_base::stop() throw (CORBA::SystemException, CF::Resource::StopError)
{
    ...
    if (serviceThread != 0) {
      {
        boost::mutex::scoped_lock lock(serviceThreadLock);
        LOG_TRACE( tagged_file_sink_c_base, "Stopping Service Function" );
        serviceThread->stop();
      }
    ...
    LOG_TRACE( tagged_file_sink_c_base, "COMPLETED STOP REQUEST" );
}

CORBA::Object_ptr tagged_file_sink_c_base::getPort(const char* _id) throw (CORBA::SystemException, CF::PortSupplier::UnknownPort)
{
...
void tagged_file_sink_c_base::releaseObject() throw (CORBA::SystemException, CF::LifeCycle::ReleaseError)
{
...
void tagged_file_sink_c_base::loadProperties()
{
...
//  Allow for logging 
// 
PREPARE_LOGGING(tagged_file_sink_c_base);
...
void tagged_file_sink_c_base::setupIOMappings( )
{
    ...
    LOG_DEBUG( tagged_file_sink_c_base, "GNUHAWK IO MAPPINGS IN/OUT " << ninput_streams << "/" << noutput_streams );
    ...
    if ( _istreams.size() > 0 ) {
        ...
        LOG_DEBUG( tagged_file_sink_c_base, "RESET INPUT SIGNATURE SIZE:" << _istreams.size() );
        IStreamList::iterator istream;
        for ( int idx=0 ; istream != _istreams.end(); idx++, istream++ ) {
            // re-add existing stream definitons
            LOG_DEBUG(  tagged_file_sink_c_base, "ADD READ INDEX TO GNU RADIO BLOCK");
   ...
   //
    LOG_TRACE( tagged_file_sink_c_base, "setupIOMappings INPUT PORTS: " << inPorts.size() );
    ...
    for( int i=0; pname != inputPortOrder.end(); pname++ ) {
        ...
        if ( p_in != inPorts.end() ) {
            ...
            LOG_DEBUG( tagged_file_sink_c_base, "ADDING INPUT MAP IDX:" << i << " SID:" << sid );
...
void tagged_file_sink_c_base::complex_in_newStreamCallback( BULKIO::StreamSRI &sri )
{
  LOG_TRACE( tagged_file_sink_c_base, "START NotifySRI  port:stream " << complex_in->getName() << "/" << sri.streamID);

  boost::mutex::scoped_lock lock(_sriMutex);
  _sriQueue.push_back( std::make_pair( complex_in, sri ) );

  LOG_TRACE( tagged_file_sink_c_base, "END  NotifySRI  QUEUE " << _sriQueue.size() << " port:stream " << complex_in->getName() << "/" << sri.streamID); 
}


void tagged_file_sink_c_base::processStreamIdChanges()
{
    boost::mutex::scoped_lock lock(_sriMutex);

    LOG_TRACE( tagged_file_sink_c_base, "processStreamIDChanges QUEUE: " << _sriQueue.size()  );
    ...
    if ( validGRBlock() ) {
        ...
        for ( ; item != _sriQueue.end(); item++ ) {
            ...
            for ( ; istream != _istreams.end(); idx++, istream++ ) {
                ...
                if ( istream->port == item->first ) {
                    LOG_DEBUG( tagged_file_sink_c_base,  "  SETTING IN_STREAM ID/STREAM_ID :" << idx << "/" << sid  );
                    istream->sri(true);
                    istream->spe(mode);
        ...
        LOG_WARN( tagged_file_sink_c_base, " NEW STREAM ID, NO GNU RADIO BLOCK DEFINED, SRI QUEUE SIZE:" << _sriQueue.size() );
    }

}

tagged_file_sink_c_base::TimeDuration tagged_file_sink_c_base::getTargetDuration()
{
    ...
    LOG_TRACE( tagged_file_sink_c_base, " SEC/USEC " << sec << "/"  << usec << "\n"  <<
              " target_duration " << t_drate );
    return t_drate;
}

tagged_file_sink_c_base::TimeDuration tagged_file_sink_c_base::calcThrottle( TimeMark &start_time,
                                             TimeMark &end_time )
{
    TimeDuration delta;
    ...
    if ( start_time.is_not_a_date_time() == false ) {
        ...
        delta /= 4;
        LOG_TRACE( tagged_file_sink_c_base, " s_time/t_dime " << s_dtime << "/" << target_duration << "\n"  <<
                  " delta " << delta );
    }
...
template < typename IN_PORT_TYPE > int tagged_file_sink_c_base::_analyzerServiceFunction( typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams )
{
    ...
    if ( validGRBlock() == false ) {
        ...
        LOG_DEBUG( tagged_file_sink_c_base, " FINISHED BUILDING  GNU RADIO BLOCK");
    }
    ...
    if ( !validGRBlock() || istreams.size() == 0 ) {
        LOG_WARN( tagged_file_sink_c_base, "NO STREAMS ATTACHED TO BLOCK..." );
        return NOOP;
    }
    ...
    for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
        ...
        if ( istream->overrun() ) {
            LOG_WARN( tagged_file_sink_c_base, " NOT KEEPING UP WITH STREAM ID:" << istream->streamID );
        }
    
        // RESOLVE issue when SRI changes that could affect the GNU Radio BLOCK
        if ( istream->sriChanged() ) {
            LOG_DEBUG( tagged_file_sink_c_base, "SRI CHANGED, STREAMD IDX/ID: " 
                   << idx << "/" << istream->pkt->streamID );
        }
    }

    LOG_TRACE( tagged_file_sink_c_base, "READ NITEMS: "  << nitems );
    ...
    while ( nout > -1 && serviceThread->threadRunning() ) {
            ...
            for ( ; istream != istreams.end(); istream++ ) {
                ...
                if ( _input_ready[idx] ) {
                    ...
                    if ( nitems > istream->nitems() ) {
                        LOG_WARN( tagged_file_sink_c_base,  "WORK CONSUMED MORE DATA THAN AVAILABLE,  READ/AVAILABLE " 
                                 << nitems << "/" << istream->nitems() );
                        nitems = istream->nitems();
                    }
                    istream->consume( nitems );
                    LOG_TRACE( tagged_file_sink_c_base, " CONSUME READ DATA  ITEMS/REMAIN " << nitems << "/" << istream->nitems());
                }
            }
        ...
        if ( nout == -1 ) {
            ...
            if ( eos ) {
                LOG_TRACE( tagged_file_sink_c_base, " DATA NOT READY, EOS:" << eos );
    ...
    if ( eos ) {
        ...
        for ( ; istream != istreams.end() ; istream++) {
            int idx=std::distance( istreams.begin(), istream );
            LOG_TRACE( tagged_file_sink_c_base, " CLOSING INPUT STREAM IDX:" << idx );
            istream->close();
        }
    }
    ...
    LOG_TRACE( tagged_file_sink_c_base, " END OF ANALYZER SERVICE FUNCTION....." << noutput_items );
...
template <  typename IN_PORT_TYPE > int tagged_file_sink_c_base::_forecastAndProcess( bool &eos, typename  std::vector< gr_istream< IN_PORT_TYPE > > &istreams )
{
    typedef typename std::vector< gr_istream< IN_PORT_TYPE > >   _IStreamList;
    ...
    if ( !eos ) {
        ...
        for ( int idx=0 ; istream != istreams.end() && serviceThread->threadRunning() ; idx++, istream++ ) {
            LOG_TRACE( tagged_file_sink_c_base, "GET MAX ITEMS: STREAM:" << idx << " NITEMS/SCALARS:"
        ...
        if ( noutput_items <= 0  ) {
           LOG_TRACE( tagged_file_sink_c_base, "DATA CHECK - MAX ITEMS  NOUTPUT/MAX_ITEMS:" <<   noutput_items << "/" << max_items_avail);
           return -1;
        }
        ...
        if ( gr_sptr->fixed_rate() ) {
            ...
            LOG_TRACE( tagged_file_sink_c_base, " FIXED FORECAST NOUTPUT/output_multiple == " 
        ...
        while ( !dataReady && noutput_items >= outMultiple  ) {
            ...
            LOG_TRACE( tagged_file_sink_c_base, "--> FORECAST IN/OUT " << _ninput_items_required[0]  << "/" << noutput_items  );
            ...
            for ( int idx=0 ; noutput_items > 0 && istream != istreams.end(); idx++, istream++ ) {
                ...
                LOG_TRACE( tagged_file_sink_c_base, "ISTREAM DATACHECK NELMS/NITEMS/REQ/READY:" << 
            ...
            LOG_TRACE( tagged_file_sink_c_base, " TRIM FORECAST NOUTPUT/READY " << noutput_items << "/" << dataReady );
        }

        // check if data is ready...
        if ( !dataReady ) {
            LOG_TRACE( tagged_file_sink_c_base, "DATA CHECK - NOT ENOUGH DATA  AVAIL/REQ:" 
        ...
        for ( int idx=0 ; istream != istreams.end(); idx++, istream++ ) {
            ...
            try {
                ...
                LOG_ERROR( tagged_file_sink_c_base, "MISSING INPUT STREAM FOR GR BLOCK, STREAM ID:" <<   istream->streamID );
                return -2;
            } 

            nitems = istream->nitems() - ritems;
            LOG_TRACE( tagged_file_sink_c_base,  " ISTREAM: IDX:" << idx  << " ITEMS AVAIL/READ/REQ " << nitems << "/" 
        ...
        if ( _input_items.size() != 0 && serviceThread->threadRunning() ) {
            LOG_TRACE( tagged_file_sink_c_base, " CALLING WORK.....N_OUT:" << noutput_items << 
            ...
            LOG_TRACE( tagged_file_sink_c_base, "RETURN  WORK ..... N_OUT:" << nout);
        }

        // check for stop condition from work method
        if ( nout < gr_block::WORK_DONE ) {
            LOG_WARN( tagged_file_sink_c_base, "WORK RETURNED STOP CONDITION..." << nout );
            nout=0;
            eos = true;
        }
    }
