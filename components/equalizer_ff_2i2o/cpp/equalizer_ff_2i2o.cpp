...
equalizer_ff_2i2o_i::equalizer_ff_2i2o_i(const char *uuid, const char *label) :
equalizer_ff_2i2o_base(uuid, label)
{
}

equalizer_ff_2i2o_i::~equalizer_ff_2i2o_i()
{
}

void equalizer_ff_2i2o_i::reset_data(const std::string &id)
{
	if (this->reset == true){
		gr_sptr->reset();
	}
...
void equalizer_ff_2i2o_i::createBlock()
{
    //
    // gr_sptr = xxx_make_xxx( args );
    //
	try {
		gr_sptr = atsc_make_equalizer();
	}
	catch(...) {
		return;
	}

	 this->setPropertyChangeListener("reset", this, &equalizer_ff_2i2o_i::reset_data);
...
//      
BULKIO::StreamSRI equalizer_ff_2i2o_i::createOutputSRI( int32_t oidx, int32_t &in_idx)
{
    //
