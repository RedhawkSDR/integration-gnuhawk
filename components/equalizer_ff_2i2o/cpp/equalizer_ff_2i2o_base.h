...
#ifndef EQUALIZER_FF_2I2O_IMPL_BASE_H
#define EQUALIZER_FF_2I2O_IMPL_BASE_H
...
#include "equalizer_ff_2i2o_GnuHawkBlock.h"

#define NOOP 0
#define FINISH -1
#define NORMAL 1

class equalizer_ff_2i2o_base;
...
class equalizer_ff_2i2o_base : public GnuHawkBlock
{
    public:
        equalizer_ff_2i2o_base(const char *uuid, const char *label);
    ...
    protected:
        ProcessThread<equalizer_ff_2i2o_base> *serviceThread; 
        ...
        int serviceFunction()
        {
            ...
            if ( retval == NORMAL && _throttle ) {
                ...
                if ( delta.is_not_a_date_time() == false && delta.is_negative() == false ) {
                    LOG_TRACE( equalizer_ff_2i2o_base, " SLEEP ...." << delta );
                    boost::this_thread::sleep( delta );
                } else {
                    LOG_TRACE( equalizer_ff_2i2o_base, " NO SLEEPING...." );
                }
            }
            p_start_time = p_end_time;
       
            LOG_TRACE( equalizer_ff_2i2o_base, " serviceFunction: retval:" << retval);
